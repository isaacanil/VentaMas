...
| Modal de inventario por vencimiento podia romper con batch eliminado y quedarse cargando | `ProductExpirySelection` fue retirado como componente huerfano; el hallazgo queda como snapshot historico de esa limpieza. | Medio | Bajo | Filtrar batches faltantes, fallback de `loading=false`, cancelar updates tardios. | Implementado y luego retirado junto con el componente legacy. |
