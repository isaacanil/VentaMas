...
Las Cloud Functions de autenticación v2 gestionan sesiones HTTPS, sincronizan presencia en tiempo real y mantienen auditoría de eventos mediante `sessionTokens` y `sessionLogs`. Además, Firestore TTL limpia tokens expirados para evitar acumulación manual.

## 🎯 Objetivos

- Unificar la creación, refresco y revocación de sesiones con políticas de intentos y límites de concurrencia.
- Exponer un historial trazable de eventos (`login`, `logout`, `view-session-logs`) que soporte auditoría y soporte.
- Automatizar la limpieza de tokens expirados mediante TTL y utilidades auxiliares para mantener el sistema saludable.
...
- 👍 Mantiene sesiones largas sin sacrificar control gracias a límites simultáneos y expiración automática.
- 👍 TTL + `cleanupOldTokens` reducen costos y evitan intervención manual en Firestore.
- ⚠️ Complejidad elevada en Functions v2; requiere monitoreo activo para detectar fallos de TTL o bloqueo de intentos.

## 🔜 Seguimiento / Próximos pasos

- [ ] Instrumentar métricas de tiempo promedio de sesión y rate de revocaciones automáticas.
- [ ] Documentar procedimiento de rotación de parámetros (`CLIENT_AUTH_*`) para entornos regulados.
- [ ] Evaluar migración de `sessionLogs` a BigQuery para análisis históricos si el volumen crece.
