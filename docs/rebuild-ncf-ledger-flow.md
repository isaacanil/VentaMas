...
## Entradas admitidas
- `businessId` *(obligatorio)*: puede llegar en varias rutas del payload (`data.businessId`, `data.business.id`, etc.).
- `userId` *(obligatorio)*: se toma de `data.userId`, `data.user.uid` o del contexto de autenticación.
