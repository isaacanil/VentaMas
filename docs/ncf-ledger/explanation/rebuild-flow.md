...
`rebuildNcfLedger` reconstruye el ledger de comprobantes fiscales leyendo facturas históricas por negocio y reescribiendo la colección `ncfLedger`. Se expone como función `onCall` para soporte interno.

## 🎯 Objetivos

- Permitir reconstrucciones parciales o totales de NCF con soporte de paginado y filtros por prefijo.
- Ofrecer controles como `dryRun` y `truncate` para estimar impacto o limpiar prefijos antes de reinsertar.
- Garantizar trazabilidad y validaciones de acceso para evitar ejecuciones no autorizadas.
...
- 👍 Permite recalibrar rápidamente el ledger ante errores históricos o migraciones sin restaurar backups completos.
- 👍 El modo `dryRun` evita escribir datos hasta confirmar la selección.
- ⚠️ Puede generar costos elevados (lecturas/escrituras) si se usa con `pageSize` grande o sin filtros.
- ⚠️ `truncate` es destructivo y requiere monitoreo cuidadoso.

## 🔜 Seguimiento / Próximos pasos

- [ ] Agregar métricas y dashboards de ejecución (tiempo, prefijos, errores).
- [ ] Persistir `lastDocId` para continuar automáticamente sin depender del cliente.
- [ ] Documentar el procedimiento para corridas masivas (producción) con aprobaciones previas.
