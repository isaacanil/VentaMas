...
| `src/components`                                                                                          | Componentes compartidos, UI legacy y componentes comunes.                            | Mantener shared UI real en `components/common` o paquetes UI acotados; los modales dominiales viven bajo `src/modules/<dominio>/components` y la orquestación global en `src/router/components/ModalManager.tsx`. |
...
| `src/models/Warehouse/{Batch,Movement,...}`, `src/firebase/warehouse/*`, `src/modules/inventory/**` y `src/domain/warehouse/*`                             | Warehouse aún tiene modelos estructurales legacy, pero product stock debe tratarse desde servicios/records y contratos activos de inventario. | Mantener solo contratos neutrales de warehouse en `src/domain` y evitar recrear los modelos retirados `ProductStock`/`Sale`. |
...
src/
  ...
  components/
    ...
    ui/                  # primitivos legacy/custom existentes; no abrir otro bucket global
