...
Activa el sistema de PIN de 6 dígitos para autorizar operaciones sensibles (Facturación, CxC) con un modal reutilizable y hooks.

## Lista de verificación

- [ ] Usuario administrador con acceso a Configuración de Autorización.
- [ ] PIN generado (vigente) para el módulo a proteger.
- [ ] Dependencias importadas: `useAuthorizationPin`, `PinAuthorizationModal`.
- [ ] Logs de auditoría habilitados (`pinAuthLogs`).
...
- Genera un PIN, valida que expira en 24 h y se registre en `pinAuthLogs`.
- Ejecuta `useAuthorizationPin` en modo sandbox y confirma que `onAuthorized` recibe el autor aprobador.
- Simula errores (PIN incorrecto, módulo no habilitado) y verifica mensajes adecuados.
- Revisa Firestore (`pinAuthLogs`) para asegurar que cada autorización produce un registro.
