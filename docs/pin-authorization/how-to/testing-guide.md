...
Checklist de pruebas manuales para validar generación de PINs, autorización en facturación y manejo de errores.

## Lista de verificación

- [ ] Usuario admin y usuario cajero disponibles.
- [ ] PIN activo generado para al menos un módulo.
- [ ] Solicitud de factura pendiente (para probar aprobación).
- [ ] Acceso a Firestore para revisar logs.
...
- Confirma que todos los items del checklist estén marcados y documentados.
- Revisa `pinAuthLogs` para cada escenario (generate, validate, fail, deactivate).
- Valida que estadísticas y estados de PIN cambien según cada prueba (activo/inactivo, módulos).
