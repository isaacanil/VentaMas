...
El digest diario de inventario recorre cada negocio, evalúa umbrales personalizados y envía correos cuando encuentra productos con existencias bajas o críticas. Este documento describe el diseño actual del job programado (`stockAlertsDailyDigest`) y su configuración operativa.

## 🎯 Objetivos

- Detectar stock bajo/crítico de forma recurrente sin intervención humana.
- Entregar correos consolidados por negocio con métricas claras y filtrado de destinatarios.
- Mantener el sistema parametrizable (CRON, límites, dominios, SMTP) para distintos entornos.
...
- 👍 Permite monitorear inventario críticamente bajo sin depender de usuarios conectados.
- 👍 Altamente configurable: límites, dominios y correos se pueden ajustar por entorno/negocio.
- ⚠️ Dependencia del proveedor SMTP; fallos bloquean notificaciones hasta siguiente corrida.
- ⚠️ Firestore puede requerir índices adicionales y generar costos si se subestima el límite de negocios.

## 🔜 Seguimiento / Próximos pasos

- Integrar envío inmediato (onWrite) para eventos críticos.
- Añadir cache o storage compartido de thresholds si múltiples funciones reutilizan estos datos.
- Implementar paginación/multi-job cuando el número de negocios supere el límite actual.
- Permitir plantillas HTML por negocio y evaluar integración con Firebase Trigger Email.
- Explorar colas de reintento (Cloud Tasks) para rechazos temporales del SMTP.
