...
Cuando se ejecuta `fbCompletePurchase`, la plataforma debe convertir cada línea de compra en lotes (`batches`), existencias físicas (`productsStock`), movimientos y actualizaciones de backorders. Este flujo es crítico para evitar diferencias entre compras y el inventario real.

## 🎯 Objetivos

- Garantizar que cada compra completada cree lotes y stocks consistentes (producto + fecha de expiración).
- Registrar movimientos y actualizaciones de backorders para mantener trazabilidad completa.
- Detectar oportunidades de mejora (ubicaciones, transacciones, validaciones) dentro del pipeline actual.
...
- 👍 Trazabilidad completa: cada compra deja batches, productStock, movimientos y actualizaciones de backorders.
- 👍 El agrupado por producto+fecha evita duplicados y simplifica consultas posteriores.
- ⚠️ La ausencia de transacciones puede provocar inconsistencias si el flujo se interrumpe a mitad; requiere tareas de sanidad (cron).
- ⚠️ `location` aún no usa objeto normalizado, lo que complica futuros reportes multi-almacén.

## 🔜 Seguimiento / Próximos pasos

- [ ] Unificar el campo `location` para que todo productStock tenga estructura `{ warehouse, shelf, row, segment }`.
- [ ] Evaluar transacciones o colas para asegurar atomicidad en compras con muchos productos.
- [ ] Documentar el flujo inverso (anulación / ajuste) para cerrar el ciclo operativo.
