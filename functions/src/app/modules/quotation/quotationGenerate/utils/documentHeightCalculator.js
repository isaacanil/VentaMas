...
export function calcHeaderHeight(biz, d) {
  ...
  const quotationFields = [
    ...
    `No: ${d.numberID || '-'}`,
    d.expirationDate
      ? `Vence: ${formatDateForCalculation(d.expirationDate)}`
      : null,
  ].filter(Boolean);
  ...
  if (shouldShowClientBlock(d)) {
    ...
    const clientFields = [
      ...
      d.client?.personalID?.trim()
        ? `RNC cliente: ${d.client.personalID.trim()}`
        : null,
    ].filter(Boolean);
...
export function calcFooterHeight(biz, d) {
  ...
  if (creditNotes.length > 0) {
    ...
    creditNotes.forEach((note) => {
      ...
      height +=
        calculateTextHeight(noteText, STYLES.default.fontSize) +
        STYLES.default.lineHeight;
    });
  }
