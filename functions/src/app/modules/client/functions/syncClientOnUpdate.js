...
export const syncClientOnUpdate = onDocumentUpdated(
  ...
  async (event) => {
    ...
    const changed = Object.keys(nextClient || {}).some(
      (key) => !isEqualValue(nextClient[key], beforeClient?.[key]),
    );
    if (!changed) return;
    ...
    const updateInvoices = async () => {
      ...
      for await (const docSnap of query.stream()) {
        ...
        writer.update(
          docSnap.ref,
          buildNestedUpdate('data.client', nextClient),
        );
        updatedCount += 1;
        stats.updated += 1;
      }
    ...
    const updateReceipts = async () => {
      ...
      if (updatedCount > 0) {
        stats.updatedByCollection.accountsReceivablePaymentReceipt =
          updatedCount;
      }
    };
