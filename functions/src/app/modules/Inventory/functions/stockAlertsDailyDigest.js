...
const loadDependencies = async () => {
  if (!dependenciesPromise) {
    dependenciesPromise = (async () => {
      const [excelModule, firebaseModule, mailerModule] = await Promise.all([
        import('exceljs'),
        import('../../../core/config/firebase.js'),
        import('../../../core/config/mailer.js'),
      ]);
...
export const stockAlertsDailyDigest = onSchedule(
  ...
  async () => {
    const { ExcelJS, db, storage, sendMail } = await loadDependencies();
    const startedAt = Date.now();
    logger.info('[stockDigest] Inicio ejecución');

    // Flags de control
    const verbose = /^true$/i.test(
      val(PARAM_DIGEST_VERBOSE, 'DIGEST_VERBOSE', 'false'),
    );
    const debug = /^true$/i.test(
      val(PARAM_STOCK_ALERT_DEBUG, 'STOCK_ALERT_DEBUG', 'false'),
    );
    const dryRun = /^true$/i.test(
      val(PARAM_STOCK_ALERT_DRY_RUN, 'STOCK_ALERT_DRY_RUN', 'false'),
    );
    if (debug) logger.info('[stockDigest] Debug activado', { dryRun });

    const businessLimit =
      Number(
        val(PARAM_DIGEST_BUSINESS_LIMIT, 'DIGEST_BUSINESS_LIMIT', '100'),
      ) || 100;
    ...
    const orderField = val(
      PARAM_DIGEST_BUSINESS_ORDER_FIELD,
    ...
    try {
      ...
      logger.warn(
        '[stockDigest] Fallback sin orderBy (campo faltante o índice no creado)',
        {
          orderField,
          error: err.message,
        },
      );
      snapshot = await db.collection('businesses').limit(businessLimit).get();
    }

    if (debug)
      logger.info('[stockDigest] Negocios obtenidos', {
    ...
    const allowedDomainsEnv = val(
      PARAM_ALLOWED_DOMAINS,
    ...
    for (const bizDoc of snapshot.docs) {
      ...
      if (!billingSnap.exists) {
        cNoBilling++;
        if ((verbose || debug) && idsNoBilling.length < 200)
          idsNoBilling.push(bid);
        if (verbose || debug)
          logger.info('[stockDigest] Skip negocio sin billing settings', {
            bid,
          });
        continue;
      }
      ...
      if (!billing.stockAlertsEnabled) {
        cDisabled++;
        if ((verbose || debug) && idsDisabled.length < 200)
          idsDisabled.push(bid);
        if (verbose || debug)
          logger.info('[stockDigest] Skip negocio con alertas deshabilitadas', {
            bid,
          });
        continue;
      }
      ...
      if (!toRaw) {
        cNoEmails++;
        if ((verbose || debug) && idsNoEmails.length < 200)
          idsNoEmails.push(bid);
        if (verbose || debug)
          logger.info('[stockDigest] Skip negocio sin correos configurados', {
            bid,
          });
        continue;
      }
      ...
      const filteredRecipients = unrestricted
        ? recipients
        : recipients.filter((r) =>
          allowedDomains.includes(r.split('@').pop().toLowerCase()),
        );
      if (filteredRecipients.length === 0) {
        logger.warn(
          '[stockDigest] Todos los correos filtrados por dominios no permitidos',
          {
            bid,
            toRaw,
          },
      ...
      const low = Number.isFinite(billing.stockLowThreshold)
        ? billing.stockLowThreshold
        : 20;
      const critical = Number.isFinite(billing.stockCriticalThreshold)
      ...
      try {
        ...
        logger.warn(
          '[stockDigest] Fallback de query de productos (verifica índices y filtros)',
          {
            bid,
            error: err.message,
          },
        );
        const fallbackSnap = await colRef
          .where('quantity', '<=', low)
          .limit(500)
          .get();
      ...
      if (combinedDocs.size === 0) {
        cNoProducts++;
        if ((verbose || debug) && idsNoProducts.length < 200)
          idsNoProducts.push(bid);
        if (verbose || debug)
          logger.info('[stockDigest] Negocio sin productos bajo umbral', {
            bid,
            low,
          });
        continue;
      ...
      if (lowList.length === 0 && criticalList.length === 0) {
        cEmptyAfterClass++;
        if ((verbose || debug) && idsEmptyAfterClass.length < 200)
          idsEmptyAfterClass.push(bid);
        if (verbose || debug)
          logger.info('[stockDigest] Lista filtrada vacía tras clasificación', {
            bid,
          });
        continue;
      }
      ...
      if (verbose || debug)
        logger.info('[stockDigest] Procesando negocio con alertas', {
          bid,
    ...
    */
      ...
      async function buildStrictCriticalRows() {
        try {
          const prodSnap = await db
            .collection(`businesses/${bid}/products`)
            .where('restrictSaleWithoutStock', '==', true)
            .where('trackInventory', '==', true)
            .limit(500)
            .get();
          ...
          for (let i = 0; i < prodIds.length; i += 10)
            chunks.push(prodIds.slice(i, i + 10));
          const stockDocs = [];
          for (const ids of chunks) {
            const snap = await db
              .collection(`businesses/${bid}/productsStock`)
              .where('productId', 'in', ids)
              .where('isDeleted', '==', false)
              .where('status', '==', 'active')
              .get();
          ...
          const warehouseMap = new Map();
          if (uniqueWarehouseIds.size) {
            const refs = Array.from(uniqueWarehouseIds).map((id) =>
              db.doc(`businesses/${bid}/warehouses/${id}`),
            );
            try {
              const snaps = await Promise.all(refs.map((r) => r.get()));
              snaps.forEach((snap) => {
                if (snap.exists)
                  warehouseMap.set(
                    snap.id,
                    snap.data().name || snap.data().shortName || snap.id,
                  );
              });
            } catch {
              /* ignore warehouse lookup errors */
            }
          }
          const fmtDate = (d) => {
            try {
          ...
          async function resolveShelf(businessId, warehouseId, shelfId) {
            ...
            try {
              ...
              const snap = await db
                .doc(
                  `businesses/${businessId}/warehouses/${warehouseId}/shelves/${shelfId}`,
                )
                .get();
              if (snap.exists) {
                const data = snap.data() || {};
                name = data.name || data.shortName || data.title || '';
              } else {
                // Fallback a colección directa si existiera
                const fb = await db
                  .doc(`businesses/${businessId}/shelves/${shelfId}`)
                  .get();
                if (fb.exists) {
                  const d = fb.data() || {};
                  name = d.name || d.shortName || d.title || '';
                }
              }
            } catch {
              /* noop */
            ...
            if (debug)
              logger.debug?.('[stockDigest] shelf resolved', {
                bid: businessId,
                warehouseId,
                shelfId,
                name,
          ...
          async function resolveRow(businessId, warehouseId, shelfId, rowId) {
            ...
            try {
              const snap = await db
                .doc(
                  `businesses/${businessId}/warehouses/${warehouseId}/shelves/${shelfId}/rows/${rowId}`,
                )
                .get();
              if (snap.exists) {
                const data = snap.data() || {};
                ...
                const fb = await db
                  .doc(`businesses/${businessId}/rows/${rowId}`)
                  .get();
                if (fb.exists) {
                  const d = fb.data() || {};
                  name = d.name || d.shortName || d.title || '';
            ...
            if (debug)
              logger.debug?.('[stockDigest] row resolved', {
                bid: businessId,
                warehouseId,
                shelfId,
                rowId,
          ...
          async function resolveSegment(
            ...
            try {
              const snap = await db
                .doc(
                  `businesses/${businessId}/warehouses/${warehouseId}/shelves/${shelfId}/rows/${rowId}/segments/${segmentId}`,
                )
                .get();
              if (snap.exists) {
                const data = snap.data() || {};
                ...
                const fb = await db
                  .doc(`businesses/${businessId}/segments/${segmentId}`)
                  .get();
                if (fb.exists) {
                  const d = fb.data() || {};
                  name = d.name || d.shortName || d.title || '';
            ...
            if (debug)
              logger.debug?.('[stockDigest] segment resolved', {
                bid: businessId,
                warehouseId,
                shelfId,
                rowId,
          ...
          for (const s of stockDocs) {
            ...
            const whName = warehouseId
              ? warehouseMap.get(warehouseId) || warehouseId
              : '';
            ...
            let segmentName = '';
            if (shelfId)
              shelfName = await resolveShelf(bid, warehouseId, shelfId);
            if (rowId)
              rowName = await resolveRow(bid, warehouseId, shelfId, rowId);
            if (segmentId)
              segmentName = await resolveSegment(
                bid,
                warehouseId,
                shelfId,
                rowId,
            ...
            const locLabel = [whName, shelfName, rowName, segmentName]
              .filter(Boolean)
              .join(' / ');
            rows.push({
              productName:
                p.name || p.productName || s.productName || 'Producto',
              batchNo: s.batchNumberId ?? '',
              exp: fmtDate(s.expirationDate),
              location: locLabel || String(s.location || ''),
              qty,
          ...
          logger.warn('[stockDigest] StrictCritical builder error', {
            bid,
            error: e.message,
          });
          return [];
        ...
        try {
          ...
          try {
            ...
            activeSnap.forEach((d) =>
              stockDocs.push({ id: d.id, ...d.data() }),
            );
            nullSnap.forEach((d) => stockDocs.push({ id: d.id, ...d.data() }));
          } catch (err) {
            logger.warn(
              '[stockDigest] Fallback de query de vencimientos (verifica índices)',
              {
                bid,
                error: err.message,
              },
            );
            const fbSnap = await colRef
              .where('expirationDate', '<=', end)
              .limit(500)
              .get();
                ...
                try {
                  ...
                  const dt = v?.toDate
                    ...
                    : typeof v?.seconds === 'number'
                      ...
                      : new Date(v);
                  return (
                    dt && !Number.isNaN(dt.getTime()) && dt >= now && dt <= end
                  );
                } catch {
                  return false;
                }
          ...
          for (const s of stockDocs) {
            ...
            const locLabel = [whName, shelfName, rowName, segmentName]
              ...
              .join(' / ');
            rows.push({
              productName: s.productName || s.productId || 'Producto',
              batchNo: s.batchNumberId ?? '',
              exp: fmtDate(s.expirationDate),
              location: locLabel || String(s.location || ''),
              qty,
          ...
          logger.warn('[stockDigest] ExpiringSoon builder error', {
            bid,
            error: e.message,
          });
          return [];
      ...
      if (!strictRows.length) {
        ...
        if ((verbose || debug) && idsEmptyAfterClass.length < 200)
          idsEmptyAfterClass.push(bid);
        if (verbose || debug)
          logger.info('[stockDigest] Sin productos críticos estrictos', {
            bid,
          });
        continue;
      }
      ...
      ws.columns = [
        ...
        { header: 'Lote', key: 'batchNo', width: 12 },
        { header: 'Vencimiento', key: 'exp', width: 14 },
        { header: 'Ubicación', key: 'location', width: 40 },
        { header: 'Cantidad', key: 'qty', width: 12 },
      ];
      strictRows.forEach((r) => ws.addRow(r));
      ...
      try {
        if (dryRun) {
          logger.info('[stockDigest] DRY RUN - se omite envío', {
            bid,
            businessName,
          ...
          if (debug)
            logger.info('[stockDigest] Enviando correo', {
              bid,
              businessName,
          ...
          await sendMail({
            ...
            subject,
            html: expRows && expRows.length ? html + expSection : html,
            text,
          });
          logger.info('[stockDigest] Correo enviado', {
            bid,
            businessName,
        ...
        logger.error('[stockDigest] Error enviando resumen', {
          bid,
          error: err.message,
        });
      }
    ...
    if (processedBusinesses === 0) {
      logger.info(
        '[stockDigest] Fin ejecución (sin negocios con alertas)',
        summary,
      );
    } else {
