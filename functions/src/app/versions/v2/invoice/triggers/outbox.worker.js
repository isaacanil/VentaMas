...
export const processInvoiceOutbox = onDocumentCreated(
  ...
  async (event) => {
    ...
    if (!snap) {
      logger.warn('processInvoiceOutbox missing snapshot');
      return null;
    }

    const { businessId, invoiceId, taskId } = event.params;
    ...
    try {
      await db.runTransaction(async (tx) => {
        ...
        if (type === 'updateInventory') {
          ...
          const snapshotDateTs = normalizeTimestamp(
            invoice?.snapshot?.createdAt,
          );
          ...
          try {
            ...
            if (canonicalSnap.exists) {
              ...
              canonicalInvoiceData = canonicalData;
            ...
            logger.warn(
              'Unable to read canonical invoice for cash count hint',
              {
                businessId,
                invoiceId,
          ...
          if (!ccSnap) {
            try {
              const fallbackSnap = await getCashCount.getOpenCashCountDocFromTx(
                tx,
                user,
              );
          ...
          if (!ccSnap) {
            logger.error('attachToCashCount could not resolve cash count', {
              businessId,
              invoiceId,
              candidateIds,
              candidateLookupTrace,
              fallbackFailureReason,
            });
            const serializedCandidates =
              candidateIds.length > 0 ? candidateIds.join('|') : 'none';
            const serializedFallback = fallbackFailureReason || 'none';
            throw new Error(
              `cash_count_not_found(candidateIds=${serializedCandidates},fallback=${serializedFallback})`,
            );
          }
          ...
          if (isAccountingRolloutEnabledForBusiness(businessId)) {
            const fallbackInvoiceSource =
              payload?.cart || invoice?.snapshot?.cart || invoice?.snapshot || {};
            const invoiceMovementSource =
              canonicalInvoiceData && Object.keys(canonicalInvoiceData).length > 0
                ? canonicalInvoiceData
                : fallbackInvoiceSource;
            const invoiceCreatedAt =
              normalizeTimestamp(invoiceMovementSource?.date) ||
              normalizeTimestamp(invoice?.createdAt) ||
              Timestamp.now();
            const cashMovements = buildInvoicePosCashMovements({
              businessId,
              invoice: invoiceMovementSource,
              invoiceId,
          ...
          if (totalInstallments > 0 && invoiceId) {
            try {
              ...
              if (!existingSnap.empty) {
                ...
                logger.info(
                  'Invoice already has accounts receivable, skipping creation',
                  {
                    businessId,
                    invoiceId,
                    existingArId,
          ...
          if (
            ...
            auditTx(tx, {
              ...
              data: {
                taskId,
                type,
                reason: 'receivable_already_exists',
                existingArId,
              },
            });
          } else {
