...
export async function addInsuranceAuth(tx, { user, authData, clientId }) {
  ...
  if (!authData?.insuranceId || !authData?.authNumber) {
    throw new https.HttpsError(
      'invalid-argument',
      'Datos de autorización incompletos',
    );
  }
  if (!clientId) {
  ...
  const dupQuery = db
    .collection(INSURANCE_AUTHS_COLLECTION)
  ...
  if (!dupSnap.empty) {
    throw new Error(
      'Ya existe una autorización con ese número para esta aseguradora',
    );
  }
  ...
  logger.info(`Autorización de seguro creada con ID: ${id}`, {
    userId: user.uid,
    businessId: user.businessID,
  });
  return id;
