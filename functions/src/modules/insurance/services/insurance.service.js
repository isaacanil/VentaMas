...
export async function getInsuranceSnap(tx, { user, insuranceId }) {
  if (!user?.businessID) {
    logger.error('User missing businessID in getInsuranceSnap', {
      uid: user?.uid,
    });
...
export async function getInsurance(tx, { user, insuranceId }) {
  if (!user?.businessID) {
    logger.error('User missing businessID in getInsuranceTransactional', {
      uid: user?.uid,
    });
    throw new https.HttpsError(
      'invalid-argument',
  ...
  if (!insuranceId || typeof insuranceId !== 'string') {
    logger.error('Invalid insuranceId in getInsuranceTransactional', {
      insuranceId,
    });
    throw new https.HttpsError(
      'invalid-argument',
  ...
  if (!snap.exists()) {
    logger.error('Insurance document not found in getInsuranceTransactional', {
      insuranceId,
    });
    throw new https.HttpsError(
      'not-found',
  ...
  logger.info('Insurance data retrieved (tx)', {
    insuranceId,
