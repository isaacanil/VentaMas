...
export const stockAlertsOnWrite = onDocumentWritten({
  ...
  const low = Number.isFinite(settings.stockLowThreshold) ? settings.stockLowThreshold : 20;
  const critical = Number.isFinite(settings.stockCriticalThreshold) ? settings.stockCriticalThreshold : 10;
  const toEmail = (settings.stockAlertEmail || '').trim();
  if (!toEmail) {
    logger.warn('[stockAlerts] No hay correo configurado para alertas', { bid });
    return;
  }
  ...
  // =============================================================
  const allowedDomainsEnv = process.env.STOCK_ALERT_ALLOWED_RECIPIENT_DOMAINS || '';
  ...
  if (filteredRecipients.length === 0) {
    logger.warn('[stockAlerts] Todos los correos filtrados por dominios no permitidos', { bid, recipientList, allowedDomains });
    return;
  }
  ...
  if (!criticalSent && newQty <= critical && (prevQty === undefined || prevQty > critical)) {
    level = 'CRITICO';
    criticalSent = true;
    lowSent = true; // si es crítico ya no hace falta enviar el bajo luego hasta reset
    shouldSend = true;
  } else if (!lowSent && newQty <= low && (prevQty === undefined || prevQty > low)) {
    level = 'BAJO';
    lowSent = true;
    shouldSend = true;
  }
  ...
  if (newQty > low && (lowSent || criticalSent)) {
    ...
    logger.info('[stockAlerts] Reset de flags por recuperación de stock', { path: event.data.after.ref.path, newQty });
    return;
  }

  if (!shouldSend) {
    // Si no hay email que enviar pero sí cambian flags (p.e. marcamos lowSent) guardar.
    if (level && (lowSent !== alertsFlags.lowSent || criticalSent !== alertsFlags.criticalSent)) {
      updates['alerts'] = { lowSent, criticalSent };
      await db.doc(event.data.after.ref.path).set(updates, { merge: true });
    }
    return;
  ...
  try {
    ...
    await db.doc(event.data.after.ref.path).set(updates, { merge: true });
    logger.info('[stockAlerts] Correo enviado (SMTP)', { productName, level, qty: newQty, path: event.data.after.ref.path });
  } catch (err) {
    logger.error('[stockAlerts] Error enviando correo', { error: err.message, stack: err.stack });
  }
});
