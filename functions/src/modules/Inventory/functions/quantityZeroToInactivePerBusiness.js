...
// ==== CONFIGURACIÓN ====
const TIME_ZONE =
  process.env.QUANTITY_CRON_TIME_ZONE || 'America/Santo_Domingo';
...
const INCLUDE_NEGATIVES = /^true$/i.test(
  process.env.QUANTITY_CRON_INCLUDE_NEGATIVES || 'false',
);
const TARGET_COLLECTIONS = (
  process.env.QUANTITY_CRON_TARGETS || 'batches,productsStock'
)
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
...
const BUSINESS_ID =
  (process.env.QUANTITY_CRON_BUSINESS_ID || '').trim() || null;
...
export const quantityZeroToInactivePerBusiness = onSchedule(
  ...
  async () => {
    ...
    for (const col of TARGET_COLLECTIONS) {
      try {
        const { scanned, eligible, scheduledUpdates } =
          await scanAndUpdateCollectionGroup(writer, col);
