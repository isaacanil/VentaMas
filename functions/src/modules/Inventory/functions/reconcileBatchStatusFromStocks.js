...
const fetchBatchSnapshots = async (refs, chunkSize = 300) => {
  const snaps = [];
...
export const reconcileBatchStatusFromStocks = onCall(
  ...
  async (req) => {
    ...
    const batchSnaps = await fetchBatchSnapshots(batchRefs);
    ...
    for (const [batchId, entry] of perBatch.entries()) {
      ...
      const expectedQty =
        entry.activeChildren > 0 ? entry.activeQuantity : 0;
