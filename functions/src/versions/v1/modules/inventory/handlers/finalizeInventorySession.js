  ...
  try {
    ...
    const toLocalNoonTimestamp = (dateStr) => {
      ...
      try {
        ...
        logger.error(
          '[finalizeInventorySession] Fecha de expiración inválida',
          { dateStr, e },
        );
        return null;
      }
    ...
    const planAdjustmentsForSources = (sourceIds, targetTotal) => {
      const sourceStocks = sourceIds
        .map((id) => stockById.get(id))
        .filter(Boolean);
      const currentTotal = sourceStocks.reduce(
        (acc, s) => acc + Number(s.quantity ?? s.stock ?? 0),
        0,
      );
    ...
    for (const g of groups || []) {
      for (const child of g?._children || []) {
        ...
        const altCountKeys = (sourceIds || []).map(
          (id) => `batchGroup:stock:${id}`,
        );
        ...
        if (Object.hasOwn(counts, key)) {
          ...
          for (const ak of altCountKeys) {
            if (Object.hasOwn(counts, ak)) {
              rawTarget = Number(counts[ak]);
              break;
            }
          }
          if (rawTarget === undefined)
            rawTarget = Number(child?.real ?? child?.stock ?? 0);
        }
        ...
        if (isFinite(rawTarget) && rawTarget < 0) {
          ...
          if (pid && qty > 0)
            negBackorderByProduct.set(
              pid,
              (negBackorderByProduct.get(pid) || 0) + qty,
            );
        }
        ...
        const isSyntheticGroup =
          sourceIds.length > 0 &&
          sources.length === 0 &&
          (key?.startsWith?.('noexp:') ||
            key?.startsWith?.('batchGroup:stock:'));
        if (isSyntheticGroup) {
          ...
          if (finalTarget > 0) {
            ...
            if (manualExp === null || manualExp === undefined) {
              const pKey =
                g?.productKey || g?.productId || g?.productName || '';
              const altNoexpKey = pKey ? `noexp:${pKey}` : null;
              if (altNoexpKey && Object.hasOwn(countsMeta || {}, altNoexpKey)) {
                manualExp =
                  countsMeta[altNoexpKey]?.manualExpirationDate ?? manualExp;
              }
            }
            if (
              (manualExp === null || manualExp === undefined) &&
            ...
            syntheticCreations.push({
              ...
              expirationDate:
                manualExp && manualExp !== '__REMOVE__' ? manualExp : null,
        ...
        if (meta && Object.hasOwn(meta, 'manualExpirationDate')) {
          ...
          for (const s of sources) {
            ...
            const currTS = current?.toDate
              ? current.toDate().toISOString().slice(0, 10)
              : current;
            const willTS = newVal?.toDate
              ? newVal.toDate().toISOString().slice(0, 10)
              : newVal;
            const differs = (currTS || null) !== (willTS || null);
            if (!differs) continue;
            expirationUpdates.push({
              productStockId: s.id,
              from: current,
              to: newVal,
              productId: s.productId,
    ...
    for (const adj of adjustments) {
      const stockRef = db.doc(
        `businesses/${businessID}/productsStock/${adj.productStockId}`,
      );
      writes.push((b) =>
        b.update(stockRef, {
          quantity: adj.toQty,
      ...
      if (adj.batchId) {
        const batchRef = db.doc(
          `businesses/${businessID}/batches/${adj.batchId}`,
        );
        writes.push((b) =>
          b.update(batchRef, {
    ...
    for (const exp of expirationUpdates) {
      const stockRef = db.doc(
        `businesses/${businessID}/productsStock/${exp.productStockId}`,
      );
      writes.push((b) =>
        b.update(stockRef, {
    ...
    if (writes.length) {
      try {
        ...
        logger.error('[finalizeInventorySession] Error commit ajustes:', e);
      }
    }
    ...
    try {
      ...
      logger.warn(
        '[finalizeInventorySession] No se pudo obtener/crear almacén por defecto',
        e,
      );
    }
    const synthWrites = [];
      ...
      try {
        ...
        const batch = await createBatch(user, {
          ...
          expirationDate: sc.expirationDate
            ? toLocalNoonTimestamp(sc.expirationDate)
            : null,
        ...
        synthWrites.push((b) =>
          b.set(
            mvRef,
            {
              ...
              destinationLocation:
                defaultWarehouseId || sc.location || 'adjustment',
        ...
        logger.error(
          '[finalizeInventorySession] Error creando lote sintético',
          e,
        );
      }
    }
    if (synthWrites.length) {
      try {
        await commitChunked(synthWrites);
      } catch (e) {
        logger.error('[finalizeInventorySession] Error commit sintéticos:', e);
      }
    }
    ...
    try {
      ...
      for (const [productId, qty] of negBackorderByProduct) {
        ...
        const boRef = db
          .collection(`businesses/${businessID}/backOrders`)
          .doc();
        boWrites.push((b) =>
          b.set(
            boRef,
      ...
      logger.error(
        '[finalizeInventorySession] Error creando backorders por negativos',
        e,
      );
    }
    ...
    if (aggWrites.length) {
      try {
        await commitChunked(aggWrites);
      } catch (e) {
        logger.error(
          '[finalizeInventorySession] Error agregados increment()',
          e,
        );
      }
    }
    ...
    try {
      ...
      logger.error(
        '[finalizeInventorySession] Error actualizando product.stock con clamp a 0',
        e,
      );
    }
    ...
    try {
      ...
      if (recWrites.length) {
        try {
          await commitChunked(recWrites);
        } catch (e) {
          logger.error(
            '[finalizeInventorySession] Error commit reconciliación product.stock',
            e,
          );
        }
      }
      ...
      logger.error(
        '[finalizeInventorySession] Error reconciliando product.stock por suma de productsStock',
        e,
      );
    }
    ...
    try {
      for (const g of groups || []) {
        ...
        for (const child of g?._children || []) {
          ...
          const frozen = isFinite(baselineStock)
            ? Math.max(0, baselineStock)
            : 0;
    ...
    const sessionRef = db.doc(
      `businesses/${businessID}/inventorySessions/${sessionId}`,
    );
    const snapshotPayload = { frozenChildrenStock, frozenProductTotals };
    const approxBytes = Buffer.from(
    ...
    if (approxBytes > 800_000) {
      ...
      await sessionRef.update({
        ...baseUpdate,
        frozenSnapshotPath: frozenRef.path,
      });
    } else {
      await sessionRef.update({ ...baseUpdate, ...snapshotPayload });
    }

    logger.info('[finalizeInventorySession] Finalización completada', {
      businessID,
      sessionId,
      adjustments: adjustments.length,
      expirationUpdates: expirationUpdates.length,
