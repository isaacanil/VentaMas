...
import { assertUserAccess } from '../../invoice/services/repairTasks.service.js';
...
const getReceivableMetadata = (invoice) => {
  ...
  const isAdded =
    ...
    candidates.some((candidate) =>
      Boolean(candidate?.isAddedToReceivables),
    );
...
async function getSetupARTaskMetadata({ businessId, invoiceId }) {
  ...
  try {
    ...
    logger.error(
      '[auditAccountsReceivableHttp] Failed to load setupAR tasks',
      {
        businessId,
        invoiceId,
        error: error?.message,
...
async function collectInvoicesMissingReceivable({
  ...
  const scanLimit = Math.max(sampleLimit * SCAN_MULTIPLIER, sampleLimit);
  const invoicesRef = db.collection(
...
async function collectReceivableInconsistencies({
  ...
  for (const docSnap of arSnap.docs) {
    ...
    issues.push({
      ...
      invoiceBalanceDue:
        invoiceDiff != null ? arBalance - invoiceDiff : null,
...
async function collectPendingBalanceMismatches({
  businessId,
  sampleLimit,
}) {
...
async function collectInvoicesWithAnomalies({
  ...
  for (const docSnap of querySnap.docs) {
    if (issues.length >= sampleLimit) break;
    const invoice = docSnap.data() || {};
    const paymentMethod = invoice.paymentMethod || invoice.paymentProfile?.method;
