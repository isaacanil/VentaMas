...
export const autoRepairInvoiceV2Http = https.onRequest(async (req, res) => {
  ...
  try {
    ...
    if (!runForAllBusinesses && !targetBusinessId) {
      return res
        .status(400)
        .json({ error: 'Debes indicar businessId o habilitar runForAllBusinesses' });
    }

    const userBusinessScope = await getUserBusinessScope(authUid);
    if (runForAllBusinesses && userBusinessScope) {
      return res.status(403).json({
        error:
          'Tu usuario está restringido a un negocio específico. Desactiva runForAllBusinesses.',
      });
    }
    ...
    for (const businessId of businessIds) {
      ...
      let invoiceQuery = buildInvoiceQuery({
        businessId,
        limit: invoicesLimit,
    ...
    return res.status(200).json({
      ...
      businesses,
      nextPage: runForAllBusinesses && nextBusinessCursor
