 ...
 */
export const useBarcodeValidator = (currentProductId = null, debounceMs = 500) => {
  ...
  const validateBarcode = useCallback(async (barcode) => {
    console.log('validateBarcode called with:', { barcode, businessID: user?.businessID, currentProductId })
    
    if (!barcode || !user?.businessID || barcode.trim() === '') {
      console.log('Early return - validation skipped:', { barcode, businessID: user?.businessID })
    ...
    try {
      ...
      console.log('Query results:', { 
        ...
        docs: querySnapshot.docs.map(doc => ({ id: doc.id, barcode: doc.data().barcode, name: doc.data().name }))
      })
      ...
      console.log('After filtering:', { duplicatesCount: duplicates.length, currentProductId })
      ...
      const result = {
        ...
        duplicateInfo: duplicateProduct ? {
          ...
          price: duplicateProduct.pricing?.price || duplicateProduct.price?.unit,
  ...
  useEffect(() => {
    return () => {
      if (debouncedValidateBarcode && typeof debouncedValidateBarcode.cancel === 'function') {
   ...
   */
  const validateMultipleBarcodes = useCallback(async (barcodes) => {
    ...
    try {
      ...
      const validBarcodes = barcodes.filter(code => code && code.trim() !== '')
      ...
      const querySnapshot = await getDocs(q)
      const foundProducts = querySnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }))
  ...
  const getDuplicateStats = useCallback(async () => {
    ...
    try {
      ...
      querySnapshot.docs.forEach(doc => {
        ...
        if (product.barcode && product.barcode.trim() !== '') {
          ...
          if (barcodes[barcode]) {
            ...
            barcodes[barcode] = { count: 1, products: [{ id: doc.id, name: product.name }] }
          }
        }
      })
      ...
      return {
        ...
        duplicateCount,
        duplicateBarcodes,
        duplicatePercentage: totalProducts > 0 ? (duplicateCount / totalProducts * 100).toFixed(2) : 0
      }
