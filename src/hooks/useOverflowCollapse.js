...
export const useOverflowCollapse = ({ 
  ...
  useLayoutEffect(() => {
    ...
    const calculateVisibleItems = () => {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        const requiredWidth = accumulatedWidth + widthWithGap - (isLastItem ? moreButtonWidth + gap : 0);
  ...
  return { 
    ...
    register, 
    visibleCount,
    hasOverflow: visibleCount !== Infinity && visibleCount < itemRefs.current.filter(Boolean).length
  };
}; 