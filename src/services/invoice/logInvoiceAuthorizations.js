...
const buildInvoiceTarget = ({ invoice, cart, discountAuth }) => {
  const invoiceNumber = invoice?.numberID || invoice?.numberId || invoice?.invoiceNumber;
  ...
  return {
    ...
    details: {
      ...
      cartId: discountAuth?.metadata?.cartId || cart?.id || cart?.cartId || null,
      clientId:
        discountAuth?.metadata?.clientId || cart?.client?.id || cart?.clientId || cart?.client?.uid || null,
      clientName: discountAuth?.metadata?.clientName || cart?.client?.name || "",
    },
  };
};

export const logInvoiceAuthorizations = async ({ user, invoice, authorizationContext, cart }) => {
  ...
  try {
    const requestedBy =
      discountAuth?.requestedBy || sanitizeUserSnapshot(cart?.seller || cart?.user || cart?.cashier || user);
    ...
    await fbRecordAuthorizationApproval({
      ...
      metadata: {
        ...
        invoiceNumber: invoice?.numberID || invoice?.numberId || invoice?.invoiceNumber || null,
        total:
          extractAmount(invoice?.totalPurchase) ?? extractAmount(discountAuth?.metadata?.total) ?? null,
        discountPercent:
          extractAmount(invoice?.discount) ?? extractAmount(discountAuth?.metadata?.discountPercent) ?? null,
      },
    });
  } catch (error) {
