...
export async function processInvoice({
  ...
  if (insuranceEnabled && insuranceAR?.totalInstallments) {
    ...
    const authDataId = await addInsuranceAuth(
      user,
      insuranceAuth,
      clientData.clientId,
    );

    await manageInsuranceReceivableAccounts({
      user,
      arData,
      invoice,
      insuranceAuth,
...
async function validateCashReconciliation({ user, dispatch, transaction }) {
  try {
    const { state, cashCount } = await checkOpenCashReconciliation(
      user,
      transaction,
    );
...
async function handleTaxReceiptGeneration({
  user,
  taxReceiptEnabled,
  ncfType,
}) {
  ...
  try {
    ...
    console.error(
      `Error processing tax receipt for type ${ncfType}:`,
      error.message,
    );
...
async function retrieveAndUpdateClientData({ user, client }) {
  ...
  try {
    ...
    throw new Error(
      `Error al actualizar los datos del cliente: ${error.message}`,
    );
  }
}
...
async function generateFinalInvoice({
  ...
  try {
    const cartWithDueDate = dueDate
      ? checkIfHasDueDate({ cart, dueDate })
      : cart;
    const { authorizationContext = null, ...cartPayload } =
      cartWithDueDate || {};
...
async function manageInsuranceReceivableAccounts({
  user,
  arData,
  invoice,
  insuranceAuth,
  ...
  try {
    ...
    const { insuranceName } = await getInsurance(
      user,
    ...
    throw new Error(
      `Error al gestionar cuentas por cobrar de seguro: ${error.message}`,
    );
  }
}
...
async function logInvoiceAuthorizations({
  user,
  invoice,
  authorizationContext,
  cart,
  ...
  try {
    const requestedBy =
      discountAuth.requestedBy || sanitizeUserSnapshot(cart?.seller || user);
    const targetUser = discountAuth.targetUser || requestedBy;

    const description =
      discountAuth.description ||
      `Autorización aplicada en factura ${invoice?.numberID || ''}`.trim();
    ...
    await fbRecordAuthorizationApproval({
      ...
      target: {
        type: 'invoice',
        id: invoice?.id || '',
        name: invoice?.numberID
          ? `Factura ${invoice.numberID}`
          : invoice?.id || '',
        details: {
...
async function processTestModeInvoice({
  ...
  try {
    ...
    if (cart?.creditNotePayment?.length > 0) {
      console.log(
        `🧪 [MODO PRUEBA] Simulando consumo de ${cart.creditNotePayment.length} notas de crédito`,
      );
      cart.creditNotePayment.forEach((note) => {
        console.log(`🧪 [MODO PRUEBA] Nota ${note.ncf}: -${note.amountUsed}`);
      });
      console.log(
        `🧪 [MODO PRUEBA] Simulando creación de registros de aplicación para factura ${mockInvoice.id}`,
      );
    }
