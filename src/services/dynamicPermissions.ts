...
export const setUserDynamicPermissions = async (
  ...
  if (!currentUser?.businessID) {
    throw new Error('currentUser.businessID es requerido para guardar permisos dinámicos');
  }
...
): Promise<{ specialCashier1: string[]; specialCashier2: string[]; errors: string[] }> => {
  try {
    ...
    return { errors: [error instanceof Error ? error.message : String(error)], specialCashier1: [], specialCashier2: [] };
  }
};
