...
import {
  ...
  getAvailableHeaders,
} from './filterEssentialHeaders';
import { productHeaderAliases } from './headerAliases';
...
export const getProductAvailableHeaders = (
  language: ProductLanguage = 'es',
) => getAvailableHeaders(productHeaderMappings, language);
...
export const importProductData = async (
  ...
  try {
    ...
    const dataMapped = mapData({
      data,
      headerMapping,
      headerAliases: productHeaderAliases,
      language,
    });
