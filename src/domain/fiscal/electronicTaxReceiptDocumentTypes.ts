...
const ELECTRONIC_DOCUMENT_TYPES = new Set(['E31', 'E32', 'E33', 'E34', 'E45']);
...
const normalizeDocumentTypeText = (value: unknown): string =>
  typeof value === 'string' ? stripDiacritics(value.trim()).toUpperCase() : '';

const resolveLegacySerie = (value: string): string | null => {
  const compact = value.replace(/[^A-Z0-9]/g, '');
  const prefixedSerie = compact.match(/^B(01|02|03|04|15)/)?.[1];
  if (prefixedSerie) return prefixedSerie;

  const standaloneSerie = compact.match(/^(01|02|03|04|15)$/)?.[1];
...
export const resolveElectronicTaxReceiptDocumentType = (
  ...
  const normalized = normalizeDocumentTypeText(fiscalReceiptName);
...
export const formatElectronicTaxReceiptLabel = (
  ...
  const name =
    typeof fiscalReceiptName === 'string' ? fiscalReceiptName.trim() : '';
