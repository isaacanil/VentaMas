  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!userWithBusiness) {
      dispatch(
        addNotification({
          title: 'Permisos',
          message: 'No se pudo determinar el negocio activo.',
          type: 'error',
        }),
      );
      return;
    ...
    if (categoryToUpdate) {
      fbUpdateCategory(category, userWithBusiness)
        .then(() => {
          onClose();
        })
        .then(() => {
      ...
      fbAddCategory(userWithBusiness, category)
        .then(() => {
          onClose();
        })
        .then(() => {
  ...
  return (
    <LazyMotion features={domAnimation}>
      ...
      >
        ...
        >
          <h2>
            {categoryToUpdate ? 'Actualizar Categoría' : 'Crear Categoría'}
          </h2>
...
const ModalOverlay = styled(m.div)<{ isOpen: boolean }>`
  position: fixed;
  top: 0;
  left: 0;
  z-index: 1000000;
