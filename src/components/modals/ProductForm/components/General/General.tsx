...
import type { ProductBrand, ProductRecord } from '@/types/products';
...
export const General = ({ showImageManager }: GeneralProps) => {
  ...
  const { data: productBrands = [] } = useListenProductBrands() as {
    data: ProductBrand[];
  };
    ...
    if (key === 'pricing') {
      ...
      if (normalizedPricing?.tax !== undefined) {
        ...
        normalizedPricing.tax =
          typeof t === 'string' ? parseFloat(t) || initTaxes[0] : Number(t);
      }
            ...
            <Space direction="vertical">
              <Card title="Imagen del producto" size="small">
                ...
                >
                  <ImageContent>
                    {product?.image && (
                      <AntdImage height={150} src={product?.image} />
                    )}
                    {!product?.image && (
                      <ImageContainer>
                        <Image src={imgFailed} />
