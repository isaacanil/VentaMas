...
export const BarcodeGenerator = ({
  ...
  useEffect(() => {
    if (visible) {
      ...
      setCurrentDisplayBarcode(currentBarcode ?? null); // Usar siempre el código del estado local/Redux
    }
  ...
  useEffect(() => {
    if (visible) {
      ...
      setLastKnownBarcode(currentBarcode ?? null);
      setLastDatabaseBarcode(currentBarcode ?? null);
      setCurrentDisplayBarcode(currentBarcode ?? null);
  ...
  const createLivePreview = useCallback(
    (companyPrefix: string, itemReference: string) => {
      ...
      try {
        ...
        const prefixes: Record<Gs1StandardKey, string> = {
          gs1rd: '746',
          gs1us: '0',
          gs1mx: '750',
          gs1co: '770',
  ...
  const getGeneratorFunction = (standard: Gs1StandardKey): GeneratorFn => {
  ...
  const getInternalGeneratorFunction = (
