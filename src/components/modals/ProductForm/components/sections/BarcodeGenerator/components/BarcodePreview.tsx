...
const EMPTY_REALTIME_STATUS: RealtimeStatus = {};
...
export const BarcodePreview = ({
  ...
  livePreview,
  currentBarcode,
  realtimeStatus = EMPTY_REALTIME_STATUS,
  ...
  const structure = barcodeAnalysis?.structure ?? null;
  const companyPrefix =
    structure && 'companyPrefix' in structure
      ? structure.companyPrefix
      : null;
  const itemReference =
    structure && 'itemReference' in structure
      ? structure.itemReference
      : null;
  const checkDigit =
    structure && 'checkDigit' in structure ? structure.checkDigit : null;
  ...
  return (
    <div>
      ...
      {!showPreview && !currentBarcode && (
        <Form.Item label="Vista Previa" style={{ marginTop: '24px' }}>
          <PreviewContainer>
