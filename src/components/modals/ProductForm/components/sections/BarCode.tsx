...
export const BarCode = ({ product }: BarCodeProps) => {
  ...
  const barcodeInfo = (barcodeValue
    ? getBarcodeInfo(barcodeValue)
    : null) as ReturnType<typeof getBarcodeInfo> | null;

  // Calcular dimensiones GS1 precisas para preview
  const getBarcodeRenderProps = useMemo<BarcodeRenderProps>(() => {
  ...
  const getValidationInfo = (): BarcodeValidationInfo | null => {
  ...
  const handleCorrectedCode = (code: string) => {
