...
import { Button, Image, Modal, Progress, Typography, Upload, message } from 'antd';
...
const ImageManager = ({ open, onCancel }: ImageManagerProps) => {
  ...
  const uploadProps: UploadProps = {
    ...
    customRequest: async (options: UploadRequestOption) => {
      ...
      let newFile: UploadFile | null = null;
      try {
        ...
        const url = await fbAddProductImg(user, typedFile, updateProgress);
        ...
        if (newFile) {
          setFileList([{ ...newFile, status: 'error' }]);
        } else {
          setFileList([]);
        }
      } finally {
