...
export const StyledTag = styled.span<{ color?: 'error' | 'info' }>`
  ...
  color: ${(props: { color?: 'error' | 'info' }) => (props.color === 'error' ? '#ff4d4f' : '#1890ff')};
  border: 1px solid
    ${(props: { color?: 'error' | 'info' }) => (props.color === 'error' ? '#ffa39e' : '#91d5ff')};
  margin-left: 8px;
  white-space: nowrap;
`;
...
export const KPICard = styled.div<{
  ...
  padding: 12px;
  background: ${(props: { background?: string }) => props.background || '#fafafa'};
  border-radius: 12px;
  border: 2px solid ${(props: { borderColor?: string }) => props.borderColor || '#e8e8e8'};
  text-align: center;
  ...
  .kpi-amount {
    font-size: ${(props: { isPrimary?: boolean }) => (props.isPrimary ? '24px' : '20px')};
    font-weight: 700;
  ...
  .kpi-status {
    ...
    padding: 2px 8px;
    border-radius: 12px;
    background: ${(props: { statusBg?: string }) => props.statusBg || 'transparent'};
...
export const HighlightCard = styled.div<{
  ...
  border: 1px solid ${(props: { $border?: string }) => props.$border || '#f0f0f0'};
  border-radius: 10px;
  padding: 16px;
  display: flex;
  align-items: flex-start;
