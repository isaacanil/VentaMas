import { FileAddOutlined, UploadOutlined } from '@/constants/icons/antd';
...
type ImportLanguage = 'es' | 'en';

type ImportModalProps = {
  open: boolean;
  onClose: () => void;
  onImport: (file: File, options?: ImportOptions) => Promise<unknown>;
  onCreateTemplate: (language: ImportLanguage, fields: string[]) => Promise<void>;
};
...
export default function ImportModal({
  ...
  const [language] = useState<ImportLanguage>('es'); // Por defecto español, podría ser configurable en el futuro
  ...
  const handleImportClick = async () => {
    if (fileList.length > 0 && fileList[0].originFileObj) {
      setIsImporting(true);
      try {
        await onImport(fileList[0].originFileObj as File);
        setFileList([]);
        setPreviewData([]);
        onClose();
  ...
  const handlePreviewClick = async () => {
    ...
    try {
      const data = await onImport(fileList[0].originFileObj as File, {
        dryRun: true,
      });
  ...
  return (
    ...
    >
      <Body>
        <Tabs
          activeKey={activeTab}
          onChange={(key) => setActiveTab(key as 'import' | 'template')}
          items={items}
        />
      </Body>
    </StyledModal>
