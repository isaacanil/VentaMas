...
export const Client = ({ invoice, isEditLocked = false }: ClientProps) => {
  ...
  const normalizedClients = useMemo<ClientRecord[]>(
    () => (Array.isArray(clients) ? (clients as ClientRecord[]) : []),
    [clients],
  );

  const clientDictionary = useMemo<Record<string, InvoiceClient>>(() => {
    return normalizedClients.reduce<Record<string, InvoiceClient>>(
      (acc, entry) => {
...
const DetailItem = styled.div<DetailItemStyleProps>`
  display: flex;
...
const ChangeClientButton = styled.button<ButtonStyleProps>`
  display: flex;
  gap: 6px;
  align-items: center;
  padding: 6px 12px;
