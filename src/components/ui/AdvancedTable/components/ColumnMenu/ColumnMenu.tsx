              ...
              .map((column, index) => (
                <MenuItem
                  ...
                  animate={{
                    opacity: 1,
                    y: 0,
                    backgroundColor: highlightedItems.includes(column.accessor)
                      ? 'var(--color2)'
                      : 'white',
                  }}
                  exit={{ opacity: 0, y: 10 }}
        ...
        <MenuItems>
          {getDeletedColumns().map((column) => (
            <MenuItem key={column.accessor}>
              ...
              <Button onClick={() => restoreColumn(column.accessor)}>
                Restaurar
              </Button>
            </MenuItem>
          ))}
  ...
  return (
    ...
    >
      ...
      <Typography.Paragraph>
        Restablece el orden original de las columnas y reactiva las columnas
        eliminadas
      </Typography.Paragraph>
      <Footer>
        <Button title={''} onClick={resetColumnOrder}>
          Restaurar
        </Button>
      </Footer>
    </Modal>
...
`;
const MenuItem = styled(m.li)<{ reorderable?: boolean }>`
  display: flex;
  align-items: center;
