...
const SplashMessage = styled.p<{ theme?: any }>`
  ...
  font-size: 1rem;
  line-height: 1.5;
  color: ${(props: { theme?: any }) => getThemeStyles(props.theme?.mode || 'dark').textColor};
  text-align: center;
`;
...
const MinimalOverlay = styled.div<{ $active?: boolean }>`
  ...
  padding: 24px;
  pointer-events: ${(props: { $active?: any }) => (props.$active ? 'auto' : 'none')};
...
const VentamaxSystemLoader = ({ active, message, error, status }: LoaderProps) => {
  const handleRetry = () => {
...
const VentamaxMinimalLoader = ({ active, message, onFinish, status }: LoaderProps) => {
...
const VentamaxSplashLoader = ({ active, message, onFinish, status }: LoaderProps) => {
