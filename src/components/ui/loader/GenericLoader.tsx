...
interface LoaderProps {
  ...
  variant?: 'minimal' | 'system' | 'session' | 'splash';
  onFinish?: () => void;
  theme?: 'light' | 'dark';
}

const getThemeStyles = (theme: 'light' | 'dark' | string) => {
  const themes: Record<string, { textColor: string }> = {
    dark: { textColor: '#fff' },
    light: { textColor: '#000' },
  };
...
const SplashMessage = styled.p<{ theme?: any }>`
  max-width: 320px;
  margin: 1.5rem 0 0;
  font-size: 1rem;
  line-height: 1.5;
  color: ${(props: { theme?: any }) =>
    getThemeStyles(props.theme?.mode || 'dark').textColor};
  text-align: center;
`;

const MinimalOverlay = styled.div<{ $active?: boolean }>`
  position: fixed;
  inset: 0;
  z-index: 1000;
  display: flex;
  ...
  padding: 24px;
  pointer-events: ${(props: { $active?: any }) =>
...
const SystemOverlay = styled.div<{ $active?: boolean }>`
  position: fixed;
  inset: 0;
  z-index: 10000;
  display: flex;
...
const SystemLogo = styled.img<{ $active?: boolean }>`
  width: 120px;
  margin-bottom: 40px;
...
const SystemSpinner = styled.div<{ $active?: boolean }>`
  width: 50px;
  height: 50px;
  ...
  return (
    <SystemOverlay $active={active}>
      {isError ? (
        ...
        <>
          <SystemLogo $active={active} src={logo} alt="Ventamax Logo" />
          <SystemSpinner $active={active} />
          <SystemLoadingText>
...
const VentamaxSplashLoader = ({
  ...
  const fadeTimelineRef = useRef<gsap.core.Tween | null>(null);
...
export default function VentamaxLoader(props: LoaderProps) {
  if (props.variant === 'minimal') {
    return (
      <VentamaxMinimalLoader {...props} active={props.active ?? props.show} />
    );
  }

  if (props.variant === 'system' || props.variant === 'session') {
    return (
      <VentamaxSystemLoader {...props} active={props.active ?? props.show} />
    );
  }

  return (
    <VentamaxSplashLoader {...props} active={props.active ?? props.show} />
  );
}
