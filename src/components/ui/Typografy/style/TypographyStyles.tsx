...
const baseTypography = css<TypographyStyleProps>`
  ...
  text-align: ${({ $align }: TypographyStyleProps) =>
    $align ? $align : 'left'};

  /* margin-bottom: ${({ $gutterBottom }: TypographyStyleProps) =>
    $gutterBottom ? '1rem' : '0'}; */
  ${({ $disableMargins }: TypographyStyleProps) =>
    $disableMargins && 'margin: 0;'}

  /* font-weight: ${({ $bold }: TypographyStyleProps) =>
    boldScale[String($bold)] || 'normal'}; */
  ${({ $bold }: TypographyStyleProps) =>
    $bold && `font-weight: ${boldScale[String($bold)]} ;`}
  ${({ $italic }: TypographyStyleProps) => $italic && 'font-style: italic;'}
  ${({ $underline }: TypographyStyleProps) =>
    $underline && 'text-decoration: underline;'}
  ${({ $uppercase }: TypographyStyleProps) =>
    $uppercase && 'text-transform: uppercase;'}
  ${({ $capitalize }: TypographyStyleProps) =>
    $capitalize && 'text-transform: capitalize;'}
  ${({ $lowercase }: TypographyStyleProps) =>
    $lowercase && 'text-transform: lowercase;'}
  ${({ $noWrap }: TypographyStyleProps) => $noWrap && 'white-space: nowrap;'}

  letter-spacing: ${({ $letterSpacing }: TypographyStyleProps) =>
    $letterSpacing || 'normal'};
  text-transform: ${({ $textTransform }: TypographyStyleProps) =>
    $textTransform || 'none'};
...
export const TypographyStyle = styled.div<TypographyStyleProps>`
  ...
  ${baseTypography}
  ${({ $color }: TypographyStyleProps) =>
    colors[$color ?? 'dark'] || colors.dark}
      
  ${({ $strikethrough }: TypographyStyleProps) =>
    $strikethrough && 'text-decoration: line-through;'}
  ${({ $textShadow }: TypographyStyleProps) =>
