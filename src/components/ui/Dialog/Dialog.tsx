...
const ConfirmButton = styled(BaseButton)<{ $type: DialogType }>`
  ...
  background: ${(props: { $type: DialogType }) =>
    dialogTheme[props.$type]?.button || '#3b82f6'};
  border: none;
  box-shadow: 0 2px 8px
    ${(props: { $type: DialogType }) => dialogTheme[props.$type]?.border}40;

  &:hover:not(:disabled) {
    background: ${(props: { $type: DialogType }) =>
      dialogTheme[props.$type]?.buttonHover || '#2563eb'};
    box-shadow: 0 4px 15px
      ${(props: { $type: DialogType }) => dialogTheme[props.$type]?.border}60;
    transform: translateY(-2px);
  }

  &:disabled {
    background: ${(props: { $type: DialogType }) =>
      dialogTheme[props.$type]?.button}90;
  }
`;
...
const CloseButton = styled(BaseButton)<{ $type: DialogType }>`
  ...
  border: 2px solid
    ${({ $type }: { $type: DialogType }) => dialogTheme[$type]?.border}40;
  border-radius: 50%;

  &:hover:not(:disabled) {
    background: ${({ $type }: { $type: DialogType }) =>
      dialogTheme[$type]?.border}20;
...
const Container = styled(m.div)<{ $size: DialogSize; $type: DialogType }>`
  ...
  border: 2px solid
    ${({ $type }: { $type: DialogType }) =>
      dialogTheme[$type]?.border || 'rgba(0,0,0,0.1)'};
...
const Header = styled.div<{ $type: DialogType }>`
  ...
  border-bottom: 1px solid
    ${({ $type }: { $type: DialogType }) => dialogTheme[$type]?.border}30;
`;
...
const IconWrapper = styled.div<{ $type: DialogType }>`
  ...
  width: 2em;
  height: 2em;
  color: ${({ $type }: { $type: DialogType }) =>
    dialogTheme[$type]?.text || 'inherit'};
`;
