  ...
  reducers: {
    addItem: (state: CategoryState, action: PayloadAction<CategoryItem>) => {
      ...
      const list = state.items.filter(
        (item: CategoryItem) => item.type === type,
      ); // Filtramos según el tipo

      const checkingNameIsDifferent = list.every(
        (item: CategoryItem) => item.id !== id,
      );
...
export const SelectCategoryList = (state: CategoryRootState) =>
  state.category.items;
export const SelectCategoryState = (state: CategoryRootState) => state.category;
export const SelectCategoryStatus = (state: CategoryRootState) =>
...
export const SelectActiveIngredients = createSelector(
  SelectCategoryList,
  (items: CategoryItem[]) =>
    items.filter((item: CategoryItem) => item.type === 'activeIngredient'),
);
