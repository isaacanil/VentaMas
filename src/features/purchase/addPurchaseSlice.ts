...
import { normalizePurchaseReplenishment } from '@/utils/purchase/workflow';
...
const normalizeSelectedProduct = (product: ProductSelected): ProductSelected => {
  ...
  return {
    ...
    receivedQuantity: Number(normalized.receivedQuantity) || 0,
    pendingQuantity: Number(normalized.pendingQuantity) || 0,
  };
};
  ...
  reducers: {
    ...
    SelectProduct: (state: AddPurchaseState, actions: PayloadAction<any>) => {
      ...
      let productData = {
        ...
        taxPercentage: product.pricing?.tax ?? product.taxPercentage ?? 0, // Prefill ITBIS from product pricing when available
      ...
      state.productSelected = normalizeSelectedProduct(productData);
    },
    ...
    updateProduct: (state: AddPurchaseState, actions: PayloadAction<any>) => {
      ...
      const updatedProduct = {
        ...
        orderedQuantity:
          value.orderedQuantity !== undefined
            ...
            : purchaseQuantity,
        receivedQuantity:
          value.receivedQuantity !== undefined ? value.receivedQuantity : 0,
        pendingQuantity:
          value.pendingQuantity !== undefined
      ...
      state.purchase.replenishments[productIndex] = normalizeSelectedProduct({
        ...updatedProduct,
        unitCost,
        subtotal,
      });
    ...
    setSelectedBackOrders: (
      ...
      state.productSelected = normalizeSelectedProduct({
    ...
    clearSelectedBackOrders: (state: AddPurchaseState) => {
      state.productSelected = normalizeSelectedProduct({
        ...
        orderedQuantity: state.productSelected.quantity,
        receivedQuantity: 0,
        pendingQuantity: state.productSelected.quantity,
...
const calculateUnitCost = (product: Partial<ProductSelected>): number => {
  ...
  const quantity =
    Number(product.quantity) || Number(product.purchaseQuantity) || 0;
...
export const selectAddPurchaseList = (state: AddPurchaseRootState) =>
  state.addPurchase.pendingOrders;
export const selectProducts = (state: AddPurchaseRootState) =>
  state.addPurchase.products;
export const selectPurchase = (state: AddPurchaseRootState) =>
  state.addPurchase.purchase;
export const selectPurchaseState = (state: AddPurchaseRootState) =>
  state.addPurchase;
