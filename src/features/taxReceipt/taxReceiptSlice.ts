...
export function getUpdatedSequenceForInvoice(comprobanteName: string, comprobantes: TaxReceiptData[]) {
...
export const taxReceiptSlice = createSlice({
  ...
  reducers: {
    getTaxReceiptData: (state: TaxReceiptState, action: PayloadAction<any>) => {
      ...
      state.availableTypes = serializedPayload.map((item: TaxReceiptData) => item.data.name);
    },
    IncreaseEndConsumer: (state: TaxReceiptState, action: PayloadAction<string | undefined>) => {
    ...
    IncreaseTaxCredit: (state: TaxReceiptState, action: PayloadAction<string | undefined>) => {
      if (state.settings.taxReceiptEnabled) {
        // Si se proporciona un nombre específico del comprobante, usar ese
    ...
    IncreaseSpecificReceipt: (state: TaxReceiptState, action: PayloadAction<string | undefined>) => {
      if (state.settings.taxReceiptEnabled && action.payload) {
        updateComprobante(state, action.payload);
      }
    },
    toggleTaxReceiptSettings: (state: TaxReceiptState, action: PayloadAction<boolean>) => {
    ...
    setTaxReceiptSettingsLoaded: (state: TaxReceiptState, action: PayloadAction<boolean>) => {
      state.settings.settingsLoaded = !!action.payload;
    },
    updateTaxCreditInFirebase: (state: TaxReceiptState) => {
      const taxReceipt = state.data;
      fbUpdateTaxReceipt(taxReceipt as any);
    },
    selectTaxReceiptType: (state: TaxReceiptState, actions: PayloadAction<string>) => {
...
export const selectTaxReceiptData = (state: TaxReceiptRootState) => state.taxReceipt.data;
export const selectNcfType = (state: TaxReceiptRootState) => state.taxReceipt.ncfType;
export const selectNcfCode = (state: TaxReceiptRootState) => state.taxReceipt.ncfCode;
...
export const selectTaxReceipt = (state: TaxReceiptRootState) => state.taxReceipt;
export const selectAvailableReceiptTypes = (state: TaxReceiptRootState) =>
  state.taxReceipt.availableTypes;
export const selectNcfTypeLocked = (state: TaxReceiptRootState) => state.taxReceipt.ncfTypeLocked;
