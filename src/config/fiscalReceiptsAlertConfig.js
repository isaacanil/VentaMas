...
export const FISCAL_RECEIPTS_ALERT_CONFIG = {
  ...
  MESSAGES: {
    CRITICAL: {
      template: '{name} - Serie {series}: ¡CRÍTICO! Solo quedan {remaining} de {total} comprobantes. Solicita más inmediatamente.',
      action: 'Solicitar urgente'
    },
    WARNING: {
      template: '{name} - Serie {series}: Quedan {remaining} de {total} comprobantes. Considera solicitar más pronto.',
      action: 'Solicitar más'
    },
    INFO: {
      template: 'Los comprobantes fiscales están deshabilitados o no configurados.',
      action: 'Configurar'
    },
    SUCCESS: {
      template: 'Todos los comprobantes están en buen estado. El de menor cantidad es {name} con {remaining} restantes.',
      action: 'Ver detalles'
    }
  },
...
export const getThresholdsForReceiptType = (receiptType) => {
  const customConfig = FISCAL_RECEIPTS_ALERT_CONFIG.CUSTOM_THRESHOLDS[receiptType];
...
export const getAlertMessage = (alertType, receiptData = {}) => {
  const messageConfig = FISCAL_RECEIPTS_ALERT_CONFIG.MESSAGES[alertType.toUpperCase()];
