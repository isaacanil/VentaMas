...
export const CashReconciliationTable = () => {
  ...
  const [tableState, dispatchTableState] = useReducer(
    (
      currentState: { cashCounts: CashCountRecord[]; loading: boolean },
      action:
        | { type: 'start' }
        | { type: 'success'; payload: CashCountRecord[] }
        | { type: 'reset' },
    ) => {
      switch (action.type) {
        case 'start':
        ...
        case 'success':
          return { cashCounts: action.payload, loading: false };
        case 'reset':
          return { cashCounts: [], loading: false };
        default:
          return currentState;
      }
    },
    { cashCounts: [], loading: true },
  );
  const { cashCounts, loading } = tableState;
  ...
  useEffect(() => {
    dispatchTableState({ type: 'start' });
  }, [subscriptionKey]);

  // Suscripción a Firebase listener
  useEffect(() => {
    const handleLoadComplete = (counts: CashCountRecord[]) => {
      dispatchTableState({ type: 'success', payload: counts });
    };
