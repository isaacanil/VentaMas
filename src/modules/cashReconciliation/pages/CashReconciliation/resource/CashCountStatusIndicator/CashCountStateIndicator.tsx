...
export const CashCountStateIndicator: React.FC<
  CashCountStateIndicatorProps
> = ({ state }) => {
...
const Container = styled.div<{ $config: StatusConfig }>`
  ...
  font-size: 13px;
  font-weight: 600;
  background-color: ${(props: { $config: StatusConfig }) =>
    props.$config.bgColor}cc;
  color: ${(props: { $config: StatusConfig }) => props.$config.color};
  border: 1px solid
    ${(props: { $config: StatusConfig }) => props.$config.color}30;
