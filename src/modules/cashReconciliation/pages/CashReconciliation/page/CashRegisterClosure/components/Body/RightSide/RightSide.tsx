...
export const RightSide: React.FC<RightSideProps> = ({
  ...
  const { data: expenses, loading: expensesLoading } = useExpensesForCashCount(
    id as string,
  );
  ...
  const metaData = useMemo(
    ...
    [cashReconciliation, invoices, expenses, arPayments, cashMovements],
  );

  const prevMetaRef = useRef<string | null>(null);

  useEffect(() => {
    if (!metaData) return;
    const key = JSON.stringify(metaData);
    if (prevMetaRef.current === key) return;
