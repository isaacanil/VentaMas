...
const ModulesNavigator = ({
  ...
  fiscalReceiptsData: any;
}) => {
  const user = useSelector(selectUser);
  const [activeModule, setActiveModule] = useState('authorizations');

  const canViewAccountsReceivablePanel = hasManageAllAccess(user);
  ...
  const getModulesConfig = () => {
    ...
    // Agregar Cuentas por Cobrar solo para usuarios privilegiados
    if (canViewAccountsReceivablePanel) {
      modules.push({
...
const ModuleTab = styled.button<{ $active?: boolean }>`
  ...
  background: ${({ $active }: { $active?: boolean }) =>
    $active ? 'linear-gradient(135deg, #1890ff 0%, #40a9ff 100%)' : '#ffffff'};
  border: 1px solid
    ${({ $active }: { $active?: boolean }) => ($active ? '#1890ff' : '#e5e7eb')};
  border-radius: 12px;
