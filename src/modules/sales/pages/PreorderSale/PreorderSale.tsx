...
import type { PreorderClientOption, PreorderFirestoreDoc } from './types';
...
export const Preorder = () => {
  ...
  const [preorders, setPreorders] = useState<PreorderFirestoreDoc[] | null>(
    null,
  );
  ...
  const effectivePreorders = useMemo(() => preorders ?? [], [preorders]);
  const isLoading = Boolean(user?.businessID) && preorders === null;

  const clientOptions = useMemo<PreorderClientOption[]>(() => {
    const uniqueClients = new Map<string, PreorderClientOption>();
    effectivePreorders.forEach((preorder) => {
  ...
  }, [effectivePreorders, getClientIdentifier]);
  ...
  const filteredPreventas = useMemo(() => {
    ...
    return effectivePreorders.filter((preorder) => {
