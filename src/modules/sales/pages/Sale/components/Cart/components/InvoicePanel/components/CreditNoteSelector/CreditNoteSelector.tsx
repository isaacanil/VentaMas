  ...
  return (
    <StyledCard>
      <CardHeader>
        ...
        <HeaderRight>
          <Tag color="blue">
            Total: {formatPriceByCurrency(totalAvailable, documentCurrency)}
          </Tag>
          {totalSelected > 0 && (
            <Tag color="green">
              Seleccionado:{' '}
              ...
              <ItemContent>
                    ...
                    <Space>
                      ...
                      {selectedAmount > 0 && (
                        <Button
                          ...
                          danger
                          onClick={() =>
                            handleQuickSelect(creditNote.id as string, 0)
                          }
                        >
                          Limpiar
                        </Button>
                    ...
                    {maxUsable < (creditNote.availableAmount as number) && (
                      <Tooltip title="El monto máximo está limitado por el total de la compra">
                        <Text type="secondary" style={{ fontSize: '11px' }}>
                          ...
                          {formatPriceByCurrency(maxUsable, documentCurrency)}
                        </Text>
                      </Tooltip>
                    )}
                  </ItemActions>
                ...
                {disabled && selectedAmount > 0 && (
                  <ItemActions>
                    <Tag color="green">
                      Aplicando:{' '}
                      {formatPriceByCurrency(selectedAmount, documentCurrency)}
                    </Tag>
                  </ItemActions>
                )}
              </ItemContent>
...
const CreditNoteItem = styled(List.Item)<{ $isSelected?: boolean }>`
  ...
  border: 1px solid
    ${({ $isSelected }) => ($isSelected ? '#52c41a' : '#f0f0f0')} !important;
  border-radius: 6px;
