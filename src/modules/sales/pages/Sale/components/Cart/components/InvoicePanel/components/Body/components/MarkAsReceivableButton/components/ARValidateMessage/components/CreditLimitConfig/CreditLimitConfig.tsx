import { Modal, Form, InputNumber, Switch, Button, Typography } from 'antd';
import { useState } from 'react';
import type { CreditLimitConfig } from '@/utils/accountsReceivable/types';

const { Text } = Typography;

type CreditLimitFormValues = {
  creditLimitEnabled?: boolean;
  creditLimitValue?: number;
  invoiceLimitEnabled?: boolean;
  invoiceLimitValue?: number;
};

type CreditLimitModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSave?: (values: CreditLimitFormValues) => void | Promise<void>;
  currentCreditLimit?: CreditLimitConfig | null;
};
...
const CreditLimitModal = ({
  ...
  const handleSave = async () => {
    try {
      setLoading(true);
      const values = (await form.validateFields()) as CreditLimitFormValues;
      if (onSave) {
        await onSave(values);
      }
      form.resetFields();
  ...
  return (
    ...
    >
      ...
      >
        <Form.Item name="creditLimitEnabled" valuePropName="checked">
        ...
        <Form.Item name="invoiceLimitEnabled" valuePropName="checked">
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Switch />
        ...
        >
          <Text type="secondary" style={{ fontSize: '12px' }}>
            <strong>Nota:</strong> Los límites se aplicarán inmediatamente
