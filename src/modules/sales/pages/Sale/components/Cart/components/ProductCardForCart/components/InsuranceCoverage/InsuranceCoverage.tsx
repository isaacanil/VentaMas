...
export const InsuranceCoverage = ({ item }: InsuranceCoverageProps) => {
  ...
  const insuranceState =
    localTrigger === derivedKey ? localValue : derivedState;
...
const CoveragePill = styled(m.div)`
  display: inline-flex;
  gap: 10px;
  align-items: center;
  padding: 6px 10px;
...
const ToggleThumb = styled(m.div)`
  position: absolute;
  left: 2px;
  z-index: 1;
  width: 24px;
...
const ValueInput = styled(m.input)`
  flex: 1;
  width: 100%;
  min-width: 76px;
  height: 32px;
