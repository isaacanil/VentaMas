...
import usePaymentDates from '@/modules/sales/pages/Sale/components/Cart/components/InvoicePanel/components/Body/components/ReceivableManagementPanel/usePaymentDates';
...
type InsuranceManagementPanelProps = {
  form?: FormInstance;
};

export const InsuranceManagementPanel = ({
  form,
  ...
  const insuranceAR = useSelector(selectInsuranceAR) as InsuranceARState;
  ...
  const getPositive = (value: number) => (value < 0 ? value * -1 : value);
  ...
  const setFrequency = useCallback(
    (value: string) => dispatch(setInsuranceAR({ paymentFrequency: value })),
    [dispatch],
  );
  ...
  const setTotalReceivable = useCallback(
    (value: number) => dispatch(setInsuranceAR({ totalReceivable: value })),
    [dispatch],
  );
  const setPaymentDate = useCallback(
    (value: number | null) => dispatch(setInsuranceAR({ paymentDate: value })),
    [dispatch],
  );
  const setComments = useCallback(
    (value: string) => dispatch(setInsuranceAR({ comments: value })),
    [dispatch],
  );
  ...
  useEffect(() => {
    if (form) {
      const formValues = {
        ...
        insurancePaymentDate: toDateTime(paymentDate),
      };
        ...
        >
          ...
          <Form layout="vertical" form={form}>
            ...
            <Group>
              ...
              >
                ...
                >
                  <span>
                    {formatPriceByCurrency(installmentAmount, documentCurrency)}
                  </span>
                </div>
              </FormItem>
            </Group>
          ...
          <Footer>
            <Header>
              ...
              <Label>
                {formatPriceByCurrency(insuranceCoverage, documentCurrency)}
              </Label>
            </Header>
          </Footer>
        </PanelContainer>
