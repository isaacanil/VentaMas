...
export const InsuranceAuthModal = () => {
  ...
  const handleBirthDateChange = (date: DateTime | null) => {
    if (date) {
      // Solo actualizar Redux, no guardar inmediatamente
      dispatch(
        updateAuthField({ field: 'birthDate', value: date.toISO() }),
      );
    }
  };
  ...
  useEffect(() => {
    if (open && authData && !initialDataLoaded) {
      ...
      const resolvedInsuranceId =
        typeof formValues.insuranceId === 'string'
          ? formValues.insuranceId
          : null;
      if (resolvedInsuranceId) {
        setSelectedInsurance(resolvedInsuranceId);
      }
      ...
      setHasDependent(Boolean(formValues.hasDependent));
    ...
    try {
      ...
      if (client?.id) {
        try {
          ...
          message.error(
            err.message || 'Error al guardar los datos de seguro',
          );
          setSubmitting(false);
          return;
        }
