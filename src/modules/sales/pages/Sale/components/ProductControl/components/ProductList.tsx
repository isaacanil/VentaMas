...
type VirtuosoContainerProps = HTMLAttributes<HTMLDivElement> & {
  context?: unknown;
};

const EMPTY_STATUS_META: StatusMeta = {};

const ListComponent = forwardRef<
  HTMLDivElement,
  VirtuosoContainerProps
>(({ context: _context, ...props }, ref) => <GridList ref={ref} {...props} />);
ListComponent.displayName = 'ProductListGrid';

const ItemComponent = forwardRef<
  HTMLDivElement,
  VirtuosoContainerProps
>(({ context: _context, ...props }, ref) => <GridItem ref={ref} {...props} />);
...
export const ProductList = ({
  products,
  productsLoading,
  statusMeta = EMPTY_STATUS_META,
}: ProductListProps) => {
  const navigate = useNavigate();

  const handlerProducts = useCallback(() => {
    const { INVENTORY_ITEMS } = ROUTES_NAME.INVENTORY_TERM;
    navigate(INVENTORY_ITEMS);
  }, [navigate]);

  const renderProduct = useCallback((_: number, product?: ProductRecord) => {
  ...
  return (
    <ProductsListContainer>
      {productsLoading ? (
        ...
        <GridVirtuoso
          data={products}
          computeItemKey={computeProductKey}
