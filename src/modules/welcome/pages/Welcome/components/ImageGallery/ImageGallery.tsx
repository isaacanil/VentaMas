...
const EMPTY_GALLERY_IMAGES: ImageGalleryImage[] = [];
...
export const ImageGallery = ({
  ...
  const handlePreview = useCallback(
    (image: ImageGalleryImage, index: number) => {
      ...
      >
        <AnimatePresence>
          {images.map((image, index) => (
            <ImageWrapper
              key={`${image.img}-${image.title || 'untitled'}`}
...
const EmptyContainer = styled(m.div)`
  display: flex;
  align-items: center;
  justify-content: center;
  min-height: 200px;
...
const GalleryContainer = styled(m.div)`
  display: grid;
...
const ImageOverlay = styled(m.div)`
  position: absolute;
  inset: 0;
  display: flex;
  gap: 12px;
...
const OverlayButton = styled(m.button)`
  display: flex;
  align-items: center;
  justify-content: center;
  width: 48px;
