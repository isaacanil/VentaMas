...
import { VmButton, VmInputGroup, VmListBox, VmSelect } from '@/components/heroui';
      ...
      <Header>
        ...
        <InputWrapper data-client-control-input="true">
          <ClientInputGroup fullWidth>
            {inputIcon && (
              <VmInputGroup.Prefix>{inputIcon}</VmInputGroup.Prefix>
            )}
            <VmInputGroup.Input
            ...
            {(clientInputValue || clientAction) && (
              <VmInputGroup.Suffix className="client-input-actions">
                {clientInputValue && (
                  <ClearClientButton
...
const ClientInputGroup = styled(VmInputGroup)`
  width: 100%;
  min-width: 0;
  height: 32px;
  min-height: 32px;
...
const ClientButton = styled(VmButton)`
  display: flex;
  align-items: center;
