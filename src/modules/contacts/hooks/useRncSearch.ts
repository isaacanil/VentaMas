...
const FIELD_MAPPINGS: Record<
  FieldType,
  { formKey: string; dgiiKey: string; label: string }
> = {
  rnc: {
    formKey: 'rnc',
    dgiiKey: 'rnc_number',
...
export const getRncSearchErrorMessage = (error: unknown) => {
  ...
  return 'No se pudo consultar el RNC. Intente de nuevo más tarde.';
};
...
export const useRncSearch = (
  ...
  const compareDgiiData = useCallback(
    <TFormData extends object>(formData: TFormData, dgiiData: DgiiRecord | null) => {
      ...
      const diffs = fieldsToCompare.reduce<Difference[]>((acc, field) => {
        const rawCurrentValue = formData[field.formKey as keyof TFormData];
        const currentValue =
  ...
  const syncWithDgii = useCallback(async () => {
    if (!rncInfo) {
      clearAll();
      return;
    }
    ...
    try {
      ...
      const updates = fieldsToSync.reduce<Record<string, unknown>>(
        (acc, field) => {
  ...
  }, [clearAll, compareDgiiData, fieldType, form, rncInfo]);

  const consultarRNC = useCallback(
    async (value: string, silent = false) => {
      ...
      try {
        ...
        const data = await fetchRncRecordByNumber(value);
        ...
        clearAll();
        if (!silent) {
          setError(getRncSearchErrorMessage(error));
        }
        return null;
      } finally {
        setLoading(false);
    ...
    [clearAll, compareDgiiData, fieldType, form],
  );
