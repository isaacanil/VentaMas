...
export const ResolveBankStatementLineModal = ({
  ...
  const selectedStatementLine =
    pendingStatementLines.find(
      (statementLine) => statementLine.id === selectedStatementLineId,
    ) ?? null;
  ...
  const selectedMovements = useMemo(
    () =>
      availableEntries.filter((entry) =>
        selectedMovementIds.includes(entry.id),
      ),
  ...
  return (
    <Modal
      ...
      open={open}
      okText={
        resolutionMode === 'write_off' ? 'Ajustar diferencia' : 'Resolver'
      }
      cancelText="Cancelar"
      closable={!submitting}
    ...
    >
      ...
      >
        <Form.Item
          ...
          rules={[
            { required: true, message: 'Seleccione la línea pendiente.' },
          ]}
        >
          <Select
            disabled={submitting}
            ...
            >
              <Select
                ...
                options={[
                  ...
                  {
                    label: 'Movimiento bancario no registrado',
                    value: 'unrecorded_bank_movement',
                  },
