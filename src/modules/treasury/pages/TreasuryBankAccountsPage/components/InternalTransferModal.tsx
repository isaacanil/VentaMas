...
export const InternalTransferModal = ({
  ...
  const selectedSourceAccount =
    activeAccounts.find(
      (account) => account.key === selectedSourceAccountKey,
    ) ?? null;
  ...
  const formatSourceMoney = (amount: number) =>
    formatMoney(amount, selectedSourceAccount?.currency ?? 'DOP');
      ...
      <Form form={form} layout="vertical">
        ...
        {selectedSourceAccount && sourceCurrentBalance != null ? (
          <Alert
            ...
            showIcon
            message={`Saldo origen actual: ${formatSourceMoney(sourceCurrentBalance)}`}
            description={
              hasOverdraftRisk
                ? `Saldo proyectado: ${formatSourceMoney(sourceProjectedBalance ?? 0)}. Debes autorizar sobregiro para continuar.`
                : sourceProjectedBalance != null
                  ? `Saldo proyectado después de transferir: ${formatSourceMoney(sourceProjectedBalance)}`
