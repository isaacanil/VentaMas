    ...
    >
      <Form form={form} layout="vertical" onValuesChange={handleValuesChange}>
        ...
        <Alert
          ...
          type="info"
          showIcon
          message={`Balance ledger final: ${moneyFormatter.format(expectedLedgerBalance)}`}
          description={
          ...
          <Alert
            ...
            description={
              variance === 0
                ...
                : `Diferencia final: ${moneyFormatter.format(variance ?? 0)}`
            }
          />
        ) : null}
        ...
        {reconciliationStatus ? (
          <Alert
            ...
            description={[
              `Inicial ledger: ${moneyFormatter.format(ledgerOpeningBalance ?? 0)}`,
              `Movimiento banco: ${moneyFormatter.format(statementMovementTotal ?? 0)}`,
              `Movimiento ledger: ${moneyFormatter.format(ledgerPeriodMovementTotal ?? 0)}`,
              `Variación inicial: ${moneyFormatter.format(openingVariance ?? 0)}`,
              `Variación periodo: ${moneyFormatter.format(periodVariance ?? 0)}`,
            ].join(' · ')}
          />
        ) : null}
