...
import { usePersonalPinController } from './hooks/usePersonalPinController';
...
const usePersonalPinManagementViewModel = () => {
  ...
  const {
    ...
    openRequestModal,
    openViewPinModal,
  } = usePersonalPinController(user);

  const canSelfGenerate = hasAuthorizationPinSelfGenerateAccess(user);
 ...
 * - Admin/Owner/Dev: Pueden generar su propio PIN
 */
export const PersonalPinManagement = () => {
  const {
    ...
    statusDisplay,
    user,
  } = usePersonalPinManagementViewModel();
