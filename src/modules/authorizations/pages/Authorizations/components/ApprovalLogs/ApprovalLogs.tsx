...
const EMPTY_APPROVAL_LOGS: ApprovalLogEntry[] = [];

interface ApprovalLogsTableProps {
  filteredLogs: ApprovalLogEntry[];
  loading: boolean;
...
const ApprovalLogs = ({ searchTerm = '' }: ApprovalLogsProps) => {
  ...
  const approvalLogsQuery = useQuery<ApprovalLogEntry[]>({
    queryKey: [
      'approvalLogs',
      businessId,
      actorId,
    ...
    queryFn: async () => {
      ...
      return Array.isArray(data) ? (data as ApprovalLogEntry[]) : [];
    },
    enabled: Boolean(businessId && actorId),
    staleTime: 15000,
    gcTime: 300000,
    refetchOnWindowFocus: false,
    retry: 1,
  });
  const logs = approvalLogsQuery.data ?? EMPTY_APPROVAL_LOGS;
  const loadError =
    approvalLogsQuery.error instanceof Error
      ? approvalLogsQuery.error.message
      : '';
  const loading = approvalLogsQuery.isLoading || approvalLogsQuery.isFetching;
    ...
    <Wrapper>
      <Controls>
        ...
        <Button
          ...
          icon={<ReloadOutlined />}
          onClick={() => {
            void approvalLogsQuery.refetch();
          }}
          loading={loading}
        >
          Actualizar
