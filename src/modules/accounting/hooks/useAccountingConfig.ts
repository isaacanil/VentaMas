...
export const useAccountingConfig = ({
  ...
  const settingsQueryKey = businessId
    ? `accountingSettings:${businessId}`
    : null;
  const [config, setConfig] = useState<AccountingSettingsConfig>(() =>
    defaultAccountingSettings(userId),
  );
  const [settingsSnapshotKey, setSettingsSnapshotKey] = useState<string | null>(
    null,
  );
  ...
  const [bankAccountsSnapshotKey, setBankAccountsSnapshotKey] = useState<
    string | null
  >(null);
  const {
  ...
  const bankAccountsQueryKey =
    includeBankingDetails && businessId
      ? `bankAccounts:${businessId}:${bankInstitutionCatalogSignature}`
      : null;
  const loading = Boolean(
    settingsQueryKey && settingsSnapshotKey !== settingsQueryKey,
  );
  const bankAccountsLoading = Boolean(
    bankAccountsQueryKey && bankAccountsSnapshotKey !== bankAccountsQueryKey,
  );
  ...
  useEffect(() => {
    if (!settingsQueryKey || !businessId) return undefined;
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setSettingsSnapshotKey(settingsQueryKey);
        setError(null);
      },
      (cause) => {
        setSettingsSnapshotKey(settingsQueryKey);
    ...
    return unsubscribe;
  }, [businessId, settingsQueryKey, userId]);

  useEffect(() => {
    if (!bankAccountsQueryKey || !businessId) return undefined;
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setBankAccountsSnapshotKey(bankAccountsQueryKey);
      },
      (cause) => {
    ...
    return unsubscribe;
  }, [bankAccountsQueryKey, businessId]);
