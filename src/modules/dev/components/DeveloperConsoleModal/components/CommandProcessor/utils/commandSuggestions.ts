...
export const getCommandSuggestions = (input: string): CommandSuggestion[] => {
  const searchText = normalizeTrimmedSearchText(input);
  ...
  const suggestions = allCommands.filter((cmd) => {
    const command = normalizeSearchText(cmd.command);
    return command.startsWith(searchText) || command.includes(searchText);
  });

  suggestions.sort((a, b) => {
    const aCommand = normalizeSearchText(a.command);
    const bCommand = normalizeSearchText(b.command);
