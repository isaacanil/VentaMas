...
const EMPTY_BUSINESS_IDS: string[] = [];

interface InventoryMigrationState {
  dryRun: boolean;
  running: boolean;
  businessIdsInput: string;
  results: SyncBusinessResult[] | null;
  error: string | null;
}

type InventoryMigrationAction =
  | { type: 'setDryRun'; value: boolean }
  | { type: 'setRunning'; value: boolean }
  | { type: 'setBusinessIdsInput'; value: string }
  | { type: 'setResults'; value: SyncBusinessResult[] | null }
  | { type: 'setError'; value: string | null }
  | { type: 'startRun' };

const createInitialInventoryMigrationState = (
  defaultBusinessIds: string[],
): InventoryMigrationState => ({
  dryRun: true,
  running: false,
  businessIdsInput: defaultBusinessIds.join(','),
  results: null,
  error: null,
});

const inventoryMigrationReducer = (
  state: InventoryMigrationState,
  action: InventoryMigrationAction,
): InventoryMigrationState => {
  switch (action.type) {
    case 'setDryRun':
    ...
    case 'setBusinessIdsInput':
      return { ...state, businessIdsInput: action.value };
    case 'setResults':
...
export default function InventoryMigration({
  defaultBusinessIds = EMPTY_BUSINESS_IDS,
}: InventoryMigrationProps) {
  const [state, dispatch] = useReducer(
    inventoryMigrationReducer,
    defaultBusinessIds,
    createInitialInventoryMigrationState,
  );
  const { dryRun, running, businessIdsInput, results, error } = state;
  ...
  const handleRun = useCallback(async () => {
    ...
    syncAllBusinessesProductsStock({
      ...
      .then((res) => {
        dispatch({
          type: 'setResults',
          value: res as SyncBusinessResult[],
        });
      })
      .catch((e) => {
        dispatch({
      ...
      >
        ...
        <div>
          <Checkbox
            checked={dryRun}
            onChange={(e) =>
              dispatch({ type: 'setDryRun', value: e.target.checked })
            }
            disabled={running}
          >
            Dry Run (solo simula)
