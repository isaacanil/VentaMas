...
export const BulkRecoveryTab: React.FC<BulkRecoveryTabProps> = ({
  ...
  const businessesWithRepairs = Array.isArray(bulkResult?.businesses)
    ? bulkResult.businesses.filter(
        (business) =>
          Array.isArray(business.repairs) && business.repairs.length > 0,
      )
    : [];
    ...
    <Card title="Recuperación automática masiva">
      <Space orientation="vertical" style={{ width: '100%' }} size="middle">
        <Paragraph type="secondary" style={{ marginBottom: 0 }}>
          El sistema evaluará facturas recientes en busca de discrepancias y
          reprogramará únicamente las tareas necesarias (crear factura canonical
          o cuenta por cobrar).
        </Paragraph>
        <Space align="center">
          ...
          <Switch
            ...
            onChange={(checked) =>
              updateBulkOption('runForAllBusinesses', checked)
            }
          />
        </Space>
        ...
        <Space wrap>
          <Button
            type="primary"
            loading={bulkLoading}
            onClick={handleBulkAutoRecovery}
          >
        ...
        {bulkResult && (
          <Space orientation="vertical" style={{ width: '100%' }} size="middle">
            <Alert
              type={
                bulkResult.metrics?.invoicesWithIssues ? 'warning' : 'success'
              }
              showIcon
              message={
                bulkOptions.dryRun
            ...
            />
            <Text type="secondary">
              Descarga el Excel para ver el detalle de cada negocio y factura
              pendiente.
            </Text>
