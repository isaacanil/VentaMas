...
import { autoRepairInvoiceV2 } from '../services/invoiceV2Admin.service';
...
export const useBulkInvoiceRecovery = ({
  ...
  const handleBulkAutoRecovery = useCallback(async () => {
    ...
    if (!bulkOptions.runForAllBusinesses && !selectedBusinessId) {
      message.warning(
        'Selecciona un negocio para ejecutar la recuperación automática.',
      );
      return;
    }
    setBulkLoading(true);
    ...
    try {
      ...
      let currentBusinessCursor =
        bulkOptions.startAfterBusinessId?.trim() || null;
      let currentInvoiceCursor =
        bulkOptions.startAfterInvoiceId?.trim() || null;
      ...
      while (continueRunning) {
        ...
        const response = (await autoRepairInvoiceV2(
          payload as any,
        )) as BulkResult;
