...
import type { StoredSession } from '@/firebase/Auth/fbAuthV2/types';
...
): ParsedStoredSession | null => {
  if (!session) {
    return null;
  }

  const rawToken = session.sessionToken;
  const rawExpiresAt = session.sessionExpiresAt;
...
export const Login = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const storedSession = parseStoredSession(getStoredSession());
