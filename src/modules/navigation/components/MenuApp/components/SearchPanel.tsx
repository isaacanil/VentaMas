...
export const SearchPanel = ({
  ...
  const searchDraftSourceKey = `${isOpen ? 'open' : 'closed'}\u0000${
    searchData ?? ''
  }`;
  const [searchDraft, setSearchDraft] = useState<SearchDraft>({
    sourceKey: searchDraftSourceKey,
    value: searchData || '',
  });

  const tempSearchData =
    searchDraft.sourceKey === searchDraftSourceKey
      ? searchDraft.value
      : searchData || '';
...
const SearchOverlay = styled(m.div)`
  position: fixed;
  inset: 0;
  z-index: 1001;
  cursor: pointer;
...
const SearchPanelContainer = styled(m.div)`
  position: fixed;
  top: 0.5em;
  right: 3%;
  left: 3%;
