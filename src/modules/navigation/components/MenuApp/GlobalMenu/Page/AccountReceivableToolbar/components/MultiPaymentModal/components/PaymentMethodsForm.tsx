...
const PaymentMethodsForm = ({
  ...
  const inputRefs = useRef<Record<string, InputNumberRef | null>>({});

  // Mapping de nombres de métodos de pago a texto legible
  const getMethodDisplayName = (method: PaymentMethodType) => {
    switch (method) {
      case 'cash':
        return 'Efectivo';
      case 'card':
  ...
  const handleMethodStatusChange = (
    ...
    if (checked && (!method.value || method.value === 0)) {
      ...
      if (activeMethodsCount === 0) {
        ...
        const totalValue = totalField ? Number(totalField.value || 0) : 0;
