import { BankOutlined, FileExcelOutlined } from '@/constants/icons/antd';
...
import type { AccountsReceivableRecord, TimestampLike } from '@/utils/accountsReceivable/types';
...
interface AccountReceivableToolbarProps {
  side?: 'left' | 'right';
  data?: ProcessedAccountRow[];
}

export const AccountReceivableToolbar = ({ side = 'left', data }: AccountReceivableToolbarProps) => {
  ...
  const { accountsReceivable: accounts } = useListenAccountsReceivable(
    user,
    datesSelected,
  ) as { accountsReceivable?: AccountsReceivableRecord[] };

  // Derivar processedAccounts durante el render usando useMemo
  const processedAccounts = useMemo<ProcessedAccountRow[]>(() => {
  ...
  const buildExportRows = (rows: ProcessedAccountRow[] = []) => {
    ...
    return rows.map((row) => {
      ...
      const createdAtMillis = toMillis(row?.date ?? null);
      const createdAtDate = createdAtMillis ? DateTime.fromMillis(createdAtMillis) : null;

      const lastPaymentMillis = toMillis(row?.lastPaymentDate ?? null);
      const lastPay = lastPaymentMillis ? DateTime.fromMillis(lastPaymentMillis) : null;
