...
export const AccountReceivableToolbar = ({
  side = 'left',
  data,
}: AccountReceivableToolbarProps) => {
  ...
  const buildExportRows = (rows: ProcessedAccountRow[] = []) => {
    ...
    return rows.map((row) => {
      ...
      const createdAtDate = createdAtMillis
        ? DateTime.fromMillis(createdAtMillis)
        : null;

      const lastPaymentMillis = toMillis(row?.lastPaymentDate ?? null);
      const lastPay = lastPaymentMillis
        ? DateTime.fromMillis(lastPaymentMillis)
        : null;
  ...
  const handleExportExcel = () => {
    ...
    void exportToExcel(rows, sheetName, fileName, onBeforeExport).then(
      () => {
