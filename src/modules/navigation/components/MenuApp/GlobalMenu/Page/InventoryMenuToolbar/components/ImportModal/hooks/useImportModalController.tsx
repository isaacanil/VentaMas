...
import { getProductAvailableHeaders } from '@/domain/products/import';
...
export const useImportModalController = ({
  ...
  const { essential, optionalGroups } = getProductAvailableHeaders(language);
  ...
  const uploadProps: UploadProps = {
    ...
    beforeUpload: (file) => {
      if (isUnsupportedLegacyExcelFile(file)) {
        message.error({
          content: (
            <div>
