...
const FieldSelector = ({
  ...
  const optionalFieldCount = flattenedOptionalFields.length;
  const selectedFieldCount = selectedFields.length;
  const checkAll =
    optionalFieldCount > 0 && selectedFieldCount === optionalFieldCount;
  const indeterminate =
    selectedFieldCount > 0 && selectedFieldCount < optionalFieldCount;

  const handleChange = (checkedValues: CheckboxValueType[]) => {
    const nextFields = checkedValues.map(String);
    setSelectedFields(nextFields);
    onFieldsChange(nextFields);
  };

  const handleCheckAllChange = (e: CheckboxChangeEvent) => {
    const nextFields = e.target.checked ? [...flattenedOptionalFields] : [];
    setSelectedFields(nextFields);
    onFieldsChange(nextFields);
  };
