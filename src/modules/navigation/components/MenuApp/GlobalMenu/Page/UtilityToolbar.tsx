...
export const UtilityToolbar = ({ side = 'left' }: ToolbarComponentProps) => {
  ...
  useEffect(() => {
    const handleAvailability = (event: Event) => {
      const customEvent = event as CustomEvent<{ canExport?: boolean }>;
      if (typeof customEvent.detail?.canExport === 'boolean') {
        setCanExport(customEvent.detail.canExport);
      }
    };
