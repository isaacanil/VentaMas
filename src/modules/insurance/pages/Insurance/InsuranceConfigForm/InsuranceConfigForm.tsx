...
function getDisplayText(period: PeriodValue | null): string {
  if (!period) return '';
...
const InsuranceConfigForm = () => {
  ...
  const buildInsuranceTypes = useCallback((rawTypes?: RawInsuranceType[]) => {
    ...
    return rawTypes.map((type) => {
      ...
      const convertPeriod = (period: unknown): PeriodValue | null => {
        ...
        if (
          ...
          const totalDays =
            period.value *
            (TIME_UNITS.find((u) => u.unit === period.timeUnit)?.value || 1);
  ...
  const initializeForm = useCallback(() => {
    setInsuranceTypes(buildInsuranceTypes(initialValues?.insuranceTypes));
    form.setFieldsValue({
  ...
  }, [buildInsuranceTypes, form, initialValues]);
  ...
  const handleSubmit = async (values: InsuranceFormValues) => {
    ...
    const saveError = await saveInsuranceConfig(
  ...
  const handlePeriodSelect = (
    periodInfo: PeriodValue & { displayText: string },
  ) => {
