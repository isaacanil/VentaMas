...
import type { PurchaseBackOrderSelection } from '@/utils/purchase/types';

type BackOrderRow = BackOrder & {
...
interface BackOrdersModalProps {
  ...
  initialSelectedBackOrders?: PurchaseBackOrderSelection[];
...
const EMPTY_BACK_ORDERS: BackOrderRow[] = [];
const EMPTY_SELECTED_BACK_ORDERS: PurchaseBackOrderSelection[] = [];
...
const toDateTime = (value: unknown) => {
  ...
  const valueWithDate = value as { toDate?: () => Date };
  if (typeof valueWithDate?.toDate === 'function') {
    return DateTime.fromJSDate(valueWithDate.toDate());
  }
  return null;
};

const BackOrdersModal = ({
  backOrders = EMPTY_BACK_ORDERS,
  isVisible,
  onCancel,
  onConfirm,
  initialSelectedBackOrders = EMPTY_SELECTED_BACK_ORDERS,
  initialPurchaseQuantity = 0,
  productId,
  backOrderAssociationId,
}: BackOrdersModalProps) => {
  ...
  const [
    { trigger, selectedBackOrders, purchaseQuantity: purchaseQuantityState },
    setLocalState,
  ] = useState<{
    trigger: string;
    selectedBackOrders: PurchaseBackOrderSelection[];
  ...
  const setLocalSelectedBackOrders = useCallback(
    (
      updater:
        ...
        | ((
            prev: PurchaseBackOrderSelection[],
          ) => PurchaseBackOrderSelection[]),
    ) => {
      setLocalState((prev) => {
      ...
      setLocalState((prev) => {
        ...
        const currentQty =
          prev.trigger === openTrigger
            ...
            : initialPurchaseQuantity;
        const nextQty =
          typeof updater === 'function' ? updater(currentQty) : updater;
        return {
          trigger: openTrigger,
  ...
  const handleBackOrderSelect = (
    ...
    if (e.target.checked) {
      setLocalSelectedBackOrders((prev) => [
        ...
        {
          id: record.id,
          quantity: record.pendingQuantity || 0,
  ...
  const columns: ColumnsType<BackOrderRow> = [
    {
      title: (
        <Checkbox
        ...
        <CompactStatsSection>
          <QuantityInputSection>
            ...
            <InputNumber
              ...
              style={{ width: 120 }}
              placeholder={String(totalBackordersQuantity)}
            />
          </QuantityInputSection>
          <QuickStats>
            <span>
