...
const GridListContainer = styled.div`
  ...
  align-content: start;
  align-items: start;
  padding: 0 1em 1em; /* Added bottom padding */
`;
...
const ProductCard = styled.button<{ $isSelected?: boolean }>`
  ...
  border: 1px solid
    ${({ $isSelected }) => ($isSelected ? '#1677ff' : '#e8e8e8')};
  border-radius: 8px;
...
const ProductModal = ({
  ...
  const inventoryProducts = useMemo(
    () =>
      filterInventoriableProducts(Array.isArray(products) ? products : []),
    [products],
  );
  ...
  const countsKey = visible && user?.businessID ? String(user.businessID) : null;
  ...
  useEffect(() => {
    ...
    const fetchAggregates = async () => {
      try {
        const inventory = await getInventoriableProductsCount(countsKey);

        if (!isMounted) return;

        setCountsState({ key: countsKey, inventory, status: 'success' });
      } catch (error) {
        console.error(
        ...
        if (!isMounted) return;
        setCountsState({ key: countsKey, inventory: null, status: 'error' });
      }
    };
  ...
  const countsLoading = !!countsKey && countsState?.key !== countsKey;
  const countsError = !!countsKey && countsState?.key === countsKey && countsState?.status === 'error';
  const countsSuccess = !!countsKey && countsState?.key === countsKey && countsState?.status === 'success';

  const inventoryTotal =
    countsSuccess && typeof countsState?.inventory === 'number'
      ? countsState.inventory
      : totalFiltered;
