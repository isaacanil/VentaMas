...
const BackOrders = () => {
  ...
  const [sortBy, setSortBy] = useState<BackorderSort>('name-asc');
  const [statusFilter, setStatusFilter] =
    useState<BackorderStatusFilter>('all');
  const [fulfillOpen, setFulfillOpen] = useState(false);
  const [selectedGroup, setSelectedGroup] = useState<BackorderGroup | null>(null);
  ...
  const groupedBackorders = useMemo(() => {
    ...
    const groups = filtered.reduce<Record<string, BackorderGroup>>((acc, item) => {
      const key = item.productId ?? 'unknown';
      const safeCreatedAt = item.createdAt ? new Date(item.createdAt) : new Date(0);
      const safeUpdatedAt = item.updatedAt ? new Date(item.updatedAt) : safeCreatedAt;
