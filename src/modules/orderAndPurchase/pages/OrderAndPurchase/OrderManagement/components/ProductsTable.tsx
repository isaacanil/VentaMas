...
interface ProductsTableProps {
  ...
  onEditProduct: (
    payload: OrderReplenishment & { index?: string | number },
  ) => void;
...
const EditableCell = (props: EditableCellProps) => {
  ...
  const inputControl = isProductModal ? (
    ...
    >
      <Input value={currentTextValue} placeholder="Seleccionar producto" readOnly />
    </ProductModal>
  ) : isNumberInput ? (
    <InputNumber
      ...
      parser={(value) =>
        String(value ?? '').replace(/\$\s?|(,*)/g, '') as any
      }
  ...
  return (
    <td {...restProps} onClick={onCellClick}>
      <Form.Item style={{ margin: 0 }} noStyle>{inputControl}</Form.Item>
    </td>
  );
};
