...
type OrderSubmissionArgs = Parameters<typeof addOrder>[0];
type OrderSubmissionUser = NonNullable<OrderSubmissionArgs['user']>;
type OrderSubmissionLocalFile = NonNullable<OrderSubmissionArgs['localFiles']>[number];
...
const toOrderSubmissionUser = (
  candidate: UserIdentity | null,
): OrderSubmissionUser | null => {
  const businessID = resolveUserIdentityBusinessId(candidate);
  const uid = resolveUserIdentityUid(candidate);
...
const toOrderSubmissionLocalFiles = (
  files: EvidenceFileInput[],
): OrderSubmissionLocalFile[] =>
  files.flatMap(({ file, ...rest }) => (file ? [{ ...rest, file }] : []));
...
const OrderManagement = () => {
  ...
  const { order: fetchedOrder, isLoading: orderLoading } = useListenOrder(
    id,
  ) as {
    order?: Order;
    isLoading?: boolean;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const submissionUser = toOrderSubmissionUser(user);
    if (!submissionUser) {
      notification.error({
        message: 'Usuario inválido',
        description:
          'No se pudo identificar el usuario y negocio actual para guardar el pedido.',
        duration: 5,
      });
      return;
    }
    ...
    const submissionLocalFiles = toOrderSubmissionLocalFiles(localFiles);
    const saveError =
      mode === 'create'
        ? await addOrder({
            user: submissionUser,
            order: submitData,
            localFiles: submissionLocalFiles,
          })
            .then(() => null)
            .catch((error) => error)
        : mode === 'update'
    ...
    navigate(ORDERS);
    setLoading(false);
  }, [dispatch, navigate, user, orderData, localFiles, ORDERS, mode]);
