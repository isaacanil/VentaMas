...
const rangesAreEqual = (
  a: UtilityDateRange | null | undefined,
  b: UtilityDateRange | null | undefined,
): boolean =>
  Boolean(a?.startDate && a?.endDate && b?.startDate && b?.endDate) &&
  a.startDate === b.startDate &&
  a.endDate === b.endDate;

const PRESET_LABELS: Record<UtilityPresetKey, string> = {
  today: 'Hoy',
  yesterday: 'Ayer',
...
const getComparisonMeta = (
  preset: UtilityPresetKey | string,
  { customRangeLabel }: { customRangeLabel?: string } = {},
): { title: string; previousLabel: string } => {
  switch (preset) {
    case 'today':
...
type MetricsEntry = UtilityDailyMetric | UtilityHourlyMetric;
type UtilityDashboardState = {
  rangeComparison: UtilityComparison;
  dailyChartData: ChartData<'line', number[], string> | null;
  dailyChartOptions: ChartOptions<'line'>;
  pieChartData: ChartData<'doughnut', number[], string> | null;
  pieChartOptions: ChartOptions<'doughnut'>;
  distributionDetails: UtilityDistributionDetail[];
  formatCurrency: UtilityCurrencyFormatter;
  formatPercentage: UtilityPercentageFormatter;
  chartSubtitle: string;
  loading: boolean;
  comparisonLoading: boolean;
  selectedPreset: UtilityPresetKey;
  rangeLabel: string;
  ...
  salesComparison: UtilityComparison;
};

export const useUtilityDashboard = (): UtilityDashboardState => {
  const [datesSelected, setDatesSelected] = useState<UtilityDateRange>(() =>
    getDateRange('today'),
  );
  const [selectedPreset, setSelectedPreset] = useState<UtilityPresetKey>('today');

  const thisWeekRange = useMemo<UtilityDateRange>(() => getDateRange('thisWeek'), []);
  const lastWeekRange = useMemo<UtilityDateRange>(() => getDateRange('lastWeek'), []);
  const previousRange = useMemo<UtilityDateRange>(
  ...
  const { invoices, loading: invoicesLoading } =
    useFbGetInvoices(datesSelected);
  ...
  const currentMetrics = useMemo<UtilityMetricsResult>(
    () => buildFinancialMetrics(invoices, expenses, datesSelected),
    [invoices, expenses, datesSelected],
  );
  const previousMetrics = useMemo<UtilityMetricsResult>(
    () =>
  ...
  const weekMetrics = useMemo<UtilityMetricsResult>(
    () =>
      isCurrentWeekRange
        ? currentMetrics
  ...
  const lastWeekMetrics = useMemo<UtilityMetricsResult>(
    () =>
  ...
  const formatCurrency = useCallback<UtilityCurrencyFormatter>(
    (value) => formatPrice(value ?? 0, 'rd'),
    [],
  );
  const formatPercentageValue = useCallback<UtilityPercentageFormatter>(
    (value) => formatPercentage(value ?? 0),
    [],
  );

  const computeWeeklySnapshot = useCallback(
    (accessor: (entry: MetricsEntry) => number): UtilityComparison => {
      if (
      ...
      const clampDate = (value: DateTime, min: DateTime, max: DateTime): DateTime => {
        if (value.toMillis() < min.toMillis()) return min;
        if (value.toMillis() > max.toMillis()) return max;
        return value;
      };

      const sumUntil = (metrics: UtilityMetricsResult, cutoff: DateTime): number => {
  ...
  const weeklyComparison = useMemo<UtilityComparison>(
    () => computeWeeklySnapshot((entry) => entry.netProfit),
    [computeWeeklySnapshot],
  );

  const weeklySalesComparison = useMemo<UtilityComparison>(
  ...
  const rangeComparison = useMemo<UtilityComparison>(() => {
  ...
  const salesComparison = useMemo<UtilityComparison>(() => {
    const meta = getComparisonMeta(selectedPreset, {
      customRangeLabel: rangeLabel,
    });
  ...
  const lastSevenDaysMetrics = useMemo<UtilityDailyMetric[]>(() => {
  ...
  const yearlyAggregatedMetrics = useMemo<UtilityDailyMetric[]>(() => {
  ...
  const chartTimeSeriesMetrics = useMemo<MetricsEntry[]>(() => {
    if (isHourlyView) {
  ...
  const dailyChartData = useMemo<ChartData<'line', number[], string> | null>(() => {
  ...
  const dailyChartOptions = useMemo<ChartOptions<'line'>>(
    () => ({
      responsive: true,
  ...
  const pieChartData = useMemo<ChartData<'doughnut', number[], string> | null>(() => {
  ...
  const handlePresetSelect = useCallback((key: UtilityPresetKey, range?: UtilityDateRange) => {
    if (key === 'custom') {
