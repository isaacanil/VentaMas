...
export const useUtilityDashboard = (): UtilityDashboardState => {
  ...
  const [selectedPreset, setSelectedPreset] =
    useState<UtilityPresetKey>('today');

  const thisWeekRange = useMemo<UtilityDateRange>(
    () => getDateRange('thisWeek'),
    [],
  );
  const lastWeekRange = useMemo<UtilityDateRange>(
  ...
  const dailyChartData = useMemo<ChartData<
    'line',
    number[],
    string
  > | null>(() => {
  ...
  const handlePresetSelect = useCallback(
    (key: UtilityPresetKey, range?: UtilityDateRange) => {
      if (key === 'custom') {
