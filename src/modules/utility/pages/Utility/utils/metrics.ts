...
const resolveExpenseTimestamp = (
  expenseEntry: UtilityExpenseEntry,
): number | null => {
...
export const buildFinancialMetrics = (
  ...
  const safeInvoices: UtilityInvoiceEntry[] = Array.isArray(invoices)
    ? invoices
    : [];
  const safeExpenses: UtilityExpenseEntry[] = Array.isArray(expenses)
    ? expenses
    : [];
  ...
  const daily = new Map<
    string,
    { sales: number; cost: number; taxes: number; expenses: number }
  >();
  ...
  const ensureDailyEntry = (
    ...
    const isoDate =
      DateTime.fromMillis(millis).startOf('day').toISODate() || '';
  ...
  safeInvoices.forEach((invoice) => {
    ...
    const invoiceSales =
      ...
      invoiceProducts.reduce((acc, entry) => {
        const product =
          (entry as { product?: UtilityProductInput })?.product ?? entry ?? {};
        return (
          acc + resolveSaleTotal(product as UtilityProductInput, taxEnabled)
        );
      }, 0);
    let invoiceCost = 0;
    ...
    invoiceProducts.forEach((productEntry) => {
      ...
      const product = (
        hasNestedDetails ? { ...productRest, ...nestedRecord } : productRest
      ) as UtilityProductInput;
  ...
  const dailyMetrics: UtilityDailyMetric[] = Array.from(daily.entries()).map(
    ([key, value]) => {
      ...
      return {
        ...
        dateLabel: date.isValid
          ? date.setLocale('es').toFormat('cccc dd/LL/yyyy')
          : 'Fecha inválida',
        sales: value.sales,
        cost: value.cost,
        taxes: value.taxes,
  ...
  const hourlyMetrics: UtilityHourlyMetric[] = Array.from(hourly.entries()).map(
    ([key, value]) => {
  ...
  const productsBreakdown: UtilityProductBreakdown[] = Array.from(
    perProduct.values(),
  )
    .map((item) => ({
      ...item,
