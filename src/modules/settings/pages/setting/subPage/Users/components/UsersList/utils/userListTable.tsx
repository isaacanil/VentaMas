...
import { getPresenceLabel, toMillis } from './userList';

type UserListColumns = NonNullable<AdvancedTableProps<UserListRow>['columns']>;
...
export const createUserListColumns = ({
  ...
  onToggleStatus,
  onViewActivity,
}: CreateUserListColumnsArgs): UserListColumns => {
  const baseColumns: UserListColumns = [
    {
      Header: '#',
      accessor: 'number',
      align: 'left',
    ...
    },
    {
      ...
      cell: ({ value }: { value: unknown }) => {
        ...
        return (
          <DateCell>
            <span className="date">
              {DateTime.fromMillis(millis).toLocaleString(DateTime.DATE_SHORT)}
            </span>
          </DateCell>
        );
      },
    },
    {
      ...
      align: 'left',
      cell: ({ value }: { value: unknown }) => {
        const roleValue = normalizeRoleId(value as UserProfile['role']);
        const role = userRoles.find((item) => item.id === roleValue);
        return (
          <Role
            $primaryColor={role?.primaryColor ?? '#475467'}
            $secondaryColor={role?.secondaryColor ?? 'rgba(148, 163, 184, 0.18)'}
          >
