...
interface DynamicPermissionsManagerProps {
  userId?: string;
  userName?: string;
  userRole?: UserRoleLike | null;
  isOpen: boolean;
  onClose: () => void;
}
...
const EMPTY_PERMISSIONS: DynamicPermissionsPayload = {
...
const DynamicPermissionsManager = ({
  ...
  isOpen,
  onClose,
}: DynamicPermissionsManagerProps) => {
  const [permissions, setPermissions] =
    useState<DynamicPermissionsPayload>(EMPTY_PERMISSIONS);
  ...
  const canManagePermissions = Boolean(abilities?.can?.('manage', 'users'));

  const loadUserPermissions = useCallback(() => {
    if (!userId || !user) return;
    void getUserDynamicPermissions(userId, user).then(
      (userPermissions) => {
        setPermissions({
          additionalPermissions: userPermissions.additionalPermissions || [],
          restrictedPermissions: userPermissions.restrictedPermissions || [],
        });
  ...
  const loading = Boolean(isOpen && userId && loadedUserId !== userId);

  const availablePermissions = useMemo(
    () => getAvailablePermissionsForRole(userRole ?? null),
    [userRole],
  );
  ...
  const permissionsByCategory = useMemo<PermissionGroup[]>(() => {
  ...
  const togglePermission = useCallback(
    (permission: PermissionDefinition, enabled: boolean) => {
      setPermissions((prev) => {
  ...
  const handleSave = () => {
    if (!userId || !user?.businessID) {
      alert('No se pudo guardar permisos: falta información del negocio.');
      return;
    }

    setSaving(true);
    void setUserDynamicPermissions(user, userId, permissions).then(
      () => {
        onClose();
        setSaving(false);
      },
      (error) => {
        ...
        alert(`Error guardando permisos: ${message}`);
        setSaving(false);
      },
    );
  };
  ...
  const roleInfo = getRolePermissionsInfo(userRole ?? null);
...
const PermissionsContainer = styled.div`
  ...
  overflow: hidden;
  border: 1px solid
    ${({ theme }: ThemeProps) => theme?.border?.primary ?? '#f0f0f0'};
  border-radius: 12px;
`;
