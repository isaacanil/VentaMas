...
const buildInvoiceList = (items: Conflict[]) => (
  <ul>
    {items.map((invoice) => {
...
export const confirmSequenceWarnings = async (
  ...
  const availableAfterRange: Range<LedgerEntry> | null =
    showAvailableAfter && availableAfterStart && availableAfterEnd
      ...
      : null;
  const currentConflictInvoiceList =
    insights?.currentConflict?.invoices?.length
      ? buildInvoiceList(insights.currentConflict.invoices)
      : null;
  ...
  const currentConflictBlock =
    showCurrentConflict && insights?.currentConflict ? (
      <div>
        <p>
