...
import type { TaxReceiptData, TaxReceiptFormValues } from '@/types/taxReceipt';
...
/* ===== Lógica original ===== */
type SequenceAnalysisStatus = 'idle' | 'loading' | 'success' | 'error';

interface SequenceAnalysisState {
  status: SequenceAnalysisStatus;
  result: unknown;
  error: unknown;
}

interface SequenceWarningDecision {
  accepted: boolean;
  warned: boolean;
}

type SequenceAnalysisAction =
  | { type: 'reset' }
  | { type: 'loading'; previousResult: unknown }
  | { type: 'success'; result: unknown }
  | { type: 'error'; error: unknown };

const initialSequenceAnalysisState: SequenceAnalysisState = {
  status: 'idle',
  result: null,
  error: null,
};

const sequenceAnalysisReducer = (
  state: SequenceAnalysisState,
  action: SequenceAnalysisAction,
): SequenceAnalysisState => {
  switch (action.type) {
    case 'reset':
      return initialSequenceAnalysisState;
    case 'loading':
      return {
        status: 'loading',
...
interface TaxReceiptFormProps {
  editModalVisible: boolean;
  setEditModalVisible: (visible: boolean) => void;
  currentEditItem: TaxReceiptData | null;
}
...
export default function TaxReceiptForm({
  ...
  const [sequenceAnalysis, dispatchSequenceAnalysis] = useReducer(
    sequenceAnalysisReducer,
    initialSequenceAnalysisState,
  );
  const analysisRequestRef = useRef(0);
  const latestSequenceResultRef = useRef<unknown>(null);
  ...
  const hasSequenceToAnalyze = Boolean(
    buildPrefix(serieValue, tipoValue) && toDigits(sequenceValue ?? ''),
  );
  const displayedSequenceAnalysis = hasSequenceToAnalyze
    ? sequenceAnalysis
    : { status: 'idle', result: null, error: null };

  useEffect(() => {
    latestSequenceResultRef.current = sequenceAnalysis.result;
  ...
  useEffect(() => {
    ...
    if (!prefix || !digits) {
      dispatchSequenceAnalysis({ type: 'reset' });
      return undefined;
    }
    ...
    const previousResult = latestSequenceResultRef.current;

    const handle = setTimeout(() => {
      dispatchSequenceAnalysis({
        type: 'loading',
        previousResult,
      });
      ...
      checkSequenceConflicts(pendingValues)
        .then((result) => {
          ...
          dispatchSequenceAnalysis({ type: 'success', result });
        })
        .catch((error) => {
          if (analysisRequestRef.current !== requestId) return;
          dispatchSequenceAnalysis({ type: 'error', error });
        });
    }, 350);
  ...
  const handleSaveEdit = async () => {
    ...
    const formResult = await form
      .validateFields()
      .then((values) => ({
        values: values as TaxReceiptFormValues,
        error: null,
      }))
      .catch((error) => ({
        values: null,
    ...
    if (formResult.error) {
      if (!formResult.error?.errorFields) {
        console.error(
          'Error al guardar el comprobante fiscal:',
          formResult.error,
        );
        message.error(
    ...
    const validationResult = await checkSequenceConflicts(values)
      .then((validation) => ({
        validation,
        error: null,
      }))
    ...
    const sequenceValidation = validationResult.validation;
    ...
    const warningDecision = (await confirmSequenceWarnings(
      sequenceValidation,
    )) as SequenceWarningDecision;
    ...
    const updateError = await updateTaxReceipt(user as any, data)
      .then(() => null)
      .catch((error) => error);

    if (updateError) {
      console.error('Error al guardar el comprobante fiscal:', updateError);
      message.error(
        'Error al actualizar el comprobante fiscal. Por favor, inténtalo de nuevo más tarde.',
      );
      finishSave();
              ...
              <FormGrid>
                ...
                <Span12 as={MobileOnly}>
                  ...
                  <SequenceLedgerInsights
                    analysisState={displayedSequenceAnalysis}
