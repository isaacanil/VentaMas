...
const EMPTY_EXISTING_RECEIPTS: TaxReceiptDocument[] = [];
...
const AddReceiptDrawer = ({
  visible,
  onCancel,
  existingReceipts = EMPTY_EXISTING_RECEIPTS,
  ...
  const handleAddTemplates = async () => {
    ...
    message.loading({
      content: `Agregando ${selectedTemplates.length} comprobante(s)...`,
      key,
    });
    ...
    try {
      ...
      message.success({
        content: `${selectedTemplates.length} comprobante(s) agregado(s) correctamente.`,
        key,
        duration: 2,
      });
