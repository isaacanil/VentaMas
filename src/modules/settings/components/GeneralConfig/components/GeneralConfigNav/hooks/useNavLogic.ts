...
export const useNavLogic = (
  ...
  >({});
  const [userToggledItems, setUserToggledItems] = useState<
    Record<string, boolean>
  >({});
  ...
  const openGroups = useMemo(() => {
    ...
    Object.entries(groupedItemsByGroupKey).forEach(([groupKey, groupInfo]) => {
      ...
      // Auto-expand on initial load if the active tab belongs to this group
    ...
    return computed;
  }, [groupedItemsByGroupKey, userToggledGroups, activeTab]);

  const itemsWithChildren = useMemo(
    () =>
      menuItems.filter((item) => item.children?.length).map((item) => item.key),
    [menuItems],
  );
  ...
  const openItems = useMemo(() => {
    ...
    itemsWithChildren.forEach((itemKey) => {
      if (activeTab === itemKey) {
        computed[itemKey] = true;
        return;
      }

      computed[itemKey] = userToggledItems[itemKey] ?? false;
    });

    return computed;
  }, [activeTab, itemsWithChildren, userToggledItems]);
  ...
  const toggleItemChildren = (itemKey: string, event: React.MouseEvent) => {
    event.stopPropagation();
