...
import { useFbGetAvailableCreditNotes } from '@/modules/invoice/hooks/creditNote/useFbGetAvailableCreditNotes';
...
const EMPTY_CREDIT_NOTE_SELECTIONS: CreditNoteSelection[] = [];
...
const CreditSelector = ({
  clientId,
  onCreditNoteSelect,
  selectedCreditNotes = EMPTY_CREDIT_NOTE_SELECTIONS,
  totalPurchase = 0,
  ...
  const { creditNotes, loading, totalAvailable } = useFbGetAvailableCreditNotes(
    clientId,
  ) as {
  ...
  const widgetContent = !clientId ? (
    <Placeholder>Seleccione un cliente</Placeholder>
  ) : loading ? (
    <Skeleton active paragraph={false} />
  ) : !creditNotes.length ? (
    <Placeholder>Sin notas de crédito</Placeholder>
  ) : remainingToPay <= 0 && totalOtherPayments > 0 ? (
    <Placeholder>Compra totalmente pagada</Placeholder>
  ) : (
    <WidgetInfo>
      <RowTop>
        <LeftSection>
      ...
      <RowBottom>
        <span>
          ...
          {formatPriceByCurrency(
            Math.min(totalAvailable, remainingToPay),
            documentCurrency,
          )}
        </span>
        {totalSelected > 0 && (
          <SelectedBadge>
            {formatPriceByCurrency(totalSelected, documentCurrency)}
          </SelectedBadge>
        )}
      </RowBottom>
    </WidgetInfo>
                        ...
                        {selectedAmount > 0 && (
                          <div className="input-section">
                            ...
                            <InputContainer>
                              ...
                              />
                              <InfoIcon
                                title={`Máximo disponible: ${formatPriceByCurrency(maxAllowed, documentCurrency)}`}
                              >
                                <InfoCircleOutlined />
                              </InfoIcon>
                            </InputContainer>
...
const WidgetCard = styled.div<WidgetCardProps>`
  padding: 16px;
  cursor: ${({ $disabled }: WidgetCardProps) =>
...
const ListItem = styled.div<ListItemProps>`
  ...
  margin-bottom: 8px;
  border: 1px solid
    ${({ $selected }: ListItemProps) => ($selected ? '#495057' : '#f0f0f0')};
  border-radius: 6px;
...
const CustomCheckbox = styled.div<CustomCheckboxProps>`
  ...
  height: 20px;
  color: white;
  cursor: ${({ $disabled }: CustomCheckboxProps) =>
    $disabled ? 'not-allowed' : 'pointer'};
  background: ${({ $checked }: CustomCheckboxProps) =>
    $checked ? '#495057' : 'white'};
  border: 1px solid
  ...
  &&:hover {
    border-color: ${({ $disabled }: CustomCheckboxProps) =>
      $disabled ? '#d9d9d9' : '#495057'};
  }
`;

const InfoIcon = styled.div<InfoIconProps>`
  margin-left: 8px;
  color: ${({ $disabled }: InfoIconProps) =>
    $disabled ? '#d9d9d9' : '#8c8c8c'};
  cursor: pointer;
