...
} from '@/modules/invoice/components/Quotation/types';
...
export const Quotation = React.forwardRef<HTMLDivElement, QuotationProps>(
  ({ data, template = 'template2', ignoreHidden }, ref) => {
    ...
    const templateContent = (
      <QuotationTemplate2 ref={ref} data={data} ignoreHidden={ignoreHidden} />
    );

    return <InvoiceWrapper $template={template}>{templateContent}</InvoiceWrapper>;
  },
);
