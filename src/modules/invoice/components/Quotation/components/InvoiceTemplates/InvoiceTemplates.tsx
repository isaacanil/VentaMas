...
import { resolveQuotationPreviewTemplate } from '@/utils/invoice/template';
...
const InvoiceContainer = styled.div<InvoiceContainerProps>`
  width: ${({ $template }: InvoiceContainerProps) =>
    TEMPLATES_CONFIG[$template]?.width};
  height: ${({ $template }: InvoiceContainerProps) =>
    TEMPLATES_CONFIG[$template]?.height};
  padding: ${({ $template }: InvoiceContainerProps) =>
...
export default function InvoiceTemplates({
  ...
  const safeInvoiceType: QuotationTemplateKey =
    resolveQuotationPreviewTemplate(invoiceType);
