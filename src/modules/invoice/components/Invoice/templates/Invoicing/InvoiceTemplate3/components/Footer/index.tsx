...
export default function Footer({ data }: FooterProps) {
  ...
  const totals = {
    subtotal: formatInvoicePrice(totalsSnapshot.totalWithoutTaxes, data),
    discount: `-${formatInvoicePrice(discount || 0, data)}`,
    tax: formatInvoicePrice(totalsSnapshot.totalTaxes, data),
    delivery: formatInvoicePrice(totalsSnapshot.delivery, data),
    total: formatInvoicePrice(totalsSnapshot.totalPurchase, data),
  };
                  ...
                  {data?.paymentMethod?.map((method, index) => {
                    ...
                    return (
                      <p key={index} style={{ margin: 0 }}>
                        ...
                        : {formatInvoicePrice(toNumber(method.value), data)}
