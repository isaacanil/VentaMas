...
const renderProducts = (
  ...
  getTaxFn: typeof getTax,
  getTotalPriceFn: typeof getTotalPrice,
  resetAmountFn: typeof resetAmountToBuyForProduct,
  formatter: typeof separator,
) => {
  let prodText = '';
  ...
  if (Array.isArray(products) && products.length > 0) {
    products.forEach((product) => {
      ...
      if (product?.weightDetail?.isSoldByWeight) {
        ...
        leftCol = `${resolveInvoiceAmount(product?.amountToBuy)} x ${separator(
          getTotalPriceFn(
            resetAmountFn(product),
            taxReceiptEnabled,
          ),
      ...
      const col2 = formatColumn(
        separator(getTaxFn(product, taxReceiptEnabled)).toString(),
        10,
        'right',
      );
      const col3 = formatColumn(
        separator(getTotalPriceFn(product, taxReceiptEnabled)).toString(),
        17,
        'right',
      );
...
const renderPaymentArea = (
  ...
  const renderSingleValueItem = (item: { label?: string; value2?: string | number | null }) => {
    const left = formatColumn((item.label || '') + ':', 20, 'left');
    const right = formatColumn(item.value2 || '', 20, 'right');
    return left + right;
  };

  const renderDoubleValueItem = (item: { subtitle?: string; value1?: string | number | null; value2?: string | number | null }) => {
  ...
  const formatNumber = (num: unknown) => formatter(toNumber(num));
...
/* =========================================================
   ...
   ========================================================= */
export const InvoiceTemplate4 = React.forwardRef<HTMLDivElement, InvoiceTemplateProps>(
  ({ data, ignoreHidden }, ref) => {
    ...
    return (
      <HiddenPrintWrapper ignoreHidden={ignoreHidden} ref={ref}>
        <pre>{factura}</pre>
      </HiddenPrintWrapper>
    );
  },
