...
type InvoiceItemWideProps = {
  data: InvoiceData;
};

export const InvoiceItemWide = ({ data }: InvoiceItemWideProps) => {
  ...
  const formatDate = (seconds?: number | null) => {
    if (!seconds) return formatLocaleDate(defaultDate);
              ...
              <PaymentProgress>
                ...
                {!isPaidInFull && (
                  <PaymentLine>
                    ...
                    <span>Pendiente: {formatPrice(paymentInfo.pending)}</span>
                  </PaymentLine>
                )}
              </PaymentProgress>
            </TotalsBlock>
