...
const ActionsMenu = ({ value }: ActionsMenuProps) => {
  ...
  const snapshotCart = (data?.snapshot as { cart?: { isAddedToReceivables?: boolean } } | null)?.cart;
...
const SaleReportTable = ({ bills = [], searchTerm, loading }: SaleReportTableProps) => {
  const dispatch = useDispatch();
  const data = bills?.map(({ data }): TableRow => {
    ...
    return {
      ...
      dateGroup: invoiceDateMs
        ? DateTime.fromMillis(invoiceDateMs as number).toLocaleString(DateTime.DATE_FULL)
        : 'Fecha no disponible',
    };
  });
