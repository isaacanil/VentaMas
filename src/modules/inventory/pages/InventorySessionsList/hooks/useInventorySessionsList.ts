...
export function useInventorySessionsList(user: InventoryUserProfile | null) {
  ...
  const sessionEditorsRef = useRef<SessionEditorsMap>(sessionEditors);
  const inFlightEditorLoadsRef = useRef<Set<string>>(new Set());
  ...
  useEffect(() => {
    sessionEditorsRef.current = sessionEditors;
  }, [sessionEditors]);
  ...
  useEffect(() => {
    ...
    const pending = sessions.filter((s) => {
      const loadKey = `${sessionsKey}:${s.id}`;
      return (
        !sessionEditorsRef.current[s.id] &&
        !inFlightEditorLoadsRef.current.has(loadKey)
      );
    });
    ...
    pending.forEach((s) => {
      const loadKey = `${sessionsKey}:${s.id}`;
      inFlightEditorLoadsRef.current.add(loadKey);
      Promise.resolve().then(async () => {
        try {
          ...
          inFlightEditorLoadsRef.current.delete(loadKey);
  ...
  }, [businessId, sessionsKey, sessionsState.key, sessionsState.sessions]);
