...
export function buildGroupedRowMeta({
  ...
  const buildMeta = (g: InventoryGroup): InventoryRowMeta => {
    ...
    if (
      ...
      if (Number(counts[key]) !== Number(persistedTop))
        isModified = true;
    }
    ...
    if (g.canEditAtTop && g.topKey) {
      ...
      if (editorsMap.size) {
        ...
        const needsName = !!(currentUid && !usersNameCache[currentUid] && !meta?.updatedByName);
        const anyLoading = !!(currentUid && needsName && resolvingUIDs[currentUid]);
        userEl = anyLoading ? (
    ...
    if (hasMultipleLots) {
      ...
      if (expirations.size) {
        const list = Array.from(expirations).filter(
