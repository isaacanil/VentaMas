...
interface CountsPayload {
  ...
  manualExpirationDate?: ExpirationEditsMap[string] | null;
  stockSistema?: FieldValue;
  conteoReal?: FieldValue;
  diferencia?: FieldValue;
}

interface UseInventoryCountsResult {
  counts: CountsMap;
  setCounts: Dispatch<SetStateAction<CountsMap>>;
  serverCounts: CountsMap;
  setServerCounts: Dispatch<SetStateAction<CountsMap>>;
  countsMeta: CountsMetaMap;
  setCountsMeta: Dispatch<SetStateAction<CountsMetaMap>>;
  expirationEdits: ExpirationEditsMap;
  setExpirationEdits: Dispatch<SetStateAction<ExpirationEditsMap>>;
  hasChanges: boolean;
  saving: boolean;
  saveCounts: (params: SaveCountsParams) => Promise<SaveCountsResult | void>;
}
...
export function useInventoryCounts({
  ...
  const [serverCounts, setServerCounts] = useState<CountsMap>({});
  const [countsMeta, setCountsMeta] = useState<CountsMetaMap>({});
  const [expirationEdits, setExpirationEdits] = useState<ExpirationEditsMap>(
    {},
  );
  ...
  const hasChanges = useMemo(() => {
    ...
    for (const k of keys)
      if ((counts[k] ?? null) !== (serverCounts[k] ?? null)) return true;
  ...
  const saveCounts = async ({
  ...
  }: SaveCountsParams): Promise<SaveCountsResult | void> => {
    if (!db || !sessionId || !user?.businessID) return;

    const keysCountChanged = Object.keys(counts).filter(
      (k) => (counts[k] ?? null) !== (serverCounts[k] ?? null),
    );
    ...
    let keysToSave = Array.from(
      new Set([...keysCountChanged, ...expirationChanged]),
    );
    ...
    if (noexpToBatchMap.size) {
      keysToSave = keysToSave.filter(
        (k) => !(k.startsWith('noexp:') && noexpToBatchMap.has(k)),
      );
    }
    ...
    setSaving(true);
    try {
      const countsData: CountsPayload[] = [];
      for (const key of keysToSave) {
        const conteoReal = (() => {
          ...
          if (key.startsWith('batchGroup:stock:productOnly:')) {
            ...
            if (
              noexpAlt &&
              Object.prototype.hasOwnProperty.call(counts, noexpAlt)
            )
              return counts[noexpAlt];
          }
          return counts[key];
        })();
        ...
        if (key.startsWith('noexp:')) {
          ...
          if (Object.prototype.hasOwnProperty.call(expirationEdits, key)) {
            manualExpirationDate =
              expirationEdits[key] === CLEAR_SENTINEL
                ? CLEAR_SENTINEL
                : (expirationEdits[key] ?? null);
          ...
          if (manualExpirationDate !== undefined)
            payload.manualExpirationDate = manualExpirationDate;
        ...
        if (key.startsWith('batchGroup:')) {
          ...
          if (child) {
            ...
            if (bKey.startsWith('bn:')) {
              ...
              subset = stocks.filter(
                (s) => String(s.batchNumberId ?? '') === String(bn),
              );
          ...
          if (Object.prototype.hasOwnProperty.call(expirationEdits, key)) {
            ...
            if (noexpAlt) {
              if (
                Object.prototype.hasOwnProperty.call(expirationEdits, noexpAlt)
              ) {
                manualExpirationDate =
                  expirationEdits[noexpAlt] === CLEAR_SENTINEL
                    ? CLEAR_SENTINEL
                    : (expirationEdits[noexpAlt] ?? null);
              } else if (
                countsMeta[noexpAlt]?.manualExpirationDate !== undefined
              ) {
                manualExpirationDate =
                  countsMeta[noexpAlt].manualExpirationDate;
              }
            }
          } else {
      ...
      setCountsMeta((prev) => {
        const clone = { ...prev };
        for (const key of keysToSave) {
          const hasManualChange = Object.prototype.hasOwnProperty.call(
            expirationEdits,
            key,
          );
          if (
            key.startsWith('batchGroup:') ||
            ...
            clone[key] = {
              ...
              updatedAt: new Date(),
              ...(hasManualChange
                ? { manualExpirationDate: expirationEdits[key] ?? null }
                : {}),
            };
            ...
            if (key.startsWith('batchGroup:stock:productOnly:')) {
              ...
              if (
                ...
                clone[key] = {
                  ...curr,
                  manualExpirationDate: expirationEdits[noexpAlt] ?? null,
                };
              }
            }
          }
      ...
      try {
        ...
        logDebug(
          '[useInventoryCounts] No se pudo actualizar presence:',
          presenceError,
        );
      }
