...
import {
  ...
  Component,
  TableContainer,
} from './components/InventoryControl.styles';
import InventoryFooterBar from './components/InventoryFooterBar';
import InventoryGroupedTable from './components/InventoryGroupedTable/InventoryGroupedTable';
...
export const InventoryControl = () => {
  ...
  useInventoryPresence({
    ...
    enabled: !!sessionId && session?.status === 'open',
  });
  ...
  const fullGroups = useMemo<InventoryGroup[]>(
    () =>
      buildInventoryGroups({
        items: augmentedStocks,
        counts,
        serverCounts,
        locationNames,
        stockFilter: 'all',
  ...
  // Export helpers
  const handleExport = async (opts: ExportInventoryOptions = {}) => {
    const filenameBase = session?.name
      ? `inventario_${session.name}`
      : 'inventario';
    const exportSession = { id: session?.id || sessionId, name: session?.name };
    try {
  ...
  const onExportMenuClick: MenuProps['onClick'] = async ({ key }) => {
    if (key === 'full')
      return handleExport({ onlyDifferences: false, includeBatchKey: false });
    if (key === 'diffs')
      return handleExport({ onlyDifferences: true, includeBatchKey: false });
  };

  const handleSaveCounts = async () => {
    let res: Awaited<ReturnType<typeof saveCounts>> | undefined;
    try {
      res = await saveCounts({ groups, stocks, currentUserResolvedName });
    } catch {
      message.error('Error al guardar conteos');
      return;
    }

    const hasSavedChanges = Boolean(res && res.saved > 0);
    if (hasSavedChanges) message.success('Conteos guardados exitosamente');
    else message.info('No hay cambios para guardar');
  };
