...
interface WarehouseSummaryState {
  loading: boolean;
  summaries: WarehouseSummaryMap;
}

type WarehouseSummaryAction =
  | { type: 'reset' }
  | { type: 'start' }
  | { type: 'success'; summaries: WarehouseSummaryMap };

const initialWarehouseSummaryState: WarehouseSummaryState = {
  loading: false,
  summaries: {},
};

const warehouseSummaryReducer = (
  state: WarehouseSummaryState,
  action: WarehouseSummaryAction,
): WarehouseSummaryState => {
  switch (action.type) {
    case 'reset':
      return initialWarehouseSummaryState;
    case 'start':
      return {
        ...state,
        loading: true,
...
export const FilterPanel = ({
  ...
  const [warehouseSummaryState, dispatchWarehouseSummary] = useReducer(
    warehouseSummaryReducer,
    initialWarehouseSummaryState,
  );
  ...
  useEffect(() => {
    ...
    const fetchSummaries = async () => {
      if (!user?.businessID || warehouseIds.length === 0) {
        if (isMounted) {
          dispatchWarehouseSummary({ type: 'reset' });
        }
        return;
      }
      dispatchWarehouseSummary({ type: 'start' });
      const summariesResult = await getWarehousesStockAggregates(
        user,
        warehouseIds,
      )
        .then((summaries) => ({
      ...
      if (summariesResult.error || !summariesResult.summaries) {
        console.error(
          'Error obteniendo agregados de almacenes:',
  ...
  const locationOptions = useMemo<InventoryLocationOption[]>(() => {
    const options = (warehouses || [])
      .map((warehouse) => {
        ...
        if (!id) return null;
        const summary = warehouseSummaryState.summaries?.[id];
          ...
          <>
            <GroupContainer>
              ...
              <InventoryLocationSelector
                ...
                loading={warehousesLoading || warehouseSummaryState.loading}
