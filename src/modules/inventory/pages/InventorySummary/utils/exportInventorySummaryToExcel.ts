...
export type InventorySummaryExportOptions = {
  filenamePrefix?: string;
};

export async function exportInventorySummaryToExcel(
  rows: InventorySummaryRow[],
  options: InventorySummaryExportOptions = {},
): Promise<void> {
  const { filenamePrefix = 'inventario_resumen' } = options;
  ...
  sheet.columns = [
    ...
    {
      header: 'Valor inventario (precio lista)',
      key: 'inventoryValueListPrice',
      width: 26,
    },
  ];
  ...
  // Formulas
  totalsRow.getCell('units').value = {
    formula: `SUM(D${startRow}:D${endRow})`,
  };
  totalsRow.getCell('inventoryValue').value = {
    formula: `SUM(F${startRow}:F${endRow})`,
  };
  totalsRow.getCell('inventoryValueListPrice').value = {
    formula: `SUM(H${startRow}:H${endRow})`,
  };
  // Weighted averages for unit cost and unit list price
  totalsRow.getCell('unitCost').value = {
    formula: `IF(D${totalsRowNumber}>0,F${totalsRowNumber}/D${totalsRowNumber},0)`,
  };
  totalsRow.getCell('unitListPrice').value = {
    formula: `IF(D${totalsRowNumber}>0,H${totalsRowNumber}/D${totalsRowNumber},0)`,
  };
