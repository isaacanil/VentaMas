...
import type { TreeConfig } from '@/modules/inventory/components/tree/types';
...
export const ProductMovementModal = ({
  ...
  const treeData = (warehouseData || []) as WarehouseNode[];
  ...
  const handleOk = async () => {
    ...
    try {
      await moveProduct(movementPayload);

      message.success('Movimiento realizado exitosamente.');
      form.resetFields();
      onOk(confirmedMovement);
    } catch (error) {
  ...
  const treeConfig: TreeConfig<WarehouseNode> = {
    onNodeClick: (node: WarehouseNode) => {
      if (node.id === currentNode?.id) {
        ...
        form.setFieldValue('destination', node.name);
      }
    },
  };
