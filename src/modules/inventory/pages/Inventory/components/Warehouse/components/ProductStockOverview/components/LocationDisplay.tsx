...
const getLocationIcon = (type: 'warehouse' | 'shelf' | 'row' | 'segment') => {
  switch (type) {
    case 'warehouse':
      return faWarehouse;
    case 'shelf':
...
const LocationPath = styled.div<{ $variant: LocationDisplayVariant }>`
  display: flex;
  flex-wrap: ${({ $variant }: { $variant: LocationDisplayVariant }) =>
    $variant === 'inline' ? 'wrap' : 'nowrap'};
  gap: 8px;
  align-items: center;
  width: 100%;
  ...
  .navigation-icon {
    display: ${({ $variant }: { $variant: LocationDisplayVariant }) =>
      $variant === 'inline' ? 'none' : 'block'};
    flex-shrink: 0;
    font-size: 0.9rem;
    color: #2563eb;
...
const LocationBadge = styled.div<{ $variant: LocationDisplayVariant }>`
  ...
  gap: ${({ $variant }: { $variant: LocationDisplayVariant }) =>
    $variant === 'inline' ? '8px' : '6px'};
  align-items: ${({ $variant }: { $variant: LocationDisplayVariant }) =>
    $variant === 'inline' ? 'center' : 'flex-start'};
  padding: ${({ $variant }: { $variant: LocationDisplayVariant }) =>
    $variant === 'inline' ? '6px 0' : '8px'};
  margin-top: ${({ $variant }: { $variant: LocationDisplayVariant }) =>
    $variant === 'inline' ? '0' : 'auto'};
  font-size: 0.9rem;
  color: #1e293b;
  cursor: pointer;
  ...
  &:hover {
    ...
    box-shadow: ${({ $variant }: { $variant: LocationDisplayVariant }) =>
      $variant === 'inline' ? 'none' : '0 4px 12px rgba(37, 99, 235, 0.1)'};
