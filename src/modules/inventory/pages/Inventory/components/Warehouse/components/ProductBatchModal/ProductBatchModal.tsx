...
import { Modal, Button, Input, Empty, Spin, Checkbox, App } from 'antd';
...
const BatchCard = styled.div<BatchCardProps>`
  ...
  gap: 10px;
  padding: 6px 10px;
  cursor: ${({ $disabled }: BatchCardProps) =>
  ...
  &:hover {
    ...
    transform: ${({ $disabled }: BatchCardProps) =>
  ...
  .check-icon {
    ...
    transform: ${({ $selected }: BatchCardProps) =>
      $selected ? 'scale(1)' : 'scale(0.5)'};
    transition: all 0.2s ease;
  }
`;
