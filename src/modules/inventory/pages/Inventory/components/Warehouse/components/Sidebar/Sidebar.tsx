...
const SummaryStatus = styled.span<{ $loading: boolean }>`
  ...
  align-items: center;
  font-size: 0.72rem;
  color: ${({ $loading }: { $loading: boolean }) =>
    $loading ? '#bfdbfe' : '#bef264'};
`;
...
const EMPTY_SIDEBAR_NODES: SidebarNodeRecord[] = [];
const EMPTY_SIDEBAR_PATH: string[] = [];
...
const Sidebar = ({
  ...
  const stockSummaryRequestKey = useMemo(() => {
    ...
    return `${user.businessID}:${locationPaths.join('|')}`;
  }, [locationPaths, user]);

  const stockSummaries = useMemo(() => {
    if (
      !stockSummaryRequestKey ||
      stockSummaryResponse.requestKey !== stockSummaryRequestKey
    ) {
      return {} as StockSummaryMap;
    }
    return stockSummaryResponse.summaries;
  }, [stockSummaryRequestKey, stockSummaryResponse]);

  const loadingStockSummaries =
    stockSummaryRequestKey !== null &&
    stockSummaryResponse.requestKey !== stockSummaryRequestKey;
  ...
  const handleDelete = (node: SidebarNodeRecord, level: number) => {
    ...
    Modal.confirm({
      ...
      cancelText: 'Cancelar',
      onOk: async () => {
        const deleteError = await deleteConfig[nodeType]
          .deleteFn(path, node)
          .then(() => null)
          .catch((error) => error);

        if (deleteError) {
          console.error(`Error al eliminar ${node.name}: `, deleteError);
  ...
  useEffect(() => {
    ...
    getStockAggregatesByLocationPaths(user, locationPaths)
      .then((summaries) => {
        ...
        setStockSummaryResponse({
          requestKey: stockSummaryRequestKey,
          summaries: summaries as StockSummaryMap,
        });
      })
      .catch((error) => {
        if (!active) {
  ...
  }, [locationPaths, stockSummaryRequestKey, user]);
