...
interface SegmentListItemProps {
  onDelete: (segment: SegmentRecord) => void;
  onNavigate: (segment: SegmentRecord) => void;
  onUpdate: (segment: SegmentRecord) => void;
...
export default function RowShelfContent() {
  ...
  const location = useMemo<InventoryStockItem['location']>(
    () =>
      rowId
        ? {
            warehouseId,
