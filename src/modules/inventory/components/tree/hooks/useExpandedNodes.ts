...
const useExpandedNodes = (data: TreeNodeData[]) => {
  const [manualExpandedNodes, setManualExpandedNodes] =
    useState<ExpandedMap>({});
  const [searchExpandedNodes, setSearchExpandedNodes] =
    useState<ExpandedMap>({});
  const [manuallyClosedNodes, setManuallyClosedNodes] =
    useState<ExpandedMap>({});
