...
const DetailLine = styled.span<{ $variant?: string }>`
  font-size: 0.72rem;
  font-weight: ${({ $variant }: { $variant?: string }) =>
    $variant === 'actual' ? 600 : 400};
  line-height: 1.1;
...
const EMPTY_NODE_DETAILS: TreeNodeDetail[] = [];
const EMPTY_TOOLTIP_DETAILS: NonNullable<NodeNameProps['tooltipDetails']> = [];
...
const NodeName = ({
  ...
  themeStyles,
  extraDetails = EMPTY_NODE_DETAILS,
  tooltipDetails = EMPTY_TOOLTIP_DETAILS,
}: NodeNameProps) => {
  ...
  const matchCountLabel =
    config?.showMatchedStockCount && matchedStockCount
      ? ` (${matchedStockCount} producto(s) encontrado(s))`
      : null;
  ...
  const summaryMeta = useMemo<SummaryMeta>(() => {
    ...
    const totalLots =
      typeof stockSummary.totalLots === 'number'
        ...
        : null;
    const directLots =
      typeof stockSummary.directLots === 'number'
        ? stockSummary.directLots
        : null;
    const totalUnits =
      typeof stockSummary.totalUnits === 'number'
        ? stockSummary.totalUnits
        : null;
    const directUnits =
      typeof stockSummary.directUnits === 'number'
    ...
    const buildLine = (
      ...
      if (unitLabel && typeof unitsValue === 'number' && unitsValue > 0)
    ...
    const directLabel =
      typeof directLots === 'number' ? formatLots(directLots) : null;
    const directEmpty =
      typeof directLots === 'number' ? (directLots ?? 0) <= 0 : false;
    ...
    <NameContainer isMatch={isMatch}>
      <TitleWrapper>
        ...
        {detailLines.map((detail) => (
          <DetailLine
            key={`${detail.type ?? 'default'}-${detail.text}`}
            $variant={detail.type}
          >
            {detail.text}
          </DetailLine>
