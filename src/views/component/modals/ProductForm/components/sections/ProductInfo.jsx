...
export const ProductInfo = ({ product, productBrands = [] }) => {
    ...
    const handleOpenBrandModal = () => dispatch(openBrandModal({ initialValues: null }));
    ...
    const brandFieldMeta = useMemo(() => {
        ...
        if (normalizedType.includes('medic') || normalizedType.includes('farm') || normalizedType.includes('salud')) {
            return {
                ...
                helper: 'Indica la marca comercial, laboratorio o denominación bajo la cual se vende el producto.'
            };
        }
        if (normalizedType.includes('bebida') || normalizedType.includes('alimento') || normalizedType.includes('consumo')) {
            return {
                ...
                helper: 'Puedes registrar la marca comercial, línea artesanal o fabricante principal.'
            };
        }
        if (normalizedType.includes('cosm') || normalizedType.includes('higiene') || normalizedType.includes('belleza')) {
            return {
                ...
                helper: 'Define la casa comercial, línea o fabricante responsable del producto.'
            };
        }
        return {
            label: 'Marca',
            placeholder: 'Ej: Samsung, Genérico, Marca Propia',
            helper: 'Registra la marca, fabricante o referencia que identifique el producto en tu catálogo.'
        };
    }, [product?.type]);
