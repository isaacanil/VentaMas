...
export const FilterBar = memo(
  ({
    ...
    const handleFiltersChange = useCallback(
      (newFilters: FilterState['filters']) => {
    ...
    const renderFilter = useCallback(
      (filterConfig: FilterConfigItem, isInDrawer: boolean) => {
        if (filterConfig.type === 'search') return null;

        let finalOptions: FilterOption[] = filterConfig.options || [];
        const dataEntry = dataConfig[filterConfig.key];
        if (dataEntry) {
          finalOptions = dataEntry.data.map(dataEntry.accessor) as FilterOption[];
        }
          ...
          case 'status':
            return (
              <FilterGroup key="status">
                <StatusSelector
                  value={state.filters[filterConfig.key] as string}
