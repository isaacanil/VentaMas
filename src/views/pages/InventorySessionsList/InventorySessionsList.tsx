...
export default function InventorySessionsList() {
  ...
  const formatUserDisplay = useCallback((raw) => {
    ...
    if (val.length > 18) {
      return `${val.slice(0, 6)}Ã¢â‚¬Â¦${val.slice(-4)}`;
    }
    return val;
  }, []);
  ...
  // Cargar sesiones (sin listeners por subcolecciÃƒÂ³n de editingUsers)
  useEffect(() => {
    if (!user?.businessID) {
      setSessions([]);
      setLoading(false);
  ...
  // Cargar editores (usuarios que actualizaron algÃƒÂºn conteo) por sesiÃƒÂ³n (lazy, sÃƒÂ³lo una vez por sesiÃƒÂ³n)
  useEffect(() => {
  ...
  const _fetchUserDisplayNameLegacy = async (uid, businessID) => {
    // ÃƒÅ¡nica ruta confirmada: users/{uid}. Se dejan otras rutas como fallback por si existen en algunos tenants.
    const tryPaths = [
      ['businesses', businessID, 'users', uid],
      ['businesses', businessID, 'staff', uid],
      ['users', uid],
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ value, row }) => (
        <EditorsList editors={value} loading={row?.original?.editorsLoading} />
      ),
    },
    {
      Header: 'Estado',
    ...
    {
      Header: 'Fecha de CreaciÃƒÂ³n',
      accessor: 'createdAt',
      minWidth: '150px',
      maxWidth: '1fr',
      cell: ({ value }) =>
        value ? DateUtils.convertMillisToISODate(value?.seconds * 1000) : 'N/A',
    },
    {
      Header: 'Creado por',
      ...
      <Component>
        <Header>
          ...
          <Space>
            <Button
              ...
              title={
                openSessionId
                  ? 'Ya existe un inventario abierto. Haz clic para ir a ÃƒÂ©l.'
        ...
        <TableContainer>
          <AdvancedTable
            ...
            emptyText="No hay inventarios aÃƒÂºn"
...
// Lista de editores (mÃƒ¡x 3 visibles + overflow)
...
function EditorsList({ editors, loading }) {
  if (loading) return <LoadingInline>CargandoÃ¢â‚¬Â¦</LoadingInline>;
  if (!editors || !editors.length)
    return <span style={{ color: '#6b7280' }}>Ã¢â‚¬â€</span>;
