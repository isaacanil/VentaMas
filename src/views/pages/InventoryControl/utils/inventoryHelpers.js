...
export function buildLocations(items, locationNamesMap = {}) {
  ...
  return Array.from(map.entries())
    ...
    .map(([locKey, quantity]) => ({
      location: locationNamesMap[locKey] || locKey,
      quantity,
    }));
}
...
export async function resolveLocationLabel(businessID, locKey) {
  try {
    ...
    if (warehouseId) {
      const name = await fetchFriendlyName(businessID, [
        'warehouses',
        warehouseId,
      ]);
    ...
    if (shelfId) {
      let name = await fetchFriendlyName(businessID, [
        'warehouses',
        warehouseId,
        'shelves',
        shelfId,
      ...
      if (!name)
        name = await fetchFriendlyName(businessID, ['shelves', shelfId]);
    ...
    if (segmentId) {
      ...
      if (!name)
        name = await fetchFriendlyName(businessID, ['segments', segmentId]);
