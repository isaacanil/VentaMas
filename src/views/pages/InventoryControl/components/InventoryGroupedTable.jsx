...
export default function InventoryGroupedTable({ 
    ...
    groups.forEach(g => {
      (g._children || []).forEach(ch => {
        ...
        if (key && expBase[key] === undefined) {
          const metaVal = countsMeta[key]?.manualExpirationDate;
            if (metaVal && metaVal !== CLEAR_SENTINEL) expBase[key] = norm(metaVal); else if (ch.type === 'batch' && ch.expirationDate) expBase[key] = norm(formatInputDate(ch.expirationDate)); else expBase[key] = '';
        }
        if (Array.isArray(ch.sources)) {
          ch.sources.forEach(src => {
            const skey = src.id || src.key; if (!skey) return;
            if (countsBase[skey] === undefined) {
              const persisted = (serverCounts[skey] !== undefined)
            ...
            if (expBase[skey] === undefined) {
              ...
              if (sMetaVal && sMetaVal !== CLEAR_SENTINEL) expBase[skey] = norm(sMetaVal); else if (src.expirationDate) expBase[skey] = norm(formatInputDate(src.expirationDate)); else if (ch.type==='batch' && ch.expirationDate) expBase[skey] = norm(formatInputDate(ch.expirationDate)); else expBase[skey] = '';
            }
          });
        }
      });
      ...
      if (g.topKey) {
        ...
        if (countsBase[g.topKey] === undefined) {
          ...
          const persistedTop = (serverCounts[g.topKey] !== undefined)
        ...
        if (expBase[g.topKey] === undefined) {
          ...
          else if (firstChild.type === 'batch' && firstChild.expirationDate) expBase[g.topKey] = norm(formatInputDate(firstChild.expirationDate));
  ...
  const columns = useMemo(() => ([
    { Header: 'Producto', accessor: 'productNameNode', sortable: true, align: 'left', minWidth: '220px', maxWidth: '1.3fr',
      cell: ({ value }) => value,
      // Usar el nombre real para ordenamiento alfabético
      sortableValue: (val, row) => row?.productName?.toLowerCase?.() || ''
    },
    { Header: 'Vencimiento', accessor: 'expirationNode', sortable: true, align: 'left', minWidth: '140px', maxWidth: '1fr', cell: ({ value }) => value,
      sortableValue: (val, row) => {
        // Use hidden sort value computed per-row
        return row?.expirationSortValue || ''
      }
    },
    { Header: 'Ubicaciones', accessor: 'locationsNode', align: 'left', minWidth: '220px', maxWidth: '1.4fr', cell: ({ value }) => value },
    { Header: 'Stock', accessor: 'totalStock', align: 'right', minWidth: '110px', maxWidth: '0.6fr', cell: ({ value }) => <strong>{formatNumber(value)}</strong> },
    { Header: 'Conteo real', accessor: 'conteoNode', align: 'right', minWidth: '130px', maxWidth: '0.7fr', clickable: false, cell: ({ value }) => value },
    { Header: 'Diferencia', accessor: 'diffNode', align: 'right', minWidth: '130px', maxWidth: '0.7fr', cell: ({ value }) => value },
    { Header: 'Editado por', accessor: 'userNode', align: 'left', minWidth: '180px', maxWidth: '1.2fr', clickable: false, cell: ({ value }) => value },
    { Header: 'Acción', accessor: 'actionsNode', align: 'right', minWidth: '70px', maxWidth: '0.5fr', clickable: false, cell: ({ value }) => value },
  ]), []);
  ...
  const rows = useMemo(() => {
    ...
    return groups.map(g => {
      ...
      const underlyingLotsCount = (g._children || []).reduce((acc, ch) => {
        if (Array.isArray(ch.sources) && ch.sources.length > 0) return acc + ch.sources.length
        return acc + 1
      }, 0)
      ...
      const hasSourceEdits = (g._children || []).some(ch => (ch.sources || []).some(src => {
        ...
        return (g._children || []).reduce((acc, ch) => {
          if (Array.isArray(ch.sources) && ch.sources.length) {
            ...
            const srcSum = ch.sources.reduce((s, src) => {
              const skey = src.id || src.key
              const val = counts[skey] ?? serverCounts[skey] ?? src.real ?? src.stock ?? src.quantity ?? 0
              return s + Number(val || 0)
            }, 0)
            return acc + srcSum
          }
          const childVal = counts[ch.key] ?? serverCounts[ch.key] ?? ch.real ?? ch.stock ?? 0
      ...
      if (hasSourceEdits || hasMultipleUnderlyingLots) {
        ...
        diffComputed = Number(computedSummedReal || 0) - Number(g.totalStock || 0)
      } else {
	...
	let diffEl = <Diff $value={diffComputed}>{formatNumber(diffComputed)}</Diff>
  ...
  let expirationSortValue = ''
      ...
      for (const child of (g._children || [])) {
        ...
        const persistedCount = (serverCounts[childKey] !== undefined)
          ? Number(serverCounts[childKey])
          : Number(child.stock ?? 0);
        const effectiveCount = counts[childKey] !== undefined ? counts[childKey] : persistedCount;
        const baseCount = baselineRef.current.counts[childKey];
        if (baseCount !== undefined && Number(effectiveCount) !== Number(baseCount)) { isModified = true; break; }
        // Pending (counts vs persistido actual)
        if (Object.prototype.hasOwnProperty.call(counts, childKey) && Number(counts[childKey]) !== Number(persistedCount)) { isModified = true; break; }
        ...
        if (expirationEdits[childKey] !== undefined) { isModified = true; break; }
        if (child.sources) {
          for (const src of child.sources) {
            const skey = src.id || src.key; if (!skey) continue;
            const srcPersisted = (serverCounts[skey] !== undefined)
              ? Number(serverCounts[skey])
              : Number(src.stock ?? src.quantity ?? 0);
            const srcEffective = counts[skey] !== undefined ? counts[skey] : srcPersisted;
            const baseSrcCount = baselineRef.current.counts[skey];
            if (baseSrcCount !== undefined && Number(srcEffective) !== Number(baseSrcCount)) { isModified = true; break; }
      ...
      if (!isModified && g.canEditAtTop && g.topKey) {
        const firstChild = (g._children || [])[0] || {};
        const originalTop = firstChild.type === 'batch' ? formatInputDate(firstChild.expirationDate) : '';
      ...
      if (!isModified && g.canEditAtTop && g.topKey && Object.prototype.hasOwnProperty.call(counts, g.topKey)) {
        const firstChild = (g._children || [])[0];
        const persistedTop = getPersistedCount(g.topKey, firstChild);
        if (Number(counts[g.topKey]) !== Number(persistedTop)) isModified = true;
      }
      ...
      const productNameNode = (
        <Tooltip title={productName} placement="topLeft">
          <ProductNameWrapper>
            ...
            {isModified && <Tag color="orange" size="small" style={{ marginTop: 4 }}>Editado</Tag>}
          </ProductNameWrapper>
        </Tooltip>
      )
        ...
        locationsNode = (
          <TagsWrap>
            {shown.map(([loc,q]) => {
              ...
              const isLoadingLoc = !locationNamesMap[loc] && !!resolvingLocations[loc]
              return (
                <Tooltip key={loc} title={`${label} (${formatNumber(q)})`}>
                  <Tag>
                    {isLoadingLoc ? <span style={{ opacity:.6, fontSize:11 }}>Cargando…</span> : shortenLocationPath(label)}
                  </Tag>
                </Tooltip>
              )
            })}
            {extra > 0 && (
              <Tooltip title={<div>{entries.slice(MAX_SHOW).map(([loc,q])=> <div key={loc}>{loc} ({formatNumber(q)})</div>)}</div>}>
                <Tag>+{extra}</Tag>
              </Tooltip>
            )}
          </TagsWrap>
      ...
      if (g.canEditAtTop && g.topKey) {
        ...
        const value = (() => {
          if (hasSourceEdits && Array.isArray(child.sources) && child.sources.length) {
            return child.sources.reduce((s, src) => s + Number(counts[src.id || src.key] ?? serverCounts[src.id || src.key] ?? src.real ?? src.stock ?? 0), 0)
          }
        ...
        const pushIfPending = (key, persisted) => {
          if (Object.prototype.hasOwnProperty.call(counts, key)) {
            ...
            if (Number(current) !== Number(persisted)) pendingCountPairs.push([key, persisted]);
          }
        };
        // top agregado (si aplica)
        {
          const currentPersistedTop = (serverCounts[g.topKey] !== undefined)
            ? Number(serverCounts[g.topKey])
            : Number(child.stock ?? 0);
          const baselineTop = baselineRef.current.counts[g.topKey];
          if (baselineTop !== undefined && Number(currentPersistedTop) !== Number(baselineTop)) {
        ...
        // children + sources
        (g._children || []).forEach(ch => {
          const currentPersistedChild = (serverCounts[ch.key] !== undefined)
            ? Number(serverCounts[ch.key])
            : Number(ch.stock ?? 0);
          const baselineChild = baselineRef.current.counts[ch.key];
          if (baselineChild !== undefined && Number(currentPersistedChild) !== Number(baselineChild)) {
          ...
          (ch.sources || []).forEach(src => {
            const skey = src.id || src.key;
            if (!skey) return;
            const currentPersistedSrc = (serverCounts[skey] !== undefined)
              ? Number(serverCounts[skey])
              : Number(src.stock ?? src.quantity ?? 0);
            const baselineSrc = baselineRef.current.counts[skey];
            if (baselineSrc !== undefined && Number(currentPersistedSrc) !== Number(baselineSrc)) {
        ...
        if ((child.type === 'batch' || child.type === 'noexp') && !readOnly) {
          ...
          const hasEditState = Object.prototype.hasOwnProperty.call(expirationEdits, key);
          const manualMetaVal = countsMeta[key]?.manualExpirationDate;
          const manualVal = (manualMetaVal && manualMetaVal !== CLEAR_SENTINEL) ? formatInputDate(manualMetaVal) : '';
          const originalVal = child.type === 'batch' ? formatInputDate(child.expirationDate) : '';
          const isRemoved = editVal === CLEAR_SENTINEL || manualMetaVal === CLEAR_SENTINEL;
          ...
          expirationNode = (
            <DatePicker
              ...
              onChange={(date) => {
                if (!date) {
                  // intento de limpiar
                  const hadExisting = !!originalVal || !!manualVal || !!currentStr;
                  if (!hadExisting) {
                  ...
                  if (child.type === 'batch' && hasOriginal) {
                    ...
                    Modal.confirm({
                      ...
                      content: 'La fecha se quitará en la sesión (no altera aún el lote real). ¿Continuar?',
                      okText: 'Sí, eliminar',
                      cancelText: 'Cancelar',
                      okButtonProps: { danger: true },
                      onOk: () => {
                        // Confirmar: marcar para eliminar
                        onChangeExpiration && onChangeExpiration(key, CLEAR_SENTINEL);
                      },
                      onCancel: () => {
                        // Cancelar: restaurar fecha previa
                        if (prevVal) onChangeExpiration && onChangeExpiration(key, prevVal);
                      }
                    });
                  } else {
                    onChangeExpiration && onChangeExpiration(key, undefined);
          ...
          const isDateDifferent = isRemoved || (currentStr && currentStr !== originalVal);
          ...
          if (canShowRestoreDate) {
            actions.push({
              ...
              onClick: () => onChangeExpiration && onChangeExpiration(key, originalVal)
            });
          }
          // --- Restablecer conteo real --- (prioriza pendientes)
          const restorePairs = pendingCountPairs.length ? pendingCountPairs : editedPairs;
          ...
          const groupManual = countsMeta[g.topKey]?.manualExpirationDate || countsMeta[child.key]?.manualExpirationDate
          const anySourceManual = Array.isArray(child.sources) && child.sources.some(src => {
            const skey = src.id || src.key
            return !!countsMeta[skey]?.manualExpirationDate
          })
          const isManual = !!(groupManual || anySourceManual)
          const effectiveStr = groupManual ? formatInputDate(groupManual) : formatInputDate(child.expirationDate)
          expirationSortValue = effectiveStr || ''
          const displayDate = effectiveStr || formatInputDate(child.expirationDate)
          expirationNode = (
            <span>
              {displayDate ? formatDate(displayDate) : <span style={{ opacity:.6 }}>-</span>}{isManual && <Tag color="gold" style={{ marginLeft:4 }}>Manual</Tag>}
            </span>
          )
        ...
        if (editorsMap.size) {
          const editors = Array.from(editorsMap.values()).sort((a, b) => (
            (getTsMs(b.updatedAt) - getTsMs(a.updatedAt))
          ));
          ...
          userEl = anyLoading ? <span style={{ fontSize:11, color:'#6b7280' }}>Cargando…</span> : <EditorsList editors={editors} />;
        }
      }
      ...
      if (hasMultipleLots) {
        ...
        (g._children || []).forEach(ch => {
          ...
          if (Object.prototype.hasOwnProperty.call(counts, ch.key) && Number(counts[ch.key]) !== Number(currentPersistedChild)) {
          ...
          (ch.sources || []).forEach(src => {
            ...
            if (Object.prototype.hasOwnProperty.call(counts, skey) && Number(counts[skey]) !== Number(currentPersistedSrc)) {
        ...
        (g._children || []).forEach(ch => {
          // Nivel hijo (lote agrupado)
            if (ch.type === 'batch' && ch.expirationDate) {
              ...
              const metaVal = countsMeta[key]?.manualExpirationDate;
              const editVal = expirationEdits[key];
              const isRemoved = editVal === CLEAR_SENTINEL || metaVal === CLEAR_SENTINEL;
              const currentStr = isRemoved ? CLEAR_SENTINEL : (norm(editVal) || norm(metaVal) || originalStr);
              const changed = (currentStr === CLEAR_SENTINEL) || (!!currentStr && currentStr !== originalStr);
            ...
            (ch.sources || []).forEach(src => {
              ...
              const sRemoved = sEditVal === CLEAR_SENTINEL || sMetaVal === CLEAR_SENTINEL;
              const currentStr = sRemoved ? CLEAR_SENTINEL : (norm(sEditVal) || norm(sMetaVal) || originalStr);
              const changed = (currentStr === CLEAR_SENTINEL) || (!!currentStr && currentStr !== originalStr);
        ...
        const restorePairs = pendingCountPairs.length ? pendingCountPairs : editedPairs;
        const menuItems = [
          { key: 'open-modal', label: 'Editar lotes', onClick: () => setModalGroup(g) },
          ...((editedPairs.length || dateResetPairs.length) ? [{ type: 'divider' }] : []),
          {
            key: 'reset-counts',
            label: 'Restablecer conteo real',
            disabled: restorePairs.length === 0,
            onClick: () => restorePairs.forEach(([k, v]) => onChangeCount(k, Number(v)))
          },
          {
            key: 'reset-dates',
            label: 'Restablecer fecha de vencimiento original',
            disabled: dateResetPairs.length === 0,
            onClick: () => dateResetPairs.forEach(([k, orig]) => onChangeExpiration && onChangeExpiration(k, orig))
          }
        ].filter(Boolean);
        ...
        if (editorsMap.size) {
          const editors = Array.from(editorsMap.values()).sort((a, b) => getTsMs(b.updatedAt) - getTsMs(a.updatedAt));
        ...
        ;(g._children || []).forEach(c => {
          ...
          if (Array.isArray(c.sources)) {
            for (const src of c.sources) {
              ...
              if (sMan) { const s = formatInputDate(sMan); if (s) expirations.add(formatDate(s)); added = true; break }
            }
          }
        ...
        if (expirations.size) {
          ...
          expirationNode = (
            <Tooltip title={list.map(d=> <div key={d}>{d}</div>)}>
              <span>{preview}{rest.length ? `, +${rest.length}` : ''}</span>
            </Tooltip>
          )
  ...
  }, [groups, counts, onChangeCount, countsMeta, usersNameCache, readOnly, expirationEdits, onChangeExpiration, resolvingUIDs, serverCounts])
        ...
        onRowClick={(row) => {
          try {
            ...
            for (const ch of row._children) {
              if (Array.isArray(ch.sources) && ch.sources.length > 0) lotes += ch.sources.length
              else lotes += 1
