...
export async function migrateAllBusinessesInventoryCounts(db, options = {}) {
  ...
  for (const bid of bizIds) {
    ...
    log(`[business] ${bid}`)
    try {
      const sessionsSnap = await getDocs(collection(db, 'businesses', bid, 'inventorySessions'))
      const sessionIds = sessionsSnap.docs.map(d => d.id)
      for (const sid of sessionIds) {
        summary.sessions++
        log(`  [session] ${sid}`)
        const countsSnap = await getDocs(collection(db, 'businesses', bid, 'inventorySessions', sid, 'counts'))
        ...
        for (const docSnap of countsSnap.docs) {
          ...
          const systemStock = (data.systemStock !== undefined)
            ? Number(data.systemStock)
            : (data.stockSistema !== undefined ? Number(data.stockSistema) : undefined)
          const realCount = (data.realCount !== undefined)
            ? Number(data.realCount)
            : (data.conteoReal !== undefined ? Number(data.conteoReal) : undefined)
          let difference = (data.difference !== undefined)
            ? Number(data.difference)
            : (data.diferencia !== undefined ? Number(data.diferencia) : undefined)

          // If not present, compute difference when possible
          if ((difference === undefined || Number.isNaN(difference)) && (realCount !== undefined) && (systemStock !== undefined)) {
            difference = Number(realCount) - Number(systemStock)
          }

          const ref = doc(db, 'businesses', bid, 'inventorySessions', sid, 'counts', id)

          if (!dryRun) {
            const update = {}
            if (systemStock !== undefined && !Number.isNaN(systemStock)) update.systemStock = Number(systemStock)
            if (realCount !== undefined && !Number.isNaN(realCount)) update.realCount = Number(realCount)
            if (difference !== undefined && !Number.isNaN(difference)) update.difference = Number(difference)
            // Delete legacy keys
            ...
            summary.docsUpdated++
            let migrated = 0
            if (Object.prototype.hasOwnProperty.call(data, 'stockSistema')) migrated++
            if (Object.prototype.hasOwnProperty.call(data, 'conteoReal')) migrated++
            if (Object.prototype.hasOwnProperty.call(data, 'diferencia')) migrated++
            summary.fieldsMigrated += migrated
          }

          if (pending >= 400) {
            try { await batch.commit() } catch (e) { summary.errors++; log(`    [error] commit: ${e?.message || e}`) }
            batch = writeBatch(db)
            pending = 0
          }
        }
  ...
  log(`[done] businesses=${summary.businesses} sessions=${summary.sessions} docsScanned=${summary.docsScanned} updated=${summary.docsUpdated} fieldsMigrated=${summary.fieldsMigrated} errors=${summary.errors}`)
  return summary
}
