...
export const BatchInfoModal = ({ isOpen, onClose, product }) => {
    const batchInfo = product?.batchInfo ?? null;
    const batchNumber = batchInfo?.batchNumber ?? product?.batchNumberId ?? product?.batchId ?? 'N/A';
    const quantity = batchInfo?.quantity ?? product?.stock ?? 'N/A';
    const expirationDate = batchInfo?.expirationDate ?? product?.expirationDate ?? null;
    const locationId = batchInfo?.locationId ?? product?.locationId ?? product?.location ?? null;
    const hasBatchData = Boolean(batchInfo || product?.batchId || product?.productStockId);
    ...
    const isStoredPlaceholder = storedLocationName === 'Cargando...' || storedLocationName === 'N/A';
    const locationLabel = locationNameInState
        || (!isStoredPlaceholder && storedLocationName)
        || null;
    ...
    }, [fetchLocationName, isOpen, isStoredPlaceholder, locationId, locationNameInState, storedLocationName]);
