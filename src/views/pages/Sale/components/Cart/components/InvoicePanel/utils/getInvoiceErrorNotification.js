const DEFAULT_ERROR_NOTIFICATION = {
    ...
    description: 'Intenta nuevamente o contacta a tu supervisor si el problema persiste.',
    duration: 6,
};
...
export const getInvoiceErrorNotification = (error) => {
    ...
    const normalizedCode = typeof error?.code === 'string' ? error.code.replace(/^functions\//, '') : null;

    if (error?.name === 'AbortError') {
        return {
            message: 'Consulta cancelada',
            description: 'La verificación del estado de la factura se canceló antes de completarse.',
            duration: 4,
        };
    }
    ...
    if (normalizedCode === 'invoice-timeout') {
        return {
            ...
            description: 'No recibimos confirmación del backend a tiempo. Verifica el historial de facturación antes de intentar nuevamente.',
            duration: 8,
        };
    }
    ...
    if (normalizedCode === 'invoice-failed') {
        ...
        const taskLabel = failedTask?.type ? `Tarea fallida: ${failedTask.type}.` : fallback;
        const detail = typeof failedTask?.lastError === 'string' ? failedTask.lastError : null;
        return {
    ...
    if (normalizedCode && normalizedCode.startsWith('cashCount')) {
        return {
            ...
            description: error?.message || 'Debes abrir un cuadre de caja antes de facturar.',
            duration: 6,
        };
    }
