...
/**
 * Calcula las fechas de pago y la pr?xima fecha de pago basado en la frecuencia y n?mero de cuotas
 * Esta es una funci?n normal, no un hook de React
 *
 * @param {string} frequency - Frecuencia de pago ('monthly', 'weekly', 'annual', etc)
 * @param {number} installments - N?mero de cuotas
...
const calculatePaymentDates = (
  ...
  // Determinar intervalo seg?n frecuencia
  const getInterval = (
  ...
  for (let i = 0; i < installments; i++) {
    dates.push(
      startDate
        .plus({ ...interval, [intervalKey]: i + 1 } as DurationLikeObject)
        .toMillis(),
    );
  }

  // Encontrar la pr?xima fecha de pago (a partir de hoy)
 ...
 */
const formatPaymentDate = (timestamp: number, format = 'dd/MM/yyyy'): string => {
  if (!timestamp) return '';
 ...
 */
const getFormattedDates = (dates: number[], format = 'dd/MM/yyyy'): string[] => {
