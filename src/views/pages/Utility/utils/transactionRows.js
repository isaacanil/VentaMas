...
export const buildTransactionRows = (dailyMetrics) => {
    ...
    const rows = filtered.map((day, index) => {
        ...
        return {
            id: day.isoDate,
            dateLabel: DateTime.fromMillis(day.timestamp).setLocale('es').toFormat('dd LLL yyyy'),
    ...
    const hasData = rows.some((row) => row.totalSales !== 0 || row.totalCost !== 0 || row.netProfit !== 0 || row.taxes !== 0);
    return hasData ? rows : [];
};
