...
export const computePreviousRange = (range, presetKey) => {
    ...
    const totalDurationMillis = Math.max(0, endDateTime.toMillis() - startDateTime.toMillis());
    const rawProgressMillis = Math.max(0, progressEnd.toMillis() - startDateTime.toMillis());
    const progressMillis =
        presetKey === 'custom' ? totalDurationMillis : Math.min(rawProgressMillis, totalDurationMillis);

    const buildRange = (startDateTime, endDateTime) => {
    ...
    const computeWithProgress = (previousStart, previousLimit) => {
        const endCandidate = previousStart.plus({ milliseconds: progressMillis });
        const boundedEnd = endCandidate.toMillis() > previousLimit.toMillis() ? previousLimit : endCandidate;
    ...
    if (presetKey === 'thisWeek') {
        const previousStart = startDay.minus({ weeks: 1 }).startOf('day');
        const previousLimit = previousStart.plus({ weeks: 1 }).minus({ milliseconds: 1 });
        return computeWithProgress(previousStart, previousLimit);
    }
