...
export const buildBusinessInsights = ({ summary, dailyMetrics, formatCurrency }) => {
    ...
    if (Number.isFinite(expenseRatio)) {
        if (expenseRatio >= 35) {
            insights.push({
                ...
                description: 'Los gastos superan el 35% de las ventas. Revisa consumos y contratos.',
                value: formatPercent(expenseRatio),
                measurement: 'Gastos operativos sobre ventas = (gastos totales / ventas totales) * 100',
            });
        } else {
            insights.push({
    ...
    if (Number.isFinite(taxRatio) && taxRatio >= 18) {
        insights.push({
            ...
            measurement: 'Carga impositiva = (impuestos totales / ventas totales) * 100',
        });
    }
    ...
    if (negativeDays.length) {
        insights.push({
            ...
            description: 'Analiza qué factores provocaron pérdidas puntuales y aplica correcciones.',
            value: negativeDays
                .slice(0, 3)
                .map((day) => day.dateLabel)
                .join(', '),
            measurement: 'Conteo de días con utilidad neta negativa dentro del período analizado.',
            meta: {
    ...
    if (bestDay && (bestDay.netProfit ?? 0) > 0) {
        insights.push({
            ...
            measurement: 'Identificación del día con mayor utilidad neta dentro del rango seleccionado.',
            meta: {
