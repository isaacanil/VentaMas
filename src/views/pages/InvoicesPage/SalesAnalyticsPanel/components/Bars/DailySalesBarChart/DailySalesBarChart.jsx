...
export const DailySalesBarChart = ({ sales }) => {
    ...
    const data = useMemo(() => {
        ...
        const labels = sortedDateKeys.map(dateKey => salesByDay[dateKey].displayDate);
        const dataTotals = sortedDateKeys.map(dateKey => salesByDay[dateKey].total);
        ...
        const dataTransfer = sortedDateKeys.map(dateKey => salesByDay[dateKey].transfer);
        const dataCreditNote = sortedDateKeys.map(dateKey => salesByDay[dateKey].creditNote);
    ...
    const handleNextPeriod = () => {
        setCurrentPeriodIndex(prev => Math.min(availablePeriods.length - 1, prev + 1));
    };
