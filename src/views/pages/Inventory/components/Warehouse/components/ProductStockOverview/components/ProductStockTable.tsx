  ...
  const tableData = useMemo<ProductStockRow[]>(
    () =>
      stocks.map((stock, index) => {
        const batchKey = stock.batchId != null ? String(stock.batchId) : 'stock';
        const rowId =
  ...
  const columns = useMemo<AdvancedTableColumn<ProductStockRow>[]>(
    () => [
      ...
      },
      {
        ...
        sortable: true,
        cell: ({ value }) => (
          <BatchBadge $empty={!value}>{value ? `#${value}` : 'Sin lote'}</BatchBadge>
        ),
      },
      {
        Header: 'Vencimiento',
        ...
        cell: ({ value }) => (
          <ExpirationBadge $empty={!value}>
            {value ? new Date(value as number).toLocaleDateString() : 'Sin fecha'}
          </ExpirationBadge>
        ),
      },
      {
