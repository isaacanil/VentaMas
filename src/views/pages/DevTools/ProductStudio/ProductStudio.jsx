...
export default function ProductStudio() {
  ...
  const { scrollContainerRef, activeSection, handleSectionNavigation } = useSectionNavigation();
  ...
  const updateUrlProductId = useCallback((nextId) => {
  ...
  const resetToCreateState = useCallback(({ skipUrlUpdate = false } = {}) => {
  ...
  const loadProductById = useCallback(async (rawId, { skipUrlUpdate = false, quiet = false } = {}) => {
    ...
    try {
      ...
      dispatch(ChangeProductData({ status: 'update', product: existingProduct }));
      if (!quiet) {
        message.success(`Producto "${existingProduct.name || existingProduct.productName}" listo para editar.`);
      }
      if (!skipUrlUpdate) {
  ...
  useEffect(() => {
    if (productIdFromParams) {
      if (productIdFromParams !== lastLoadedProductIdRef.current) {
        loadProductById(productIdFromParams, { skipUrlUpdate: true, quiet: true });
      }
      return;
    }
    if (!productIdFromParams && lastLoadedProductIdRef.current) {
  ...
  const handleValuesChange = useCallback((changedValues) => {
    ...
    if (key === 'pricing') {
      ...
      if (normalizedPricing?.tax !== undefined) {
        ...
        normalizedPricing.tax = typeof t === 'string' ? parseFloat(t) || 0 : Number(t);
      }
    ...
    if (key === 'weightDetail') {
      dispatch(ChangeProductData({ product: { weightDetail: { ...product?.weightDetail, ...value } } }));
      return;
    }

    if (key === 'warranty') {
      dispatch(ChangeProductData({ product: { warranty: { ...product?.warranty, ...value } } }));
      return;
    }
    ...
    try {
      ...
      if (isUpdateMode || status === 'update') {
        ...
        notification.success({
          ...
          description: 'Los cambios están disponibles para el equipo en segundos.',
        });
      } else {
        await fbAddProduct(sanitizedProduct, user);
        notification.success({
          message: 'Producto creado',
          description: 'El nuevo producto ya está disponible para ventas e inventario.',
        });
        resetToCreateState();
      }
    } catch (err) {
      if (err?.errorFields?.length) {
        err.errorFields.forEach((error) => {
          notification.error({
            ...
            description: error.errors?.[0] || 'Hay valores pendientes por completar.',
            duration: 10,
          });
        });
        return;
        ...
        <ScrollArea ref={scrollContainerRef}>
          <StudioWrapper>
            ...
            />
              ...
              {summaryVisible && (
                <StickySummary>
                  <ProductSummary product={product} previewMetrics={previewMetrics} />
                </StickySummary>
              )}
            </StudioGrid>
            ...
            <Drawer
              ...
              bodyStyle={{ padding: 0 }}
            >
              <ImageManager hideImageManager={() => setImageDrawerVisible(false)} />
            </Drawer>
          </StudioWrapper>
        </ScrollArea>
      </Workspace>
