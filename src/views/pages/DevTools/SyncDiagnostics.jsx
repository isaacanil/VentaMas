...
export default function SyncDiagnostics() {
  ...
  const columnsProducts = [
    ...
    { title: 'Declarado (product.stock)', dataIndex: 'declaredStock', key: 'declaredStock' },
    { title: 'Suma productsStock', dataIndex: 'stockFromProductsStock', key: 'stockFromProductsStock' },
    { title: 'Delta', dataIndex: 'delta', key: 'delta', render: (v) => <Tag color={v===0?'green':(v>0?'blue':'red')}>{v}</Tag> },
    {
        ...
        <Space>
          <Popconfirm
            ...
            onConfirm={async () => {
              ...
              try {
                setSyncing(true);
                await syncProductsStockFromProductsStock(businessID, { productIds: [row.productId], dryRun: false });
          ...
          >
            <Button size="small" loading={syncing} disabled={!businessID}>Sincronizar</Button>
          </Popconfirm>
        </Space>
      )
    }
  ...
  const columnsBatches = [
    ...
    { title: 'Declarado (batch.quantity)', dataIndex: 'declaredQuantity', key: 'declaredQuantity' },
    { title: 'Suma productsStock', dataIndex: 'stockFromProductsStock', key: 'stockFromProductsStock' },
    { title: 'Delta', dataIndex: 'delta', key: 'delta', render: (v) => <Tag color={v===0?'green':(v>0?'blue':'red')}>{v}</Tag> },
    { title: 'Producto existe', dataIndex: 'productMissing', key: 'productMissing', render: (m) => m ? <Tag color="red">Missing</Tag> : <Tag color="green">OK</Tag> },
  ];
  ...
  const columnsOrphans = [
    ...
    { title: 'Razón', dataIndex: 'reason', key: 'reason', render: (r) => <Tag color="red">{r}</Tag> },
  ];

  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      <Typography.Title level={3}>Diagnóstico de sincronización: Productos vs Batches vs ProductsStock</Typography.Title>
      <Typography.Paragraph type="secondary">
        Herramienta de desarrollo. Compara totales declarados con agregados reales y verifica integridad referencial.
      </Typography.Paragraph>

      {!businessID && <Alert type="warning" message="No hay businessID en sesión" showIcon />}

      <Card>
        <Space wrap>
          <Button type="primary" loading={loading} onClick={runDiagnostics} disabled={!businessID}>
            Ejecutar diagnóstico
          </Button>
          <Button
            onClick={async () => {
              if (!businessID) return;
              setSyncResult(null); setError(null);
              try {
                setSyncing(true);
                const res = await syncProductsStockFromProductsStock(businessID, { dryRun: true });
                setSyncResult({ mode: 'dry-run', ...res });
                message.info(`Dry-run: ${res.updates.length} productos quedarían actualizados`);
              } catch (e) {
                console.error(e);
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              ...
              try {
                setSyncing(true);
                const res = await syncProductsStockFromProductsStock(businessID, { dryRun: false });
      ...
      {summary && (
        <Card>
          <Row gutter={16}>
            <Col span={4}><Statistic title="Productos" value={summary.products} /></Col>
            <Col span={4}><Statistic title="Batches" value={summary.batches} /></Col>
            <Col span={4}><Statistic title="Stocks" value={summary.stocks} /></Col>
            <Col span={4}><Statistic title="Prod con mismatch" value={summary.productMismatches} valueStyle={{ color: summary.productMismatches? '#cf1322':'#3f8600' }} /></Col>
            <Col span={4}><Statistic title="Batches con mismatch" value={summary.batchMismatches} valueStyle={{ color: summary.batchMismatches? '#cf1322':'#3f8600' }} /></Col>
            <Col span={4}><Statistic title="Orphans" value={summary.orphans} valueStyle={{ color: summary.orphans? '#cf1322':'#3f8600' }} /></Col>
          </Row>
        </Card>
      )}

      <Card title={`Productos con diferencias (${productMismatches.length})`}>
        <Table size="small" rowKey={(r) => `${r.productId}`} dataSource={productMismatches} columns={columnsProducts} pagination={{ pageSize: 10 }} />
      </Card>

      <Card title={`Batches con diferencias o referencias inválidas (${batchMismatches.length})`}>
        <Table size="small" rowKey={(r) => `${r.batchId}`} dataSource={batchMismatches} columns={columnsBatches} pagination={{ pageSize: 10 }} />
      </Card>

      <Card title={`ProductsStock huérfanos (${orphanStocks.length})`}>
        <Table size="small" rowKey={(r) => `${r.stockId}`} dataSource={orphanStocks} columns={columnsOrphans} pagination={{ pageSize: 10 }} />
      </Card>
      ...
      {syncResult && (
        <Alert
          ...
          showIcon
          message={syncResult.mode === 'apply' ? 'Sincronización aplicada' : 'Resultado dry-run'}
