...
import { convertFirestoreTimestamps } from '@/firebase/utils/firestoreDates';
...
const resolveProvider = async (
  providerRef: OrderDocument['data']['provider'],
): Promise<OrderDocument['data']['provider']> => {
  if (!isDocumentReferenceLike(providerRef)) {
  ...
  try {
    ...
    return (providerDoc?.provider as OrderDocument['data']['provider']) ?? null;
  } catch (error) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(orderRef, (snapshot) => {
      ...
      const orderArray = snapshot.docs.map(async (item) => {
        ...
        if (orderData.data) {
          orderData.data.provider = await resolveProvider(
