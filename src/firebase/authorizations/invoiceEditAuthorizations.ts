...
const getLegacyColRef = (businessID: string) =>
  ...
  ) as CollectionReference<AuthorizationRequest, DocumentData>;
...
const normalizeModuleFromData = (data: Record<string, unknown> = {}) => {
  ...
  if (metadata && typeof metadata.module === 'string') return metadata.module;
...
type AuthorizationDocSnap = QueryDocumentSnapshot<
  AuthorizationRequest,
  DocumentData
>;

const mapDocToAuthorization = (
  docSnap: AuthorizationDocSnap,
  collectionKey: string,
): AuthorizationDoc => {
...
const resolveRequestDoc = async (
  ...
  ref: DocumentReference<AuthorizationRequest, DocumentData>;
  snap: DocumentSnapshot<AuthorizationRequest, DocumentData>;
  collectionKey: string;
} | null> => {
  const genericRef = doc(
    ...
    GENERIC_COLLECTION_NAME,
    requestId,
  ) as DocumentReference<AuthorizationRequest, DocumentData>;
...
const buildUserSnapshot = (
  ...
  uid: userLike?.uid ?? '',
  name: userLike?.name || userLike?.realName || '',
...
const validateInvoiceConstraints = async (
  ...
  const cashCountId =
    typeof invoice?.cashCountId === 'string'
      ? invoice.cashCountId
      : typeof invoice?.cashCountID === 'string'
        ? invoice.cashCountID
        : null;
...
export const expireIfNeeded = async (businessID: string, requestId: string) => {
  ...
  if (data && shouldExpirePending(data)) {
