...
export const getAccountReceivableById = async (businessId, arId) => {
    const accountRef = doc(db, 'businesses', businessId, 'accountsReceivable', arId);
...
export const getAllInstallmentsByArId = async (businessId, arId) => {
    const installmentsRef = collection(db, 'businesses', businessId, 'accountsReceivableInstallments');
 ...
 * @param {object} writer - El objeto de transacción o batch de Firestore.
 */
export const applyPaymentToInstallment = (writer, { user, installment, amountToApply, paymentId, clientId, arId }) => {
 ...
 * @param {object} writer - El objeto de transacción o batch de Firestore.
 */
export const updateAccountReceivableState = (writer, { businessId, arId, totalPaid, newArBalance, paidInstallmentIds, existingPaidInstallments = [] }) => {
    const arRef = doc(db, "businesses", businessId, "accountsReceivable", arId);
    const allPaidInstallments = [...new Set([...existingPaidInstallments, ...paidInstallmentIds])];
 ...
 * @param {object} writer - El objeto de transacción o batch de Firestore.
 */
export const updateInvoiceOnPayment = async (writer, { businessId, invoiceId, amountPaid }) => {
    ...
    const newTotalPaid = roundToTwoDecimals((invoiceData.totalPaid || 0) + amountPaid);
...
export const validatePaymentAmounts = (paymentDetails) => {
    ...
    const creditNoteTotal = creditNotePayment.reduce((sum, note) => 
        sum + (parseFloat(note.amountUsed || note.amountToUse || 0)), 0
    );
    ...
    const totalPaymentAndCredit = roundToTwoDecimals(totalPaidFloat + creditNoteTotal);
    if (totalPaymentAndCredit > paymentAmountFloat) {
        return { 
            isValid: false, 
            error: 'Total payment and credit notes exceed invoice amount',
            amounts: { paymentAmountFloat, totalPaidFloat, creditNoteTotal, totalPaymentAndCredit }
        };
    }
...
export const createAccountReceiptData = ({ 
    ...
    return {
        ...
        remainingInstallments: account.totalInstallments - (account.paidInstallments?.length || 0) - paidInstallments.length,
 ...
 */
export const safeNumber = (val) => typeof val === 'number' && !isNaN(val) ? val : 0;
 ...
 */
export const safeString = (val) => (val !== undefined && val !== null) ? String(val) : null;
 ...
 */
export const processInstallmentPayment = (writer, { 
    user, 
    installment, 
    remainingAmount, 
    paymentId, 
 ...
 */
export const updateInvoiceTotals = (writer, { 
    businessId, 
    invoiceId, 
    amountPaid, 
    invoice 
...
export const createFullPaymentReceipt = ({ 
    ...
    const receiptData = {
        ...
        accounts: Array.isArray(accounts) ? accounts.map(account => ({
            ...
            paidInstallments: Array.isArray(account.paidInstallments) ? account.paidInstallments : [],
    ...
    if (undefinedFields.length > 0) {
        ...
        throw new Error(`Payment receipt has undefined fields: ${undefinedFields.map(([key]) => key).join(', ')}`);
    }
 ...
 */
export const validateBasicPaymentParams = ({ user, clientId, totalPaid, paymentMethods }) => {
 ...
 * @param {Object} params - Parámetros de actualización
 */
export const updateAccountAfterPayment = (writer, { 
    user, 
    account, 
    totalPaid 
}) => {
...
export const extractCreditNoteInfo = (paymentDetails) => {
    ...
    const validCreditNotes = creditNotePayment.filter(note => 
        note && (note.amountUsed > 0 || note.amountToUse > 0)
    );
    
    const totalCreditAmount = validCreditNotes.reduce((sum, note) => 
        sum + (note.amountUsed || note.amountToUse || 0), 0
    );
...
export const adjustPaymentAmountsForCreditNotes = (paymentDetails) => {
    ...
    const effectiveAmountToPay = roundToTwoDecimals(baseInvoiceAmount - totalCreditAmount);
...
export const validateFirestoreData = (data, context = 'data') => {
    ...
    if (undefinedPath) {
        console.error(`❌ Undefined field found in ${context} at path: ${undefinedPath}`);
        console.error(`❌ Original data:`, data);
        console.error(`❌ Cleaned data:`, cleanedData);
        throw new Error(`Cannot save ${context} to Firestore: undefined field at ${undefinedPath}`);
    }
...
export const validateAccountHasPendingBalance = async (user, arId) => {
    try {
        ...
        const accountRef = doc(db, 'businesses', user.businessID, 'accountsReceivable', arId);
        const accountDoc = await getDoc(accountRef);
