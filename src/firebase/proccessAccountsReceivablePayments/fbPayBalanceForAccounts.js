...
export const fbPayBalanceForAccounts = async ({ user, paymentDetails }) => {
    ...
    try {
        ...
        for (let account of accounts) {
            ...
            const accountValidation = await validateAccountHasPendingBalance(user, account.id);
            
            if (!accountValidation.isValid) {
                console.warn(`⚠️ Skipping account ${account.id}: ${accountValidation.error}`);
                continue; // Continuar con la siguiente cuenta en lugar de fallar
            }

            console.log(`✅ Account ${account.id} has pending balance: ${accountValidation.balance}`);
            
            const accountInstallments = await getActiveInstallmentsByArId(user, account.id);
            ...
            for (let installment of accountInstallments) {
                ...
                accountTotalPaid = roundToTwoDecimals(accountTotalPaid + result.amountToApply);
                account.arBalance = roundToTwoDecimals(account.arBalance - result.amountToApply);
