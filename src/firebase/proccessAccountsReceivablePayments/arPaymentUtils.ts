...
import type { DocumentReference, Transaction, WriteBatch } from 'firebase/firestore';
...
export type FirestoreDoc<T extends FirestoreData = FirestoreData> = T & {
  id: string;
};
export type UserWithBusinessAndUid = UserWithBusiness & { uid: string };

const isWriteBatch = (writer: FirestoreWriter): writer is WriteBatch =>
  typeof (writer as WriteBatch).commit === 'function';
...
export type InvoiceLike =
  | (InvoiceFirestoreDoc & { data?: InvoiceData })
  | InvoiceData;
...
export const createPaymentRecord = (
  writer: FirestoreWriter,
  {
    user,
    paymentDetails,
  }: { user: UserWithBusiness; paymentDetails: PaymentDetails },
): string => {
  const {
    totalPaid,
    paymentMethods,
  ...
  writerSet(writer, paymentsRef, paymentData);
  return paymentId;
};
...
export const applyPaymentToInstallment = (
  ...
  writerUpdate(writer, installmentRef, {
  ...
  writerSet(writer, installmentPaymentRef, {
...
export const updateInvoiceOnPayment = async (
  ...
  writerUpdate(writer, invoiceRef, {
...
const safeStringOrNumber = (val: unknown): string | number | null =>
  typeof val === 'string' || typeof val === 'number' ? val : null;

const normalizeStringOrNumber = (val: string | number | null): string | null => {
  if (val === null || val === undefined) return null;
  const parsed = String(val).trim();
  return parsed.length > 0 ? parsed : null;
};

const getInvoiceDataFromInvoiceLike = (
  invoice?: InvoiceLike | null,
): InvoiceData | undefined =>
  invoice && typeof invoice === 'object' && 'data' in invoice
    ? (invoice.data as InvoiceData | undefined)
    : (invoice as InvoiceData | undefined);

export const resolveReceiptDocumentMetadata = ({
  account,
  invoice,
}: {
  ...
  documentLabel: 'PREVENTA' | 'FACTURA';
  documentNumber: string | null;
  documentType: 'preorder' | 'invoice';
  invoiceNumber: string;
} => {
  const invoiceData = getInvoiceDataFromInvoiceLike(invoice);
  const invoiceNumberRaw = safeStringOrNumber(
    invoiceData?.numberID ?? invoiceData?.number ?? account?.invoiceNumber,
  );
  const preorderNumberRaw = safeStringOrNumber(
    invoiceData?.preorderDetails?.numberID ??
      invoiceData?.preorderDetails?.number,
  );

  const isPreorderOrigin =
    account?.originType === 'preorder' || Boolean(account?.preorderId);
  const hasInvoiceNumber = Boolean(normalizeStringOrNumber(invoiceNumberRaw));
  const stillPreorder = invoiceData
    ? isPreorderDocument(invoiceData)
    : isPreorderOrigin && !hasInvoiceNumber;

  const documentNumberRaw = stillPreorder
    ? (preorderNumberRaw ?? invoiceNumberRaw)
    : (invoiceNumberRaw ?? preorderNumberRaw);

  return {
    documentLabel: stillPreorder ? 'PREVENTA' : 'FACTURA',
    documentNumber: normalizeStringOrNumber(documentNumberRaw),
    documentType: stillPreorder ? 'preorder' : 'invoice',
    invoiceNumber: normalizeStringOrNumber(invoiceNumberRaw) ?? 'N/A',
  };
};
...
export const createAccountReceiptData = ({
  ...
  return {
    ...
    remainingInstallments:
      totalInstallments - paidInstallmentsCount - paidInstallments.length,
    totalInstallments,
...
export const mergePaymentMethods = (
  ...
  const sanitizedExisting = Array.isArray(existingMethods)
    ? existingMethods.filter(isPaymentMethodWithKey).map((m) => ({
        ...m,
...
export const updateInvoiceTotals = (
  ...
  const invoiceRef = doc(db, 'businesses', businessId, 'invoices', invoiceId);
  const invoiceData = (
    'data' in invoice && invoice.data ? invoice.data : invoice
  ) as InvoiceDataWithTotals;

  const invoiceTotal = roundToTwoDecimals(
    Number(invoiceData.totalAmount ?? invoiceData.totalPurchase?.value ?? 0),
  );
  ...
  const newBalanceDue = roundToTwoDecimals(
    Math.max(invoiceTotal - newAccumulatedPaid, 0),
  );
  ...
  if (isPreorder) {
    ...
    writerUpdate(writer, invoiceRef, invoiceUpdate);
  ...
  const invoiceUpdate = {
    ...baseUpdate,
    // IMPORTANT: Do not touch lifecycle status here. AR payments only update payment-related fields.
    paymentStatus,
...
type PaymentReceiptAccount = {
  ...
  documentLabel?: string | null;
  documentNumber?: string | null;
  documentType?: 'preorder' | 'invoice' | null;
  invoiceId?: string | number | null;
  paidInstallments?: PaidInstallmentInfo[];
  remainingInstallments?: number;
  totalInstallments?: number;
  ...
  const receiptData = {
    ...
    accounts: Array.isArray(accounts)
      ? accounts.map((account) => ({
          ...
          documentLabel: safeString(account.documentLabel),
          documentNumber: safeString(account.documentNumber),
          documentType:
            account.documentType === 'preorder' ||
            account.documentType === 'invoice'
...
export const updateAccountAfterPayment = (
  ...
  writerUpdate(writer, accountRef, {
...
export const adjustPaymentAmountsForCreditNotes = (
  paymentDetails: Pick<
    PaymentDetails,
    'totalPaid' | 'totalAmount' | 'creditNotePayment'
  >,
): {
 ...
 */
export const validateFirestoreData = <T>(data: T, context = 'data'): T => {
