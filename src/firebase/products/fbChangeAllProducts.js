...
export async function fbUpdateProductsToNewFormatForMultipleBusinesses() {
  try {
    ...
    console.error(
      'Error actualizando productos para múltiples negocios:',
      error,
    );
  }
}
...
function convertDecimalToPercentage(valorDecimal) {
  if (
    ...
    console.log(
      'Valor decimal inesperado. Retornando 0 como valor por defecto.',
    );
...
export const newProductSchema = (products) => {
  ...
  if (!Array.isArray(products)) {
    console.error(
      'Los productos no son un arreglo. Abortando la actualización.',
    );
    return [];
  }
    ...
    return {
      ...
      pricing: {
        ...
        minPrice: product.minimumPrice
          ...
          : product.price.unit, // Usando el costo unitario si no hay precio mínimo
        avgPrice: product.averagePrice
          ? product.averagePrice
          : product.price.unit, // Usando el costo unitario si no hay precio promedio
