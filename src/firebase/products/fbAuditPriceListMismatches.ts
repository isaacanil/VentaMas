...
/* ───────────────────────────── Helpers comunes ───────────────────────────── */
const resolveProductName = (data: UnknownRecord | null | undefined): string => {
  const name = data?.name;
  if (typeof name === 'string' && name.trim()) return name;
  const title = data?.title;
  if (typeof title === 'string' && title.trim()) return title;
  const nested = (data as { product?: { name?: unknown } } | undefined)?.product
    ?.name;
...
const downloadBytes = (
  ...
  const blob = new Blob(Array.isArray(bytes) ? bytes : [bytes], {
    type: mime,
  });
...
export async function fbAuditAllBusinessesPriceVsList(
  ...
  // eslint_disable-next-line no-constant-condition
  while (true) {
