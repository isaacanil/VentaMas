...
const applyClientSideFilters = (invoices, filters) => {
  return invoices.filter(invoice => {
    ...
    if (filters.paymentMethod) {
      const hasPaymentMethod = invoiceData.paymentMethod?.some(method => 
  ...
  useEffect(() => {
    ...
    const fetchInvoices = async () => {
      try {
        ...
        if (filters.clientId) {
          queryConstraints.push(where("data.client.id", "==", filters.clientId));
        }
    ...
    fetchInvoices();
  }, [user?.businessID, filters.startDate, filters.endDate, filters.clientId, filters.paymentMethod, filters.minAmount, filters.maxAmount, user?.uid, user?.role]);
