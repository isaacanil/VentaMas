...
export async function getQuotation(
  ...
  try {
    ...
    if (quotationSnapshot.exists()) {
      ...
      quotationData.expired =
        expirationDate instanceof Date ? expirationDate < new Date() : false;
...
export async function addQuotation(
  ...
  try {
    console.info('[QuotationDebug][Service] addQuotation start', {
      userId: user?.uid ?? null,
      businessId: user?.businessID ?? null,
      hasQuotationData: Boolean(quotationData),
    });
    const id = nanoid();
    const quotationRef = doc(
      db,
    ...
    console.info('[QuotationDebug][Service] Quotation saved to Firestore', {
      quotationId: id,
      numberID: data.numberID,
    });

    const quotation = await getQuotation(user, id);
    console.info('[QuotationDebug][Service] Quotation reloaded from Firestore', {
      quotationId: id,
      numberID:
        typeof quotation === 'object' && quotation !== null
          ? (quotation as Record<string, unknown>).numberID ?? null
          : null,
    });
    ...
    console.error('[QuotationDebug][Service] addQuotation failed', {
