 ...
 */
export const fbConsumeCreditNotes = async (user, creditNotePayments, invoiceId, invoiceData = {}) => {
  ...
  try {
    ...
    const applicationDataList = await runTransaction(db, async (transaction) => {
      ...
      creditNoteSnapshots.forEach((snapshot, index) => {
        if (!snapshot.exists()) {
          throw new Error(`Nota de crédito ${creditNotePayments[index].id} no encontrada`);
        }
        
        const creditNoteData = snapshot.data();
        const currentAvailable = creditNoteData.availableAmount ?? creditNoteData.totalAmount;
        const amountToConsume = creditNotePayments[index].amountUsed;
        
        if (currentAvailable < amountToConsume) {
          throw new Error(`Saldo insuficiente en nota de crédito ${creditNoteData.ncf || creditNoteData.number}`);
        }
      });
      ...
      creditNoteSnapshots.forEach((snapshot, index) => {
        ...
        const updates = {
          ...
          status: newAvailable === 0 ? CREDIT_NOTE_STATUS.FULLY_USED : CREDIT_NOTE_STATUS.APPLIED
        };
    ...
    console.log(`✅ Consumidas ${creditNotePayments.length} notas de crédito para factura ${invoiceId}`);
    console.log(`✅ Creados ${applicationPromises.length} registros de aplicación`);
