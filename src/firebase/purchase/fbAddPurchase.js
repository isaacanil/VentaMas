...
export async function addPurchase({
  ...
  try {
    ...
    const updatedReplenishments =
      purchase.replenishments?.map((item) => ({
        ...item,
        expirationDate: item.expirationDate
          ? Timestamp.fromMillis(new Date(item.expirationDate).getTime())
          : null,
      })) || [];
...
export const fbAddPurchase = async (
  ...
  try {
    ...
    const updatedReplenishments = purchase.replenishments.map((item) => ({
      ...item,
      expirationDate: item.expirationDate
        ? Timestamp.fromMillis(item.expirationDate)
        : null,
    }));
