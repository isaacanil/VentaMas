...
const normalizeMillisInput = (
  value: ValueLike | NumberLike | null | undefined,
): number | null => {
  ...
  if (typeof (value as { seconds?: number }).seconds === 'number') {
    const nanos =
      typeof (value as { nanoseconds?: number }).nanoseconds === 'number'
        ...
        : 0;
    return (
      (value as { seconds: number }).seconds * 1000 + Math.floor(nanos / 1e6)
    );
  }
...
export const toMillis = (ts: TimestampLike): number | null =>
  normalizeMillisInput(ts);
