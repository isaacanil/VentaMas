...
export function analyzeBarcodeStructure(barcode) {
  ...
  switch (length) {
    ...
    case 13: {
      ...
      const gtin13CheckDigit = calculateGTIN13CheckDigit(
      ...
      if (!analysis.isValid) {
        // Solo actualizar si no fue marcado como válido anteriormente
        analysis.isValid = analysis.checkDigit.isValid;
      }
      break;
    }
    ...
    case 14: {
      ...
      const gtin14CheckDigit = calculateGTIN14CheckDigit(
        cleanCode.slice(0, 13),
      );
      analysis.checkDigit = {
