...
- `Pendiente por archivos raiz`: `auth-adapter.ts`, `files.ts`, `fiscalReceiptsUtils.ts`, `flowTrace.ts`, `formatDate.ts`, `formatNumber.ts`, `formatters.ts`, `formRules.ts`, `invoiceValidation.ts`, `lazyWithRetry.ts`, `lodash-minimal.ts`, `menuAccess.ts`, `printPdf.ts`, `reduxStateUtils.ts`, `referenceUtils.ts`, `sales.ts`, `text.ts`, `userValidation.ts`, `validators.ts`
