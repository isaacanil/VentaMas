...
function checkEnv() {
    ...
    if (!fs.existsSync(examplePath)) {
        console.warn(`⚠️  Warning: No ${ENV_EXAMPLE_FILE} found to compare against.`);
        console.log(`💡 Tip: Create a ${ENV_EXAMPLE_FILE} so the team knows which variables are required.`);
        return;
    }

    if (!fs.existsSync(envPath)) {
        console.error(`❌ Error: No ${ENV_FILE} file found.`);
        console.log(`👉 Please copy ${ENV_EXAMPLE_FILE} to ${ENV_FILE} and fill in your values.`);
        process.exit(1);
    }
    ...
    if (missingKeys.length > 0) {
        ...
        console.log(`✅ All ${exampleKeys.size} variables from ${ENV_EXAMPLE_FILE} are present in ${ENV_FILE}.`);
    }
