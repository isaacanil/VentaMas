...
if (!currentBranch) {
  console.error(
    `${colors.red}❌ Error: No se pudo determinar la rama actual.${colors.reset}`,
  );
  process.exit(1);
}

console.log(
  `${colors.green}👉 Rama actual detectada:${colors.reset} ${colors.yellow}${currentBranch}${colors.reset}`,
);
...
if (!remotes || !remotes.split('\n').includes('alt')) {
  ...
  console.log(
    `${colors.yellow}👉 Añádelo con:${colors.reset} git remote add alt <URL-del-repositorio>`,
  );
  process.exit(1);
}
...
console.log('');
rl.question(
  `⚠️  Esto eliminará TODAS las ramas remotas en 'alt' y subirá '${currentBranch}' como 'main'. ¿Continuar? (y/n): `,
  (answer) => {
    ...
    try {
      console.log('');
      console.log(
        `${colors.blue}🧹 Eliminando ramas remotas existentes en 'alt'...${colors.reset}`,
      );
      ...
      if (lsRemote) {
        ...
        for (const branch of branches) {
          console.log(
            `   🗑️  Eliminando rama remota: ${colors.yellow}${branch}${colors.reset}`,
          );
      ...
      console.log('');
      console.log(
        `${colors.blue}⬆️  Subiendo '${colors.yellow}${currentBranch}${colors.blue}' como 'main' al remote 'alt'...${colors.reset}`,
      );
      ...
      console.log(
        `${colors.green}   • '${colors.yellow}${currentBranch}${colors.green}' se subió como 'main' a 'alt'.${colors.reset}`,
      );
      console.log(
        `${colors.green}   • Todas las ramas remotas previas en 'alt' fueron eliminadas.${colors.reset}`,
      );
      console.log('');
      console.log(
        `${colors.blue}💡 Cuando Codex haya terminado, puedes traer los cambios así:${colors.reset}`,
      );
      ...
      console.error(
        `${colors.red}❌ Un error ocurrió durante el proceso:${colors.reset}`,
        err,
      );
    } finally {
      rl.close();
