...
function scanFile(file, content) {
  ...
  for (const { seq, char } of suspiciousSequences) {
    ...
    if (seq === 'Ãƒ' && suspiciousSequences.some(s => s.seq !== 'Ãƒ' && s.seq.startsWith('Ãƒ') && content.includes(s.seq))) {
      continue;
    }
  ...
  if (!isPackageLock) {
    for (const { pattern, fix } of questionMappings) {
      if (newContent.includes(pattern)) {
        ...
        if (fixMode) {
          ...
          issues.push({ sequence: pattern, char: fix, count: matches, fixed: true });
        } else {
          issues.push({ sequence: pattern, char: fix, count: matches, fixed: false });
        }
      }
    }
    ...
    if (matches) {
      matches.forEach((m) => {
        ...
        if (!issues.some(i => i.fixed && i.char.includes(m.replace('?', '')))) {
          issues.push({ sequence: m, char: 'Carácter roto (?)', count: 1, fixed: false });
        }
      });
    }
  } else {
    ...
    if (matches) {
      matches.forEach((m) => issues.push({ sequence: m, char: 'Carácter roto (?)', count: 1, fixed: false }));
    }
  }

  return { issues, content: newContent, fixed: (fixMode && newContent !== content) };
}
  ...
  try {
    ...
    if (result.issues.length > 0) {
      ...
      result.issues.forEach(i => {
        console.log(`  [${i.fixed ? 'FIXED' : 'WARN'}] '${i.sequence}' -> '${i.char}' (${i.count} veces)`);
      });
      if (result.fixed) {
