...
async function main() {
    if (!fs.existsSync(LOG_FILE)) {
        console.error(`No se encontró el archivo ${LOG_FILE}. Ejecuta 'npm run typecheck > ${LOG_FILE}' primero.`);
        return;
    }
    ...
    const propertyRegex = /error TS2339: Property ['"](.+)['"] does not exist on type ['"](.+)['"]\./;
    const fileLineRegex = /^(.+\.tsx?)\((\d+),\d+\):/;

    // Regex para Module '"..."' has no exported member '...'
    const exportRegex = /error TS2305: Module .+ has no exported member ['"](.+)['"]\./;
    ...
    if (propertyErrors.length > 0) {
        ...
        propertyErrors.forEach(err => {
            console.log(`  - Property '${err.prop}' field missing in type '${err.type}' at ${err.file}:${err.line}`);
        });
        console.log(`Total: ${propertyErrors.length}\n`);
    }

    if (missingExports.length === 0 && propertyErrors.length === 0) {
        console.log('✅ No se detectaron errores críticos de exportación o propiedades en el log.');
    }
