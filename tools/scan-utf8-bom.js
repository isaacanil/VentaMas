...
if (fixMode) {
    console.log('🔧 Modo corrección activado: Se eliminará el BOM de los archivos afectados.');
}
    ...
    if (checkForBOM(file)) {
        ...
        if (fixMode) {
            try {
                ...
                console.error(`❌ Error al corregir ${path.relative(rootDir, file)}: ${err.message}`);
            }
        } else {
...
if (!foundIssues) {
    ...
    if (fixMode) {
        ...
        console.log("👉 Ejecuta este script con '--fix' para eliminarlos automáticamente: node tools/scan-utf8-bom.js --fix");
    }
}
